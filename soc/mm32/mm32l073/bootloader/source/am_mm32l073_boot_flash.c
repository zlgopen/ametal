...
static int __boot_flash_erase_region(void     *p_drv,
                                     ...
                                     uint32_t  length)
{
    am_mm32l073_boot_flash_dev_t *p_dev = (am_mm32l073_boot_flash_dev_t *)p_drv;
    ...
    if((length & (sector_size - 1)) != 0) {
     ...
     for (i = 0 ; i < erase_sector_count; i++) {
         ...
         ret = am_mm32_flash_sector_erase(
             (amhw_mm32_flash_t *)p_dev->amhw_mm32_flash, i * sector_size + start_addr);
                                ...
                                uint32_t   size)
{
    ...
    for(i = 0; i < program_sector_count; i++) {
        ...
        key = am_int_cpu_lock();
        ret = am_mm32_flash_flash_program((amhw_mm32_flash_t *)p_dev->amhw_mm32_flash,
 ...
 */
am_boot_flash_handle_t am_mm32l073_boot_flash_init(am_mm32l073_boot_flash_dev_t     *p_dev,
                                                 am_mm32l073_boot_flash_devinfo_t *p_devinfo)
{
    if(p_dev == NULL || p_devinfo == NULL) {
        return NULL;
    }
    p_dev->amhw_mm32_flash = (amhw_mm32_flash_t *)p_devinfo->flash_reg_rebese;
    ...
    am_mm32_flash_init(p_dev->amhw_mm32_flash);

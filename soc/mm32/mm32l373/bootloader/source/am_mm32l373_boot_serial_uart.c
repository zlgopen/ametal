...
#define UART_CH       MM32L373_UART1_BASE  /**< \brief UART通道 */
...
{
    am_mm32l373_boot_serial_uart_dev_t  *p_dev = (am_mm32l373_boot_serial_uart_dev_t *)p_arg;
...
{
    uint8_t data;
    if (amhw_mm32_uart_int_flag_check((amhw_mm32_uart_t *)UART_CH, AMHW_MM32_UART_INT_RX_VAL_FLAG) == AM_TRUE) {

        amhw_mm32_uart_int_flag_clr((amhw_mm32_uart_t *)UART_CH,AMHW_MM32_UART_INT_RX_VAL_FLAG);
        /* 获取新接收数据，并发送出去 */
        data = amhw_mm32_uart_data_read((amhw_mm32_uart_t *)UART_CH);

        __g_uart_byte_receive_callback(data);
    }
}

static void __mm32l373_uart_int_init()
{
    /* 使能RDRF接收准中断 */
    amhw_mm32_uart_int_enable((amhw_mm32_uart_t *)UART_CH, AMHW_MM32_UART_INT_RX_VAL_ENABLE);
    amhw_mm32_uart_int_flag_clr((amhw_mm32_uart_t *)UART_CH,AMHW_MM32_UART_INT_RX_VAL_ENABLE);
...
static am_mm32l373_boot_serial_uart_dev_t __g_serial_uart_dev;

am_boot_serial_handle_t am_mm32l373_boot_serial_uart_init(am_boot_autobaud_handle_t autobaud_handle)
{
    int ret;
    uint32_t baund;
    ...
    am_mm32l373_boot_autobaud_inst_deinit (autobaud_handle);

    uart_handle = am_mm32l373_uart1_inst_init();

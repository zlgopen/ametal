...
/** \brief 中断未连接标识 */
#define AM_MM32L073_GPIO_INVALID_PIN_MAP    0xFF

/** \brief 定义指向GPIO设备信息的指针 */
#define __GPIO_DEVINFO_DECL(p_gpio_devinfo, p_dev)  \
        const am_mm32l073_gpio_devinfo_t *p_gpio_devinfo = p_dev->p_devinfo
...
/** \bruef 指向GPIO设备的指针 */
am_mm32l073_gpio_dev_t *__gp_gpio_dev;
...
int am_gpio_pin_cfg (int pin, uint32_t flags)
{
    ...
    amhw_mm32_gpio_t *p_hw_gpio  = NULL;

    if (__gp_gpio_dev == NULL) {
        return -AM_ENXIO;
    }

    p_hw_gpio = (amhw_mm32_gpio_t*)p_gpio_devinfo->gpio_regbase;
    ...
    if (func != 0x00) {
        ...
        /* 表明不需要再设置方向 */
        flags &= ~AM_MM32L073_GPIO_DIR;

        /* 表明不需要再设置引脚复用功能，仅使用GPIO功能 */
        flags &= ~AM_MM32L073_GPIO_FUNEN;

        if (amhw_mm32_gpio_pin_afr_get(p_hw_gpio, pin) != AMHW_MM32_GPIO_AF_DEFAULT) {

            /* 复用功能回到默认状态，表明使用GPIO功能 */
            amhw_mm32_gpio_pin_afr_set(p_hw_gpio, AMHW_MM32_GPIO_AF_DEFAULT, pin);
        ...
        case AM_GPIO_OUTPUT_INIT_HIGH:
            ...
            if ((flags & AM_MM32L073_GPIO_OUTRES_RATE) == 0) {
                AM_BITS_SET(iocon_cfg, 0, 2, AMHW_MM32_GPIO_SPEED_10MHz);
            }
    ...
    if (mode != 0x00) {
        ...
        case AM_GPIO_PULLUP:
            if (dir != 1) {
                ...
                AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_IPU);
            } else {
                AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_OUT_PP);
            }
            break;
        ...
        case AM_GPIO_PULLDOWN:
            if (dir != 1) {
                ...
                /* 使OD位置为0, 表明是下拉 */
                amhw_mm32_gpio_pin_out_low(p_hw_gpio, pin);
                AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_IPD);
            } else {
                AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_OUT_PP);
            }
            break;
        ...
        case AM_GPIO_FLOAT:
            if (dir != 1) {
                AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_IN_FLOATING);
            } else {
                AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_OUT_OD);
            }
            break;
    ...
    /* 管脚的模式 */
    if ((flags & AM_MM32L073_GPIO_MODE) != 0) {

       switch (AM_MM32L073_GPIO_MODE_GET(flags)) {

       case 0:
           AM_BITS_SET(iocon_cfg, 0, 2, 0x00);
           AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_AIN);
       ...
       case 6:
           AM_BITS_SET(iocon_cfg, 0, 2, AMHW_MM32_GPIO_SPEED_10MHz);
           AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_AF_PP);

           break;

       case 7:
           AM_BITS_SET(iocon_cfg, 0, 2, AMHW_MM32_GPIO_SPEED_10MHz);
           AM_BITS_SET(iocon_cfg, 2, 2, AMHW_MM32_GPIO_MODE_AF_OD);
           break;
    ...
    if ((flags & AM_MM32L073_GPIO_OUTRES_RATE) != 0) {
        ...
        AM_BITS_SET(iocon_cfg, 0, 2, AM_MM32L073_GPIO_OUTRES_RATE_GET(flags));

    }

    /* 设置复用功能时管脚模式 */
    amhw_mm32_gpio_pin_set(p_hw_gpio, iocon_cfg, pin);

    /* 设置管脚的复用功能 */
    if ((flags & AM_MM32L073_GPIO_FUNEN) != 0) {
        amhw_mm32_gpio_pin_afr_set(p_hw_gpio, AM_MM32L073_GPIO_FUNC_GET(flags), pin);
    }
...
int am_gpio_get (int pin)
{
    ...
    amhw_mm32_gpio_t *p_hw_gpio = (amhw_mm32_gpio_t *)p_gpio_devinfo->gpio_regbase;

    return amhw_mm32_gpio_pin_get(p_hw_gpio, pin);
}
...
int am_gpio_toggle (int pin)
{
    ...
    amhw_mm32_gpio_pin_toggle(p_hw_gpio, pin);
...
int am_gpio_trigger_cfg (int pin, uint32_t flag)
{
    ...
    amhw_mm32_gpio_t   *p_hw_gpio   = NULL;
    amhw_mm32_syscfg_t *p_hw_syscfg = NULL;
    ...
    p_hw_gpio   = (amhw_mm32_gpio_t   *)p_gpio_devinfo->gpio_regbase;
    p_hw_syscfg = (amhw_mm32_syscfg_t *)p_gpio_devinfo->syscfg_regbase;
    p_hw_exti   = (amhw_mm32_exti_t   *)p_gpio_devinfo->exti_regbase;
    ...
    /* 设置管脚为输入方向 */
    amhw_mm32_gpio_pin_dir_input(p_hw_gpio, pin);

    /* 管脚为上拉/下拉模式 */
    amhw_mm32_gpio_pin_mode_set(p_hw_gpio, AMHW_MM32_GPIO_MODE_IPD, pin);

    /*
     * 中断线0只能连一个端口编号为0的GPIO引脚，比如说，PIOA_0连接到了中断线0，
     * PIOB_0、PIOC_0、PIOD_0就不能连到中断线0, 但此时PIOA_1可以连接到中断线1。
     */
    amhw_mm32_syscfg_exti_line_config(p_hw_syscfg, port, slot);

    /* 清除中断线配置 */
    amhw_mm32_exti_imr_clear(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));
    amhw_mm32_exti_emr_clear(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));

    /* 清除中断线触发方式配置 */
    amhw_mm32_exti_rtsr_clear(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));
    amhw_mm32_exti_ftsr_clear(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));
    ...
    case AM_GPIO_TRIGGER_RISE:          /* 上升沿触发 */
        ...
        /* rising  edge */
        amhw_mm32_exti_rtsr_set(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));
    ...
    case AM_GPIO_TRIGGER_FALL:          /* 下降沿触发 */
        ...
        /* falling  edge */
        amhw_mm32_exti_ftsr_set(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));
...
int am_gpio_trigger_on (int pin)
{
    ...
    /* 清引脚中断线标志位 */
    amhw_mm32_exti_pending_clear(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));

    /* 设置开放中断线请求 */
    amhw_mm32_exti_imr_set(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot));
...
static void __port_exit0_1_int_isr (void *p_arg)
{
    ...
    /* 判断中断线挂起标志是否置位 */
    if (amhw_mm32_exti_pr_read(p_hw_exti, (amhw_mm32_exti_line_num_t)(1UL << slot)) != 0) {
                             ...
                             void         *p_arg)
{
    ...
    if ((p_gpio_devinfo->p_infomap)[extipinsource] == AM_MM32L073_GPIO_INVALID_PIN_MAP) {
        slot = extipinsource;
    }  else {
        already = AM_TRUE;
    }
                                ...
                                void         *p_arg)
{
    ...
    (p_gpio_devinfo->p_infomap)[slot]               = AM_MM32L073_GPIO_INVALID_PIN_MAP;
 ...
 * \brief GPIO初始化
 */
int am_mm32l073_gpio_init (am_mm32l073_gpio_dev_t           *p_dev,
                         const am_mm32l073_gpio_devinfo_t *p_devinfo)
{
    int i;
    ...
    if (p_dev->valid_flg) {
        for (i = 0; i < p_devinfo->pint_count; i++) {
            (p_devinfo->p_infomap)[i]              = AM_MM32L073_GPIO_INVALID_PIN_MAP;
 ...
 * \brief GPIO去初始化
 */
void am_mm32l073_gpio_deinit (void)
{
    __GPIO_DEVINFO_DECL(p_gpio_devinfo, __gp_gpio_dev);
    ...
    if (__gp_gpio_dev->valid_flg) {
        for (i = 0; i < p_gpio_devinfo->pint_count; i++) {
            (p_gpio_devinfo->p_infomap)[i]               = AM_MM32L073_GPIO_INVALID_PIN_MAP;

...
am_local void __spi_cs_on  (am_mm32_spi_int_dev_t *p_this, am_spi_device_t *p_dev);
am_local void __spi_cs_off (am_mm32_spi_int_dev_t *p_this, am_spi_device_t *p_dev);

am_local void __spi_write_data (am_mm32_spi_int_dev_t *p_dev);
am_local void __spi_read_data (am_mm32_spi_int_dev_t *p_dev);

am_local void __spi_irq_handler (void *p_arg);
am_local int  __spi_hard_init (am_mm32_spi_int_dev_t *p_this);
am_local int  __spi_config (am_mm32_spi_int_dev_t *p_this);

am_local uint32_t __spi_speed_cfg (am_mm32_spi_int_dev_t *p_dev, uint32_t target_baud);

am_local int  __spi_mst_sm_event (am_mm32_spi_int_dev_t *p_dev, uint32_t event);
/*******************************************************************************
  SPI驱动函数声明
*******************************************************************************/
 ...
 */
am_local
void __spi_cs_on (am_mm32_spi_int_dev_t *p_this, am_spi_device_t *p_dev)
{
 ...
 */
am_local
void __spi_cs_off (am_mm32_spi_int_dev_t *p_this,
                   am_spi_device_t      *p_dev)
{
    if (p_this->p_tgl_dev == p_dev) {
        p_this->p_tgl_dev = NULL;
...
am_local
void __spi_write_data (am_mm32_spi_int_dev_t *p_dev)
{
    ...
    while(!(amhw_mm32_spi_reg_cstat_get (p_hw_spi) & AMHW_MM32_SPI_CSTAT_TX_EMPTY));
    ...
    if (p_trans->p_txbuf != NULL) {
        if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
            ...
            amhw_mm32_spi_tx_data8_put(p_hw_spi, *ptr);

        } else {
            uint32_t *ptr = (uint32_t *)(p_trans->p_txbuf) + p_dev->data_ptr;
            amhw_mm32_spi_tx_data32_put(p_hw_spi, *ptr);
        }
    ...
    /* tx_buf 无效 */
        ...
        /* 待发送数据无效 直接发0 */
        if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
            amhw_mm32_spi_tx_data8_put(p_hw_spi, 0x00);
        } else {
            amhw_mm32_spi_tx_data32_put(p_hw_spi, 0x0000);
        }
    }
...
am_local
void __spi_read_data (am_mm32_spi_int_dev_t *p_dev)
{
    amhw_mm32_spi_t *p_hw_spi = (amhw_mm32_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    if (amhw_mm32_spi_reg_cstat_get (p_hw_spi) & AMHW_MM32_SPI_CSTAT_RXVAL) {

        /* rx_buf 有效 */
        if (p_trans->p_rxbuf != NULL && p_dev->nbytes_to_recv) {
            if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
                 *p_buf8 = amhw_mm32_spi_rx_data8_get(p_hw_spi);
            } else {
                 *p_buf32 = amhw_mm32_spi_rx_data32_get(p_hw_spi);
            }

        /* rx_buf 无效或者不需要接收数据 */
        } else {
            if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
                (void)amhw_mm32_spi_rx_data8_get(p_hw_spi);

            } else {
                (void)amhw_mm32_spi_rx_data32_get(p_hw_spi);
            }
        }
 ...
 */
am_static_inline
void __spi_msg_in (am_mm32_spi_int_dev_t *p_dev,
 ...
 */
am_static_inline
struct am_spi_message *__spi_msg_out (am_mm32_spi_int_dev_t *p_dev)
{
...
{
    am_mm32_spi_int_dev_t *p_this = (am_mm32_spi_int_dev_t *)p_arg;
 ...
 */
am_local
int __spi_hard_init (am_mm32_spi_int_dev_t *p_this)
{
...
{
    am_mm32_spi_int_dev_t  *p_dev    = (am_mm32_spi_int_dev_t *)p_arg;
    amhw_mm32_spi_t        *p_hw_spi = (amhw_mm32_spi_t *)(p_dev->p_devinfo->spi_reg_base);

    uint32_t spi_status = (uint32_t)amhw_mm32_spi_reg_intstat_get(p_hw_spi);

    /* 发送数据 */
    if((spi_status & AMHW_MM32_SPI_INTSTAT_TX_FALG)) {
        amhw_mm32_spi_int_flag_clr(p_hw_spi, AMHW_MM32_SPI_INTSTAT_TX_FALG);
        __spi_mst_sm_event(p_dev, __SPI_EVT_M_SEND_DATA);
    }

    /* 接收数据 */
    if((spi_status & AMHW_MM32_SPI_INTSTAT_RX_FALG) != 0) {
        amhw_mm32_spi_int_flag_clr(p_hw_spi, AMHW_MM32_SPI_INTSTAT_RX_FALG);
        __spi_mst_sm_event(p_dev, __SPI_EVT_M_RECV_DATA);
    }
}

am_local
int __spi_config (am_mm32_spi_int_dev_t *p_this)
{
    const am_mm32_spi_int_devinfo_t *p_devinfo = p_this->p_devinfo;
...
int __spi_msg_start (void              *p_drv,
                     ...
                     am_spi_message_t  *p_msg)
{
    am_mm32_spi_int_dev_t *p_this = (am_mm32_spi_int_dev_t *)p_drv;
 ...
 */
am_local
int __spi_mst_sm_event (am_mm32_spi_int_dev_t *p_dev, uint32_t event)
{
    amhw_mm32_spi_t *p_hw_spi = (amhw_mm32_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    while (1) {
        ...
        case __SPI_ST_MSG_START:    /* 消息开始 */
        {
            ...
            if (p_cur_msg) {
                ...
                /* 禁能所有中断 */
                amhw_mm32_spi_int_disable(p_hw_spi, AMHW_MM32_SPI_INTSTAT_INT_ALL);
                p_dev->busy = AM_FALSE;
            }
            am_int_cpu_unlock(key);
        ...
        case __SPI_ST_M_SEND_DATA:    /* 发送数据 */
        {
            ...
            if (p_dev->data_ptr >= p_cur_trans->nbytes) {
                ...
                if ((amhw_mm32_spi_reg_cstat_get(p_hw_spi) & AMHW_MM32_SPI_CSTAT_TX_EMPTY) != 0) {
                    ...
                    /* Enable RX interrupt */
                    amhw_mm32_spi_int_enable(p_hw_spi, AMHW_MM32_SPI_INTSTAT_RX_FALG);
                } else {

                    /* 打开中断等待发送就绪 */
                    amhw_mm32_spi_int_enable(p_hw_spi, AMHW_MM32_SPI_INTSTAT_TX_FALG);
                }
            }
            break;
        }
        ...
        case __SPI_ST_M_RECV_DATA:   /* 接收数据 */
        {
            ...
            if (p_dev->nbytes_to_recv == 0) {
                ...
                /* 禁能发送及接收中断 */
                amhw_mm32_spi_int_disable(p_hw_spi, AMHW_MM32_SPI_INTSTAT_RX_FALG |
                                                      AMHW_MM32_SPI_INTSTAT_TX_FALG);
 ...
 * \brief SPI初始化
 */
am_spi_handle_t am_mm32_spi_int_init (am_mm32_spi_int_dev_t           *p_dev,
                                     const am_mm32_spi_int_devinfo_t *p_devinfo)
{
 ...
 * \brief SPI 去除初始化
 */
void am_mm32_spi_int_deinit (am_spi_handle_t handle)
{
    am_mm32_spi_int_dev_t *p_dev    = (am_mm32_spi_int_dev_t *)handle;
    amhw_mm32_spi_t       *p_hw_spi = (amhw_mm32_spi_t *)(p_dev->p_devinfo->spi_reg_base);
 ...
 */
am_local
uint32_t __spi_speed_cfg (am_mm32_spi_int_dev_t *p_dev, uint32_t target_speed)
{

...
static int __wwdg_info_get (void *p_drv, am_wdt_info_t *p_info)
{
    am_mm32_wwdg_dev_t    *p_dev        = (am_mm32_wwdg_dev_t *)p_drv;
...
{
    am_mm32_wwdg_dev_t *p_dev     = (am_mm32_wwdg_dev_t *)p_drv;
    amhw_mm32_wwdg_t   *p_hw_wwdg =
                           (amhw_mm32_wwdg_t *)p_dev->p_devinfo->wwdg_regbase;

    amhw_mm32_wwdg_counter_set(p_hw_wwdg, p_dev->count);
...
am_wdt_handle_t
am_mm32_wwdg_init (am_mm32_wwdg_dev_t              *p_dev,
                     const am_mm32_wwdg_devinfo_t *p_devinfo)
{
    if (p_devinfo == NULL) {
        return NULL;
    }
...
/** \brief WWDT解初始化 */
void am_mm32_wwdg_deinit (am_wdt_handle_t handle)
{
    am_mm32_wwdg_dev_t *p_dev = (am_mm32_wwdg_dev_t *)handle;

...
#ifndef __AM_ZSN700_UART_H
#define __AM_ZSN700_UART_H
...
/**
 * \addtogroup am_zsn700_if_uart
 * \copydoc am_zsn700_uart.h
 * @{
 */
 ...
 *        便用户判断具体的中断源，size为1。
 */
#define AM_ZSN700_UART_ERRCODE_UART_OTHER_INT  3

/**
 * \brief 用于软件流控的XON字符定义，默认0x11
 */
#define AM_ZSN700_UART_XON       0x11

/**
 * \brief 用于软件流控的XOFF字符定义，默认0x13
 */
#define AM_ZSN700_UART_XOFF      0x13
 ...
 */
typedef struct am_zsn700_uart_devinfo {

    uint32_t uart_reg_base;     /**< \brief 指向UART寄存器块的指针 */

    uint8_t  inum;              /**< \brief 串口中断号 */

    amhw_zsn700_uart_work_mode_t work_mode; /**< \brief 串口工作模式 */
    ...
    /**
     * \brief 指定使能的中断,AMHW_ZSN700_UART_INT_*宏值或多个AMHW_ZSN700_UART_INT_*宏
     */
...
} am_zsn700_uart_devinfo_t;

/**
 * \brief 串口设备结构体定义
 */
typedef struct am_zsn700_uart_dev {
    ...
    const am_zsn700_uart_devinfo_t *p_devinfo; /**< \brief 指向设备信息常量的指针 */

} am_zsn700_uart_dev_t;

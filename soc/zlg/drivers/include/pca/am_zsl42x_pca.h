...
typedef enum am_zsl42x_pca_int {
    ZSL42x_PCA_CF   = (0x1 << 7),   /* 计数器溢出标志 */
    ZSL42x_PCA_CCF4 = (0x1 << 4),   /* 计数器模块4比较/捕获标志位 */
    ZSL42x_PCA_CCF3 = (0x1 << 3),   /* 计数器模块3比较/捕获标志位 */
    ZSL42x_PCA_CCF2 = (0x1 << 2),   /* 计数器模块2比较/捕获标志位 */
    ZSL42x_PCA_CCF1 = (0x1 << 1),   /* 计数器模块1比较/捕获标志位 */
    ZSL42x_PCA_CCF0 = (0x1 << 0)    /* 计数器模块0比较/捕获标志位 */
} am_zsl42x_pca_int_t;
...
typedef enum am_zsl42x_pca_clk {
    ...
    ZSL42x_PCA_EC1     = (0x7),  /* ECI 外部时钟，时钟PCLK四分频采样 */
} am_zsl42x_pca_clk_t;

/* 比较捕获模式控制枚举 */
typedef enum am_zsl42x_pca_ccapm {
    ZSL42x_PCA_ECOM = (0x1 << 6),   /* 允许比较器功能控制位 */
    ZSL42x_PCA_MAT  = (0x1 << 3),   /* 允许匹配控制位 */
    ZSL42x_PCA_PWM  = (0x1 << 1),   /* 脉冲调制控制位 */
    ZSL42x_PCA_CCIE = (0x1 << 0)    /* PCA使能中断 */
} am_zsl42x_pca_ccapm_t;

/* 边沿捕获枚举 */
typedef enum am_zsl42x_pca_capn {
    ZSL42x_PCA_CAPP_EN  = (0x1 << 1),   /* 允许正沿捕获 */
    ZSL42x_PCA_CAPP_DIS = (0x0 << 1),   /* 禁止正沿捕获 */
    ZSL42x_PCA_CAPN_EN  = (0x1),        /* 允许负沿捕获 */
    ZSL42x_PCA_CAPN_DIS = (0x0),        /* 禁止负沿捕获 */
} am_zsl42x_pca_capn_t;

/* 翻转控制枚举 */
typedef enum am_zsl42x_pca_tog {
    ZSL42x_PCA_TOG_EN   = (0x1),   /* 使能翻转 */
    ZSL42x_PCA_TOG_DIS  = (0x0),   /* 禁能翻转 */
} am_zsl42x_pca_tog_t;

/* PWM类型选择 */
typedef enum am_zsl42x_pca_pwm {
    ZSL42x_PCA_PWM_8  = 0,   /* 8位PWM */
    ZSL42x_PCA_PWM_16 = 1    /* 16位PWM */
} am_zsl42x_pca_pwm_t;
...
typedef struct am_zsl42x_pca_devinfo {
    ...
    uint16_t                inum;               /* 中断号 */
    am_zsl42x_pca_clk_t     clk_src;            /* 时钟源选择 */
    am_zsl42x_pca_ioinfo_t  *ioinfo;            /* 引脚配置信息 */

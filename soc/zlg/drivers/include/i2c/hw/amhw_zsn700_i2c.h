...
typedef enum amhw_zsn700_status_flag{
    ...
    /* 无指定模式状态 */
    AMHW_ZSN700_STATUS_FLAG_0   = (0x00),   /**< \brief 出现总线错误或I2C进入未定义的状态 */
    AMHW_ZSN700_STATUS_FLAG_1   = (0x08),   /**< \brief 已发送起始条件 */
    AMHW_ZSN700_STATUS_FLAG_2   = (0x10),   /**< \brief 已发送重复起始条件 */

    /* 主发送器状态 */
    AMHW_ZSN700_STATUS_FLAG_3   = (0x18),   /**< \brief 已发送SLA+W，已接收ACK */
    AMHW_ZSN700_STATUS_FLAG_4   = (0x20),   /**< \brief 已发送SLA+W，已接收非ACK */
    AMHW_ZSN700_STATUS_FLAG_5   = (0x28),   /**< \brief 已发送数据，已接收ACK */
    AMHW_ZSN700_STATUS_FLAG_6   = (0x30),   /**< \brief 已发送数据 */
    AMHW_ZSN700_STATUS_FLAG_7   = (0x38),   /**< \brief 在SLA+ 读写或写数据字节时丢失仲裁 */

    /* 主接收器状态 */
    AMHW_ZSN700_STATUS_FLAG_8   = (0x40),   /**< \brief 已发送SLA +R,已接收ACK */
    AMHW_ZSN700_STATUS_FLAG_9   = (0x48),   /**< \brief 已发送SLA +R,已接收非ACK */
    AMHW_ZSN700_STATUS_FLAG_10  = (0x50),   /**< \brief 已接收数据字节，ACK已返回 */
    AMHW_ZSN700_STATUS_FLAG_11  = (0x58),   /**< \brief 已接收数据字节，非ACK已返回 */

    /* 从接收器状态 */
    AMHW_ZSN700_STATUS_FLAG_12  = (0x60),   /**< \brief 已接收自身的SLA+W；已接收ACK */
    AMHW_ZSN700_STATUS_FLAG_13  = (0x68),   /**< \brief 主控时在SLA+读写丢失仲裁；已接收自身的SLA+W；已返回ACK； */
    AMHW_ZSN700_STATUS_FLAG_14  = (0x70),   /**< \brief 已接收通用调用地址（0x00）；已返回ACK */
    AMHW_ZSN700_STATUS_FLAG_15  = (0x78),   /**< \brief 主控时在SLA+读写中丢失仲裁；已接收通用调用地址；已返回ACK； */
    AMHW_ZSN700_STATUS_FLAG_16  = (0x80),   /**< \brief 前一次寻址使用自身从地址；已接收数据字节；已返回ACK； */
    AMHW_ZSN700_STATUS_FLAG_17  = (0x88),   /**< \brief 前一次寻址使用自身从地址；已接收数据字节；已返回非ACK； */
    AMHW_ZSN700_STATUS_FLAG_18  = (0x90),   /**< \brief 前一次寻址使用通用调用地址；已接收数据；已返回ACK； */
    AMHW_ZSN700_STATUS_FLAG_19  = (0x98),   /**< \brief 前一次寻址使用通用调用地址；已接收数据；已返回非ACK； */
    AMHW_ZSN700_STATUS_FLAG_20  = (0xa0),   /**< \brief 当使用从接收/从发送模式中静态寻址时，接收到停止条件或重复起始条件 */

    /* 从发送器状态 */
    AMHW_ZSN700_STATUS_FLAG_22  = (0xa8),   /**< \brief 已接收自身的SLA+R；已返回ACK */
    AMHW_ZSN700_STATUS_FLAG_23  = (0xb0),   /**< \brief 当主控时在SLA+ 读写中丢失仲裁；已接收自身SLA+R；已返回ACK； */
    AMHW_ZSN700_STATUS_FLAG_24  = (0xb8),   /**< \brief 已发送数据；已接收ACK； */
    AMHW_ZSN700_STATUS_FLAG_25  = (0xc0),   /**< \brief 已发送数据字节；已接收非ACK; */
    AMHW_ZSN700_STATUS_FLAG_26  = (0xc8),   /**< \brief 装入的数据字节已被发送；已接收ACK； */

    /* 其它状态 */
    AMHW_ZSN700_STATUS_FLAG_27  = (0xf8)    /**< \brief 无可用的相关状态信息；si=0； */
 ...
 *
 * \param[in]  p_hw_i2c  : 指向I2C寄存器结构体的指针
 * \param[in]  flag      : i2c配置标志， see:AMHW_ZSN700_I2C_HIGH_SPEED
 *
 * \return 无
 */
am_static_inline

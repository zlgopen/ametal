 ...
 * \brief PWR 驱动接口声明
 *
 * 该驱动依赖于 RCC 和 CLK 模块，因位RCC 和 CLK 仅支持 ZML165， 
 * 所以该驱动也支持 ZML165
 *
 * \internal
...
#ifndef __AM_ZML165_PWR_H
#define __AM_ZML165_PWR_H
...
/**
 * \addtogroup am_zml165_if_pwr
 * \copydoc am_zml165_pwr.h
 * @{
 */
 ...
 * \brief PWR 模式
 */
typedef enum am_zml165_pwr_mode {
    AM_ZML165_PWR_MODE_RUN = 0,      /**< \brief 运行模式 */
    AM_ZML165_PWR_MODE_SLEEP,        /**< \brief 睡眠模式 */
    AM_ZML165_PWR_MODE_STOP,         /**< \brief 停止模式 */
    AM_ZML165_PWR_MODE_STANBY,       /**< \brief 待机模式 */
} am_zml165_pwr_mode_t;

/** \brief PWR PVD 监测电压信息配置 */
typedef struct am_zml165_pwr_pvd_info {
...
} am_zml165_pwr_pvd_info_t;

/**
 * \brief PWR 模式及唤醒引脚
 */
typedef struct am_zml165_pwr_mode_init {
    am_zml165_pwr_mode_t  pwr_mode; /**< \brief 指向 PWR 寄存器块的指针 */
    int                   pin;      /**< \brief 唤醒引脚 */
} am_zml165_pwr_mode_init_t;

/**
 * \brief PWR 设备信息结构体
 */
typedef struct am_zml165_pwr_devinfo {
    ...
    am_zml165_pwr_mode_init_t *p_pwr_mode;   /**< \brief 指向 PWR 模式数组的指针 */

    am_zml165_pwr_pvd_info_t  *p_pvd_info;   /**< \brief 指向 PVD 电压监控信息的指针 */

    void (*pfn_plfm_init)(void);             /**< \brief 平台初始化函数 */

    void (*pfn_plfm_deinit)(void);           /**< \brief 平台去初始化函数 */

} am_zml165_pwr_devinfo_t;

/**
 * \brief PWR 设备结构体
 */
typedef struct am_zml165_pwr_dev {

    /** \brief 指向 PWR 设备信息指针 */
    const am_zml165_pwr_devinfo_t *p_pwrdevinfo;

    /** \brief PWR 模式 */
    am_zml165_pwr_mode_t           pwr_mode;

} am_zml165_pwr_dev_t;

/** \brief PWR 设备操作句柄类型定义 */
typedef am_zml165_pwr_dev_t *am_zml165_pwr_handle_t;
 ...
 *
 * \return 返回 am_zml165_pwr_handle_t 类型的句柄
 */
am_zml165_pwr_handle_t
am_zml165_pwr_init (am_zml165_pwr_dev_t           *p_dev,
 ...
 * \brief 获取系统模式
 *
 * \param[in] mode         PWR 模式定义，值为 am_zml165_pwr_mode_t 这一枚举类型
 * \param[in] pfn_callback 唤醒回调函数
 * \param[in] p_arg        唤醒回调函数 参数
 *
 * \return 无
 *
 * \note 这个唤醒的配置的 PWR 模式必须与 am_zml165_pwr_mode_into 函数的选择 PWR
 *       模式相同，在 AM_ZML165_PWR_MODE_STANBY 模式下，唤醒会复位，注册中断唤醒
 *       函数回调函数不起作用
 */
 ...
 *       2. 配置 PWR 模式为 AM_ZML165_PWR_MODE_STANBY 失败后，WKUP 引脚会被配置为下拉
 *          输入模式
 */
 ...
 * \param[in] 无
 *
 * \return am_zml165_pwr_mode_t 这一枚举类型的值
 */

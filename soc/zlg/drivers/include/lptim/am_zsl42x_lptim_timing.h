...
#ifndef __AM_ZSL42x_LPTIM_LPTIMING_H
#define __AM_ZSL42x_LPTIM_LPTIMING_H
...
/**
 * \addtogroup am_zsl42x_if_lptim_timing
 * \copydoc am_zsl42x_lptim_timing.h
 * @{
 */
 ...
 */
typedef struct am_zsl42x_lptim_timing_devinfo {

    uint32_t               lptim_regbase;   /**< \brief LPTIM寄存器块基址 */

    uint8_t                inum;            /**< \brief LPTIM中断号 */

    amhw_zsl42x_lptim_clk_src_t   clk_src;  /**< \brief 计数/计时时钟源选择 */

    /** \brief 计数/计时功能选择.选择计数时需要初始化相关ETR引脚，对外部输入的下降沿进行计数 */
    amhw_zsl42x_lptim_function_t  fun_sel;

    amhw_zsl42x_lptim_mode_t      mode;     /**< \brief 重载/不重载模式选择  */
...
} am_zsl42x_lptim_timing_devinfo_t;

/**
 * \brief LPTIM定时功能设备
 */
typedef struct am_zsl42x_lptim_timing_dev {
 ...
 */
am_timer_handle_t
am_zsl42x_lptim_timing_init (am_zsl42x_lptim_timing_dev_t           *p_dev,
                               const am_zsl42x_lptim_timing_devinfo_t *p_devinfo);

/**
 * \brief 不使用LPTIM定时功能时，解初始化LPTIM定时功能，释放相关资源
 *
 * \param[in] handle : am_zsl42x_lptim_timing_init() 初始化函数获得的Timer服务句柄
 *
 * \return 无
 */
...
#endif /* __AM_ZSL42x_LPTIM_LPTIMING_H */

...
static int __crc_init (void *p_cookie, am_crc_pattern_t *p_pattern)
{
    am_zlg118_crc_dev_t *p_dev      = (am_zlg118_crc_dev_t *)p_cookie;
    amhw_zlg118_crc_t   *p_hw_crc   = NULL;
    ...
    p_hw_crc = (amhw_zlg118_crc_t *)(p_dev->p_devinfo->crc_reg_base);

    /* 根据模型选择校验方式（crc-16 或  crc-32） */
    if (p_dev->p_pattern->width == 32){

        amhw_zlg118_crc_cal_way(p_hw_crc, AM_ZLG118_CRC_32);

        /* 初始化结果寄存器值 */
        amhw_zlg118_crc_res_32data_init(p_hw_crc);
    }else{

        amhw_zlg118_crc_cal_way(p_hw_crc, AM_ZLG118_CRC_16);

        /* 初始化结果寄存器值 */
        amhw_zlg118_crc_res_16data_init(p_hw_crc);
    }
...
static int __crc_cal (void *p_cookie, const uint8_t *p_data, uint32_t nbytes)
{
    ...
    uint32_t tdata;
    am_zlg118_crc_dev_t *p_dev    = (am_zlg118_crc_dev_t *)p_cookie;
    amhw_zlg118_crc_t   *p_hw_crc =
        (amhw_zlg118_crc_t *)(p_dev->p_devinfo->crc_reg_base);
    ...
    for (i = 0; i < nbytes; i+=4) {
        ...
        amhw_zlg118_crc_32bit_write_data(p_hw_crc, tdata);
    }
...
static int __crc_final (void *p_cookie, uint32_t *p_value)
{
    ...
    if (p_dev->p_pattern->width == 32){
        ...
        *p_value = amhw_zlg118_crc_32bit_read_data(p_hw_crc);
    }else {

        *p_value = amhw_zlg118_crc_16bit_read_data(p_hw_crc);
    }
 ...
 * \brief CRC初始化
 */
am_crc_handle_t am_zlg118_crc_init (am_zlg118_crc_dev_t           *p_dev,
                                    const am_zlg118_crc_devinfo_t *p_devinfo)
{
 ...
 * \brief CRC解初始化
 */
void am_zlg118_crc_deinit (am_crc_handle_t handle)
{
    am_zlg118_crc_dev_t *p_dev    = (am_zlg118_crc_dev_t *)handle;

 ...
 * \brief 串口模式（查询或中断）设置
 */
int __lpuart_mode_set (am_zlg118_lpuart_dev_t *p_dev, uint32_t new_mode);

/**
 * \brief 串口硬件设置
 */
int __lpuart_opt_set (am_zlg118_lpuart_dev_t *p_dev, uint32_t opts);

/**
 * \brief 流控接受器流控状态设置
 */
static int __lpuart_flow_rxstat_set (am_zlg118_lpuart_dev_t *p_dev, uint32_t ctrl);

/**
 * \brief 流控模式设置
 */
static int __lpuart_flow_mode_set (am_zlg118_lpuart_dev_t *p_dev, uint32_t mode);

/**
 * \brief 流控发送器流控状态获取
 */
static int __lpuart_flow_txstat_get (am_zlg118_lpuart_dev_t *p_dev);
...
{
    am_zlg118_lpuart_dev_t *p_dev     = (am_zlg118_lpuart_dev_t *)p_drv;
    amhw_zlg118_lpuart_t   *p_hw_lpuart = (amhw_zlg118_lpuart_t *)
    ...
    case AM_UART_BAUD_SET:
        ...
        while (amhw_zlg118_lpuart_flag_check(
                   p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_TX_EMPTY) == AM_FALSE);

        status = amhw_zlg118_lpuart_baudrate_set(p_hw_lpuart,
                                                 p_dev->clk_rate,
                                                 (uint32_t)p_arg);
...
int __lpuart_tx_startup (void *p_drv)
{
    ...
    if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
        amhw_zlg118_lpuart_data_write(p_hw_lpuart, data);
    }

    /* 使能发送完成中断 */
    amhw_zlg118_lpuart_int_enable(p_hw_lpuart,
                                  AMHW_ZLG118_LPUART_INT_TX_COMPLETE);
...
{
    am_zlg118_lpuart_dev_t *p_dev     = (am_zlg118_lpuart_dev_t *)p_drv;
    amhw_zlg118_lpuart_t   *p_hw_lpuart = (amhw_zlg118_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;

    /* 发送模块是否空闲, AM_FALSE:忙; TURE: 空闲 */
    if(amhw_zlg118_lpuart_flag_check(
           p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_TX_EMPTY) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {
        ...
        /* 发送一个字符 */
        amhw_zlg118_lpuart_data_write(p_hw_lpuart, outchar);
...
static int __lpuart_poll_getchar (void *p_drv, char *p_char)
{
    ...
    if(amhw_zlg118_lpuart_flag_check(
           p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_RX_COMPLETE) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {

        /* 接收一个字符 */
        *p_inchar = amhw_zlg118_lpuart_data_read(p_hw_lpuart);
...
int __lpuart_mode_set (am_zlg118_lpuart_dev_t *p_dev, uint32_t new_mode)
{
    ...
    if (new_mode == AM_UART_MODE_INT) {
        ...
        /* 使能接收完成中断 */
        amhw_zlg118_lpuart_int_enable(p_hw_lpuart,
                                      AMHW_ZLG118_LPUART_INT_RX_COMPLETE);
    } else {
 ...
 * \brief 串口选项配置
 */
int __lpuart_opt_set (am_zlg118_lpuart_dev_t *p_dev, uint32_t options)
{
    amhw_zlg118_lpuart_t *p_hw_lpuart = (amhw_zlg118_lpuart_t *)
                                        p_dev->p_devinfo->lpuart_reg_base;
    ...
    if (options & AM_UART_STOPB) {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZLG118_LPUART_STOP_2_0_BIT;
    } else {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZLG118_LPUART_STOP_1_0_BIT;
    }
    ...
    if (options & AM_UART_PARENB) {
        ...
        if (options & AM_UART_PARODD) {
            cfg_flags |= AMHW_ZLG118_LPUART_HW_PARITY_ODD;
        } else {
            cfg_flags |= AMHW_ZLG118_LPUART_HW_PARITY_EVEN;
        }
    } else {
        cfg_flags &= ~(0x3ul << 2);
        cfg_flags |= AMHW_ZLG118_LPUART_PARITY_NO;
    }

    /* 保存和生效配置 */
    amhw_zlg118_lpuart_stop_bit_sel(p_hw_lpuart  , (cfg_flags & (0x3ul << 14)));
    amhw_zlg118_lpuart_parity_bit_sel(p_hw_lpuart, (cfg_flags & (0x3ul << 2)));
 ...
 * \brief 接收器流控状态设置（开或关）
 */
static int __lpuart_flow_rxstat_set (am_zlg118_lpuart_dev_t *p_dev,
                                     uint32_t                ctrl)
{
    amhw_zlg118_lpuart_t *p_hw_lpuart = NULL;

    if (NULL == p_dev) {
        return -AM_EINVAL;
    }

    p_hw_lpuart = (amhw_zlg118_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;
    ...
    /* 如果是硬件流控，配置RTS输出引脚电位 */
        ...
        if (AM_UART_FLOWSTAT_ON == ctrl) {
            amhw_zlg118_lpuart_data_write(p_hw_lpuart, AM_ZLG118_LPUART_XON);
        } else {
            amhw_zlg118_lpuart_data_write(p_hw_lpuart, AM_ZLG118_LPUART_XOFF);
        }
    }
...
static int __lpuart_flow_mode_set(am_zlg118_lpuart_dev_t *p_dev, uint32_t mode)
{
    ...
    if(AM_UART_FLOWCTL_NO == p_dev->flowctl_mode) {
        ...
        /* 硬件流控禁能     */
        amhw_zlg118_lpuart_disable(p_hw_lpuart, AMHW_ZLG118_LPUART_CTS);
        amhw_zlg118_lpuart_disable(p_hw_lpuart, AMHW_ZLG118_LPUART_RTS);
    ...
    /* 硬件流控 */
        ...
        amhw_zlg118_lpuart_enable(p_hw_lpuart, AMHW_ZLG118_LPUART_CTS);
        amhw_zlg118_lpuart_enable(p_hw_lpuart, AMHW_ZLG118_LPUART_RTS);
    } else {
...
static int __lpuart_flow_txstat_get (am_zlg118_lpuart_dev_t *p_dev)
{
    ...
    if(AM_UART_FLOWCTL_HW == p_dev->flowctl_mode) {
        ...
        /* cts的状态发生变化时，状态位有效 */
        if(amhw_zlg118_lpuart_flag_check(
               p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_CTS_TRIGGER) == AM_TRUE) {

            /* 清除cts状态位 */
            amhw_zlg118_lpuart_flag_clr(p_hw_lpuart,
                                        AMHW_ZLG118_LPUART_FLAG_CTS_TRIGGER);
 ...
 * \brief 串口接收中断服务
 */
void __lpuart_irq_rx_handler (am_zlg118_lpuart_dev_t *p_dev)
{
    amhw_zlg118_lpuart_t *p_hw_lpuart = (amhw_zlg118_lpuart_t *)
                                        p_dev->p_devinfo->lpuart_reg_base;
    char data;
    ...
    /* 是否为接收Rx中断 */
    if (amhw_zlg118_lpuart_flag_check(
            p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_RX_COMPLETE) == AM_TRUE) {

        amhw_zlg118_lpuart_flag_clr(p_hw_lpuart,
                                    AMHW_ZLG118_LPUART_FLAG_RX_COMPLETE);

        /* 获取新接收数据 */
        data = amhw_zlg118_lpuart_data_read(p_hw_lpuart);
 ...
 * \brief 串口发送中断服务
 */
void __lpuart_irq_tx_handler (am_zlg118_lpuart_dev_t *p_dev)
{
    amhw_zlg118_lpuart_t *p_hw_lpuart = (amhw_zlg118_lpuart_t *)
                                        p_dev->p_devinfo->lpuart_reg_base;
    ...
    if (amhw_zlg118_lpuart_flag_check(
            p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_TX_COMPLETE) == AM_TRUE) {

        amhw_zlg118_lpuart_flag_clr(p_hw_lpuart,
                                    AMHW_ZLG118_LPUART_FLAG_TX_COMPLETE);
        ...
        if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
            ...
            /* 没有数据传送就关闭发送中断 */
            amhw_zlg118_lpuart_int_disable(p_hw_lpuart,
                                           AMHW_ZLG118_LPUART_FLAG_TX_COMPLETE);
...
{
    am_zlg118_lpuart_dev_t  *p_dev     = (am_zlg118_lpuart_dev_t *)p_arg;
    amhw_zlg118_lpuart_t    *p_hw_lpuart = (amhw_zlg118_lpuart_t *)
                                       p_dev->p_devinfo->lpuart_reg_base;

    uint32_t lpuart_int_stat = amhw_zlg118_lpuart_flag_get(p_hw_lpuart);
    ...
    } else if (amhw_zlg118_lpuart_flag_check(
                   p_hw_lpuart, AMHW_ZLG118_LPUART_FLAG_TX_COMPLETE) == AM_TRUE) {
    ...
    if ((p_dev->other_int_enable & lpuart_int_stat) != 0) {
        ...
        if (p_dev->pfn_err != NULL) {
            p_dev->pfn_err(p_dev->err_arg,
                           AM_ZLG118_LPUART_ERRCODE_LPUART_OTHER_INT,
                           (void *)p_hw_lpuart,
                           1);
        }
    }
...
int __lpuart_connect (void *p_drv)
{
    ...
    am_int_enable(p_dev->p_devinfo->inum);
    amhw_zlg118_lpuart_int_enable(p_dev->p_devinfo->lpuart_reg_base,
 ...
 * \brief 串口初始化函数
 */
am_uart_handle_t am_zlg118_lpuart_init (
                     am_zlg118_lpuart_dev_t           *p_dev,
                     const am_zlg118_lpuart_devinfo_t *p_devinfo)
{
    amhw_zlg118_lpuart_t  *p_hw_lpuart;
    ...
    /* 获取配置参数 */
    p_hw_lpuart                = (amhw_zlg118_lpuart_t  *)
    ...
    /* 设置通信时钟源 */
    amhw_zlg118_lpuart_clk_src_sel(p_hw_lpuart, p_devinfo->sclk_src);

    if(p_devinfo->sclk_src == AMHW_ZLG118_LPUART_SCLK_SRC_XTL) {
    ...
    } else if(p_devinfo->sclk_src == AMHW_ZLG118_LPUART_SCLK_SRC_RCL) {
    ...
    if(p_devinfo->work_mode == AMHW_ZLG118_LPUART_WORK_MODE_0) {

        /* 模式0通信时钟分频系数设置无效 */
        amhw_zlg118_lpuart_clk_div_sel(p_hw_lpuart,
                                       AMHW_ZLG118_LPUART_SCLK_DIV_MODE0_NO);
    } else {

        /* 设置通信时钟分频系数为最小 */
        amhw_zlg118_lpuart_clk_div_sel(p_hw_lpuart,
                                       AMHW_ZLG118_LPUART_SCLK_DIV_MODE2_8);
    }
    ...
    if((p_devinfo->work_mode == 2) || (p_devinfo->work_mode == 3)) {
        ...
        if((p_devinfo->mut_addr.enable == AM_TRUE)) {
            ...
            amhw_zlg118_lpuart_slaver_addr_set(p_hw_lpuart,
            ...
            /* 多机地址自动识别使能 */
            amhw_zlg118_lpuart_enable(p_hw_lpuart,
                                      AMHW_ZLG118_LPUART_MUT_ADR_AUTO);
        }
    }

    /* 获取串口检验方式配置选项 */
    if (p_devinfo->cfg_flags & (AMHW_ZLG118_LPUART_HW_PARITY_ODD)) {
        p_dev->options |= AM_UART_PARENB | AM_UART_PARODD;
    } else if(p_devinfo->cfg_flags & (AMHW_ZLG118_LPUART_HW_PARITY_EVEN)){
    ...
    /* 获取串口停止位配置选项 */
    if (p_devinfo->cfg_flags & (AMHW_ZLG118_LPUART_STOP_2_0_BIT)) {
    ...
    /* 设置波特率 */
    p_dev->baud_rate = amhw_zlg118_lpuart_baudrate_set(p_hw_lpuart,
                                                       p_dev->clk_rate,
                                                       p_devinfo->baud_rate);
    /* 默认轮询模式 */
...
void am_zlg118_lpuart_deinit (am_zlg118_lpuart_dev_t *p_dev)
{
    ...
    amhw_zlg118_lpuart_disable(p_hw_lpuart,AMHW_ZLG118_LPUART_RX);
    amhw_zlg118_lpuart_disable(p_hw_lpuart,AMHW_ZLG118_LPUART_CTS);
    amhw_zlg118_lpuart_disable(p_hw_lpuart,AMHW_ZLG118_LPUART_RTS);
    amhw_zlg118_lpuart_disable(p_hw_lpuart,AMHW_ZLG118_LPUART_DMA_TX);
    amhw_zlg118_lpuart_disable(p_hw_lpuart,AMHW_ZLG118_LPUART_DMA_RX);
    amhw_zlg118_lpuart_disable(p_hw_lpuart,AMHW_ZLG118_LPUART_MUT_ADR_AUTO);

 ...
 * \brief 串口模式（查询或中断）设置
 */
int __lpuart_mode_set (am_zsl42x_lpuart_dev_t *p_dev, uint32_t new_mode);

/**
 * \brief 串口硬件设置
 */
int __lpuart_opt_set (am_zsl42x_lpuart_dev_t *p_dev, uint32_t opts);
...
{
    am_zsl42x_lpuart_dev_t *p_dev       = (am_zsl42x_lpuart_dev_t *)p_drv;
    amhw_zsl42x_lpuart_t   *p_hw_lpuart =
            (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;
    ...
    case AM_UART_BAUD_SET:
        ...
        /* 只有在发送空的基础上才允许修改波特率 */
        while (amhw_zsl42x_lpuart_flag_check(
                   p_hw_lpuart, AMHW_ZSL42x_LPUART_FLAG_TX_EMPTY) == AM_FALSE);

        status = amhw_zsl42x_lpuart_baudrate_set(p_hw_lpuart,
                                               p_dev->clk_rate,
                                               (uint32_t)p_arg);
...
int __lpuart_tx_startup (void *p_drv)
{
    ...
    /* 等待上一次传输完成 */
    while (amhw_zsl42x_lpuart_flag_check(p_hw_lpuart,
                                       AMHW_ZSL42x_LPUART_FLAG_TX_EMPTY) ==
                                       AM_FALSE);

    /* 获取发送数据并发送 */
    if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
        amhw_zsl42x_lpuart_data_write(p_hw_lpuart, data);
    }

    /* 使能发送完成中断 */
    amhw_zsl42x_lpuart_int_enable(p_hw_lpuart,
                                AMHW_ZSL42x_LPUART_INT_TX_COMPLETE);
...
{
    am_zsl42x_lpuart_dev_t *p_dev     = (am_zsl42x_lpuart_dev_t *)p_drv;
    amhw_zsl42x_lpuart_t   *p_hw_lpuart = (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;

    /* 发送模块是否空闲, AM_FALSE:忙; TURE: 空闲 */
    if(amhw_zsl42x_lpuart_flag_check(
           p_hw_lpuart, AMHW_ZSL42x_LPUART_FLAG_TX_EMPTY) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {
        ...
        /* 发送一个字符 */
        amhw_zsl42x_lpuart_data_write(p_hw_lpuart, outchar);
...
static int __lpuart_poll_getchar (void *p_drv, char *p_char)
{
    ...
    if(amhw_zsl42x_lpuart_flag_check(
           p_hw_lpuart, AMHW_ZSL42x_LPUART_FLAG_RX_COMPLETE) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {

        /* 接收一个字符 */
        *p_inchar = amhw_zsl42x_lpuart_data_read(p_hw_lpuart);

        amhw_zsl42x_lpuart_flag_clr(p_hw_lpuart,
                                  AMHW_ZSL42x_LPUART_FLAG_RX_COMPLETE);
    }
...
int __lpuart_mode_set (am_zsl42x_lpuart_dev_t *p_dev, uint32_t new_mode)
{
    ...
    if (new_mode == AM_UART_MODE_INT) {
        ...
        /* 使能接收完成中断 */
        amhw_zsl42x_lpuart_int_enable(p_hw_lpuart,
                                    AMHW_ZSL42x_LPUART_INT_RX_COMPLETE);
    } else {

        /* 关闭所有串口中断 */
        amhw_zsl42x_lpuart_int_disable(p_hw_lpuart, AMHW_ZSL42x_LPUART_INT_ALL);
    }
 ...
 * \brief 串口选项配置
 */
int __lpuart_opt_set (am_zsl42x_lpuart_dev_t *p_dev, uint32_t options)
{
    amhw_zsl42x_lpuart_t *p_hw_lpuart =
            (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;
    ...
    if (options & AM_UART_STOPB) {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZSL42x_LPUART_STOP_2_0_BIT;
    } else {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZSL42x_LPUART_STOP_1_0_BIT;
    }
    ...
    if (options & AM_UART_PARENB) {
        ...
        if (options & AM_UART_PARODD) {
            cfg_flags |= AMHW_ZSL42x_LPUART_HW_PARITY_ODD;
        } else {
            cfg_flags |= AMHW_ZSL42x_LPUART_HW_PARITY_EVEN;
        }
        amhw_zsl42x_lpuart_mode_sel(p_hw_lpuart, AMHW_ZSL42x_LPUART_WORK_MODE_3);
    } else {
        cfg_flags &= ~(0x3ul << 2);
        cfg_flags |= AMHW_ZSL42x_LPUART_PARITY_NO;
        amhw_zsl42x_lpuart_mode_sel(p_hw_lpuart, AMHW_ZSL42x_LPUART_WORK_MODE_1);
    }

    /* 保存和生效配置 */
    amhw_zsl42x_lpuart_stop_bit_sel(p_hw_lpuart  , (cfg_flags & (0x3ul << 14)));
    amhw_zsl42x_lpuart_parity_bit_sel(p_hw_lpuart, (cfg_flags & (0x3ul << 2)));
 ...
 * \brief 串口接收中断服务
 */
void __lpuart_irq_rx_handler (am_zsl42x_lpuart_dev_t *p_dev)
{
    amhw_zsl42x_lpuart_t *p_hw_lpuart =
            (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;
    char data;
    ...
    /* 是否为接收Rx中断 */
    if (amhw_zsl42x_lpuart_flag_check(
            p_hw_lpuart, AMHW_ZSL42x_LPUART_FLAG_RX_COMPLETE) == AM_TRUE) {

        amhw_zsl42x_lpuart_flag_clr(p_hw_lpuart,
                                  AMHW_ZSL42x_LPUART_FLAG_RX_COMPLETE);

        /* 获取新接收数据 */
        data = amhw_zsl42x_lpuart_data_read(p_hw_lpuart);
 ...
 * \brief 串口发送中断服务
 */
void __lpuart_irq_tx_handler (am_zsl42x_lpuart_dev_t *p_dev)
{
    amhw_zsl42x_lpuart_t *p_hw_lpuart =
            (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;
    ...
    if (amhw_zsl42x_lpuart_flag_check(
            p_hw_lpuart, AMHW_ZSL42x_LPUART_FLAG_TX_COMPLETE) == AM_TRUE) {

        amhw_zsl42x_lpuart_flag_clr(p_hw_lpuart,
                                  AMHW_ZSL42x_LPUART_FLAG_TX_COMPLETE);
        ...
        if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
            ...
            /* 没有数据传送就关闭发送中断 */
            amhw_zsl42x_lpuart_int_disable(p_hw_lpuart,
                                         AMHW_ZSL42x_LPUART_FLAG_TX_COMPLETE);
...
{
    am_zsl42x_lpuart_dev_t  *p_dev       = (am_zsl42x_lpuart_dev_t *)p_arg;
    amhw_zsl42x_lpuart_t    *p_hw_lpuart =
            (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;

    uint32_t lpuart_int_stat = amhw_zsl42x_lpuart_flag_get(p_hw_lpuart);
    ...
    } else if (amhw_zsl42x_lpuart_flag_check(
                   p_hw_lpuart, AMHW_ZSL42x_LPUART_FLAG_TX_COMPLETE) == AM_TRUE) {
    ...
    if ((p_dev->other_int_enable & lpuart_int_stat) != 0) {
        ...
        if (p_dev->pfn_err != NULL) {
            p_dev->pfn_err(p_dev->err_arg,
                           AM_ZSL42x_LPUART_ERRCODE_LPUART_OTHER_INT,
                           (void *)p_hw_lpuart,
                           1);
        }
    }
 ...
 * \brief 串口初始化函数
 */
am_uart_handle_t am_zsl42x_lpuart_init (
                     am_zsl42x_lpuart_dev_t           *p_dev,
                     const am_zsl42x_lpuart_devinfo_t *p_devinfo)
{
    amhw_zsl42x_lpuart_t  *p_hw_lpuart;
    ...
    /* 获取配置参数 */
    p_hw_lpuart                =
            (amhw_zsl42x_lpuart_t  *)p_devinfo->lpuart_reg_base;
    ...
    /* 设置通信时钟源 */
    amhw_zsl42x_lpuart_clk_src_sel(p_hw_lpuart, p_devinfo->sclk_src);

    if(p_devinfo->sclk_src == AMHW_ZSL42x_LPUART_SCLK_SRC_XTL) {
    ...
    } else if(p_devinfo->sclk_src == AMHW_ZSL42x_LPUART_SCLK_SRC_RCL) {

        /* 设置RCL时钟频率校准值 （38.4KHz）*/
        amhw_zsl42x_rcc_rcl_trim_set(*((uint16_t*)0x00100C20));
    ...
    if (AM_TRUE == p_devinfo->asyn_hd) {
        amhw_zsl42x_lpuart_single_line_half_enable(p_hw_lpuart);
    }

    /* 获取串口检验方式配置选项 */
    if ((p_devinfo->cfg_flags & (0xc)) == (AMHW_ZSL42x_LPUART_HW_PARITY_ODD)) {
        p_dev->options |= AM_UART_PARENB | AM_UART_PARODD;
    } else if((p_devinfo->cfg_flags & (0xc)) == (AMHW_ZSL42x_LPUART_HW_PARITY_EVEN)){
    ...
    /* 获取串口停止位配置选项 */
    if (p_devinfo->cfg_flags & (AMHW_ZSL42x_LPUART_STOP_2_0_BIT)) {
    ...
    if(amhw_zsl42x_lpuart_mode_get(p_hw_lpuart) == AMHW_ZSL42x_LPUART_WORK_MODE_0) {

        /* 模式0通信时钟分频系数设置无效 */
        amhw_zsl42x_lpuart_clk_div_sel(p_hw_lpuart,
                                       AMHW_ZSL42x_LPUART_SCLK_DIV_MODE0_NO);
    ...
    /* 模式2情况下，波特率 = 时钟 / 分频系数 */
        ...
        /* 设置通信时钟分频系数为最小 */
        amhw_zsl42x_lpuart_clk_div_sel(p_hw_lpuart,
                                       AMHW_ZSL42x_LPUART_SCLK_DIV_MODE2_8);
    }
    
    /* 设置波特率 */
    p_dev->baud_rate = amhw_zsl42x_lpuart_baudrate_set(p_hw_lpuart,
                                                     p_dev->clk_rate,
                                                     p_devinfo->baud_rate);
    /* 默认轮询模式 */
     ...
     */
    amhw_zsl42x_lpuart_enable(p_hw_lpuart, AMHW_ZSL42x_LPUART_RX);
 ...
 * \brief 串口去初始化
 */
void am_zsl42x_lpuart_deinit (am_zsl42x_lpuart_dev_t *p_dev)
{
    amhw_zsl42x_lpuart_t *p_hw_lpuart =
            (amhw_zsl42x_lpuart_t *)p_dev->p_devinfo->lpuart_reg_base;
    ...
    amhw_zsl42x_lpuart_disable(p_hw_lpuart,AMHW_ZSL42x_LPUART_RX);
    amhw_zsl42x_lpuart_disable(p_hw_lpuart,AMHW_ZSL42x_LPUART_CTS);
    amhw_zsl42x_lpuart_disable(p_hw_lpuart,AMHW_ZSL42x_LPUART_RTS);
    amhw_zsl42x_lpuart_disable(p_hw_lpuart,AMHW_ZSL42x_LPUART_DMA_TX);
    amhw_zsl42x_lpuart_disable(p_hw_lpuart,AMHW_ZSL42x_LPUART_DMA_RX);
    amhw_zsl42x_lpuart_disable(p_hw_lpuart,AMHW_ZSL42x_LPUART_MUT_ADR_AUTO);

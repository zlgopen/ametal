 ...
 * \brief 等待 RTC 寄存器同步
 */
am_local void __sync_wait (amhw_zmf237_rtc_t *p_hw_rtc)
{
    while (amhw_zmf237_rtc_crl_read_statu(p_hw_rtc, AMHW_ZMF237_RTC_RSF) == 0);
}

/**
 * \brief 等待 RTC 操作完成
 */
am_local void __operation_wait (amhw_zmf237_rtc_t *p_hw_rtc)
{
    while (amhw_zmf237_rtc_crl_read_statu(p_hw_rtc, AMHW_ZMF237_RTC_RTOFF) == 0);
}
...
am_local int __rtc_time_get (void *p_drv, am_tm_t *p_tm)
{
    am_zmf237_rtc_dev_t *p_dev    = (am_zmf237_rtc_dev_t *)p_drv;
    amhw_zmf237_rtc_t   *p_hw_rtc = (amhw_zmf237_rtc_t  *)p_dev->p_devinfo->rtc_regbase;
    uint32_t             sec;
    ...
    if (AM_OK != am_zmf237_rtc_cnt_get(&p_dev->rtc_serv, &sec)) {
        return -AM_EINVAL;
    }
...
am_local int __rtc_time_set (void *p_drv, am_tm_t *p_tm)
{
    ...
    am_zmf237_rtc_cnt_set(&p_dev->rtc_serv, sec);
...
{
    am_zmf237_rtc_dev_t *p_dev = (am_zmf237_rtc_dev_t *)p_arg;
    amhw_zmf237_rtc_t   *p_hw_rtc = NULL;

    p_hw_rtc =(amhw_zmf237_rtc_t *)p_dev->p_devinfo->rtc_regbase;

    /* 秒中断 */
    if (amhw_zmf237_rtc_crl_read_statu(p_hw_rtc, AMHW_ZMF237_RTC_SECF)) {
        amhw_zmf237_rtc_clr_status_clear(p_hw_rtc, AMHW_ZMF237_RTC_SECF);
    ...
    /* 闹钟中断 */
    if (amhw_zmf237_rtc_crl_read_statu(p_hw_rtc, AMHW_ZMF237_RTC_ALRF)) {
        amhw_zmf237_rtc_clr_status_clear(p_hw_rtc, AMHW_ZMF237_RTC_ALRF);
    ...
    /* 溢出中断 */
    if (amhw_zmf237_rtc_crl_read_statu(p_hw_rtc, AMHW_ZMF237_RTC_OWF)) {
        amhw_zmf237_rtc_clr_status_clear(p_hw_rtc, AMHW_ZMF237_RTC_OWF);
...
{
    am_zmf237_rtc_dev_t *p_dev    = NULL;

    if (NULL == handle) {
        return AM_FALSE;
    }

    p_dev = (am_zmf237_rtc_dev_t *)AM_CONTAINER_OF(handle, am_zmf237_rtc_dev_t, rtc_serv);
...
am_err_t am_zmf237_rtc_apb1_reflush (am_rtc_handle_t handle)
{
    ...
    amhw_zmf237_rtc_clr_status_clear(p_hw_rtc, AMHW_ZMF237_RTC_RSF);
...
am_err_t am_zmf237_rtc_cnt_get (am_rtc_handle_t handle, uint32_t *p_cnt)
{
    ...
    *p_cnt = (amhw_zmf237_rtc_cnth_get(p_hw_rtc) << 16) +
             amhw_zmf237_rtc_cntl_get(p_hw_rtc);
...
am_err_t am_zmf237_rtc_cnt_set (am_rtc_handle_t handle, uint32_t cnt)
{
    ...
    amhw_zmf237_rtc_clr_status_clear(p_hw_rtc,             /* 软件清零 RSF 标志位 */
                                     AMHW_ZMF237_RTC_RSF);
    amhw_zmf237_rtc_crl_cnf_enter(p_hw_rtc);               /* 允许配置 */
    amhw_zmf237_rtc_cnth_set(p_hw_rtc, cnt >> 16);         /* 写入高 16 位 */
    amhw_zmf237_rtc_cntl_set(p_hw_rtc, cnt & 0xffff);      /* 写入低 16 位 */
    amhw_zmf237_rtc_crl_cnf_out(p_hw_rtc);                 /* 配置更新 */
    __operation_wait(p_hw_rtc);                            /* 等待 RTC 操作完成 */
...
am_err_t am_zmf237_rtc_alr_set (am_rtc_handle_t handle, uint32_t alr)
{
    ...
    __operation_wait(p_hw_rtc);                       /* 等待 RTC 操作完成 */
    amhw_zmf237_rtc_crl_cnf_enter(p_hw_rtc);          /* 允许配置 */
    amhw_zmf237_rtc_alrh_set(p_hw_rtc, alr >> 16);    /* 写入高 16 位 */
    amhw_zmf237_rtc_alrl_set(p_hw_rtc, alr & 0xFFFF); /* 写入低 16 位 */
    amhw_zmf237_rtc_crl_cnf_out(p_hw_rtc);            /* 配置更新 */
    __operation_wait(p_hw_rtc);                       /* 等待 RTC 操作完成 */
...
am_err_t am_zmf237_rtc_alarm_set (am_rtc_handle_t handle, am_tm_t *p_tm)
{
    ...
    am_zmf237_rtc_alr_set(handle, sec);
                                     ...
                                     void           *p_arg)
{
    ...
    if (AM_ZMF237_RTC_CALLBACK_SECOND == type) {
        p_dev->pfn_callback[0] = pfn_callback;
        p_dev->p_arg[0] = p_arg;
    } else if (AM_ZMF237_RTC_CALLBACK_ALARM == type) {
        p_dev->pfn_callback[1] = pfn_callback;
        p_dev->p_arg[1] = p_arg;
    } else if (AM_ZMF237_RTC_CALLBACK_OVER == type) {
                                   ...
                                   uint8_t         type)
{
    ...
    if (AM_ZMF237_RTC_CALLBACK_SECOND == type) {
        amhw_zmf237_rtc_crh_allow_int(p_hw_rtc, AMHW_ZMF237_RTC_SECIE);
        p_dev->int_state |= AM_ZMF237_RTC_CALLBACK_SECOND;
    } else if (AM_ZMF237_RTC_CALLBACK_ALARM == type) {
        amhw_zmf237_rtc_crh_allow_int(p_hw_rtc, AMHW_ZMF237_RTC_ALRIE);
        p_dev->int_state |= AM_ZMF237_RTC_CALLBACK_ALARM;
    } else if (AM_ZMF237_RTC_CALLBACK_OVER == type) {
        amhw_zmf237_rtc_crh_allow_int(p_hw_rtc, AMHW_ZMF237_RTC_OWIE);
        p_dev->int_state |= AM_ZMF237_RTC_CALLBACK_OVER;
    } else {
        return -AM_EINVAL;
    }
                                    ...
                                    uint8_t         type)
{
    ...
    if (AM_ZMF237_RTC_CALLBACK_SECOND == type) {
        amhw_zmf237_rtc_crh_forbid_int(p_hw_rtc, AMHW_ZMF237_RTC_SECIE);
        p_dev->int_state &= ~AM_ZMF237_RTC_CALLBACK_SECOND;
    } else if (AM_ZMF237_RTC_CALLBACK_ALARM == type) {
        amhw_zmf237_rtc_crh_forbid_int(p_hw_rtc, AMHW_ZMF237_RTC_ALRIE);
        p_dev->int_state &= ~AM_ZMF237_RTC_CALLBACK_ALARM;
    } else if (AM_ZMF237_RTC_CALLBACK_OVER == type) {
        amhw_zmf237_rtc_crh_forbid_int(p_hw_rtc, AMHW_ZMF237_RTC_OWIE);
        p_dev->int_state &= ~AM_ZMF237_RTC_CALLBACK_OVER;
    } else {
        return -AM_EINVAL;
    }
                                    ...
                                    const am_zmf237_rtc_devinfo_t *p_devinfo)
{
    ...
    amhw_zmf237_bkp_t *p_hw_bkp = NULL;

    if ((NULL == p_dev) || (NULL == p_devinfo)) {
        return NULL;
    }

    p_hw_rtc                = (amhw_zmf237_rtc_t *)p_devinfo->rtc_regbase;
    p_hw_pwr                = (amhw_zlg_pwr_t *)p_devinfo->pwr_regbase;
    p_hw_bkp                = (amhw_zmf237_bkp_t *)p_devinfo->bkp_regbase;
    ...
    if (0x5A5A != amhw_zmf237_bkp_dr_read(p_hw_bkp, 0)) {
        ...
        switch (p_dev->p_devinfo->rtc_clk_sour){
            ...
            case AMHW_ZMF237_RTCCLK_LSE:

                /* 使能 LSE */
                amhw_zmf237_rcc_bdcr_lseon_enable();
                while (!amhw_zmf237_rcc_bdcr_lserdy_read());

                prl_val = 32768 - 1;
                break;

            case AMHW_ZMF237_RTCCLK_LSI:

                /* 使能 LSI */
                amhw_zmf237_rcc_lsi_enable();
                while (!amhw_zmf237_rcc_lsirdy_read());

                prl_val = 40000 - 1;
                break;

            case AMHW_ZMF237_RTCCLK_HSE_DIV128:

                /* 使能 HSE */
                amhw_zmf237_rcc_hseon_enable();
                while (!amhw_zmf237_rcc_hserdy_read());
        ...
        /* RTC 时钟源选择 */
        amhw_zmf237_rcc_bdcr_rtc_clk_set((amhw_zmf237_rtc_clk_src)p_devinfo->rtc_clk_sour);

        /* RTC 时钟使能 */
        amhw_zmf237_rcc_bdcr_rtc_enable();
        ...
        __operation_wait(p_hw_rtc); /* 等待 RTC 操作完成 */
        amhw_zmf237_rtc_clr_status_clear(p_hw_rtc, AMHW_ZMF237_RTC_RSF);
        amhw_zmf237_rtc_crl_cnf_enter(p_hw_rtc); /* 允许配置 RTC */
        amhw_zmf237_rtc_prlh_div_write(p_hw_rtc, prl_val >> 16);
        amhw_zmf237_rtc_prll_div_write(p_hw_rtc, prl_val & 0xffff);
        amhw_zmf237_rtc_crl_cnf_out(p_hw_rtc);   /* 配置更新 */
        __operation_wait(p_hw_rtc); /* 等待 RTC 操作完成 */

        amhw_zmf237_bkp_dr_write(ZMF237_BKP, 0, 0x5A5A);

        p_dev->rtc_continue = AM_FALSE;
    } else {
        if (AMHW_ZMF237_RTCCLK_LSI == p_dev->p_devinfo->rtc_clk_sour) {

            /* 使能 LSI */
            amhw_zmf237_rcc_lsi_enable();
            while (!amhw_zmf237_rcc_lsirdy_read());
        } else if (AMHW_ZMF237_RTCCLK_HSE_DIV128 == p_dev->p_devinfo->rtc_clk_sour) {
...
{
    am_zmf237_rtc_dev_t *p_dev    = (am_zmf237_rtc_dev_t *)handle;
    amhw_zlg_pwr_t   *p_hw_pwr    = NULL;
    ...
    amhw_zmf237_bkp_dr_write(ZMF237_BKP, 0, 0);             /* 清除标记 */
    ...
    amhw_zmf237_rcc_bdcr_rtc_disable();                     /* RTC 时钟失能 */

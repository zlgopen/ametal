...
#include "am_zmf237_usbd.h"
#include "amhw_zmf237_usbd.h"
#include "am_usb.h"
#include "am_int.h"
...
{
    amhw_zmf237_usbd_t *p_usb = (amhw_zmf237_usbd_t *)p_dev->p_info->usb_regbase;
    ...
    amhw_zmf237_usbd_ep_halt_reset(p_usb, 1 << endpoint);     /* 清除端点暂停 */
    amhw_zmf237_usbd_ep_enable(p_usb, 1 << endpoint);         /* 使能端点 */
                                                    ...
                                                    uint8_t            endpoint)
{
    ...
    amhw_zmf237_usbd_ep_disable(p_usb, 1 << endpoint);  /* 禁能端点 */
                                                   ...
                                                   uint8_t            endpoint_addr)
{
    ...
    amhw_zmf237_usbd_ep_halt_set(p_usb, 1 << endpoint);
                                                      ...
                                                      uint8_t               endpoint_addr)
{
    ...
    amhw_zmf237_usbd_dtog_data0(p_usb, endpoint);

    amhw_zmf237_usbd_ep_halt_reset(p_usb, 1 << endpoint);
...
static am_usb_status_t __usb_device_init (am_usbd_handle_t handle)
{
    ...
    amhw_zmf237_usbd_t   *p_usb = (amhw_zmf237_usbd_t *)p_dev->p_info->usb_regbase;
    /* 复位USB控制器的端点和FIFO */
    amhw_zmf237_usbd_connect_set(p_usb, ZMF237_USB_DISCONNECT);
    amhw_zmf237_usbd_reset_set(p_usb, ZMF237_USB_RESET);
    amhw_zmf237_usbd_reset_set(p_usb, ZMF237_USB_NORESET);
    amhw_zmf237_usbd_connect_set(p_usb, ZMF237_USB_DISCONNECT);

    /* 清中断状态 */
    amhw_zmf237_usbd_int_state_clear(p_usb, AMHW_ZMF237_USB_INT_STATE_ALL);

    /* 清端点中断状态 */
    amhw_zmf237_usbd_ep_int_state_clear(p_usb, AMHW_ZMF237_EP_INT_STATE_EP_ALL);
    p_dev->int_ep_union.int_ep_flag = 0;

    amhw_zmf237_usbd_ep0_int_state_clear(p_usb, AMHW_ZMF237_EP0_INT_STATE_ALL);
    p_dev->ep_int_type_union.ep_int_type[0] = 0;

    for (i = 0; i < AM_USBD_MAX_EP_CNT - 1; i++) {
        amhw_zmf237_usbd_epx_int_state_clear(p_usb, (zmf237_usb_epx_t)i, AMHW_ZMF237_EPX_INT_STATE_ALL);
    ...
    /* 如果使能了SOF中断的话那每隔1ms就会进入一次中断，如果不使能SOF中断，中断标志也会置位，但是不会
                 进入中断     */
    amhw_zmf237_usbd_int_enable(p_usb,
                                AMHW_ZMF237_USB_INT_EN_RSTIE |
                                AMHW_ZMF237_USB_INT_EN_SUSPENDIE |
                                AMHW_ZMF237_USB_INT_EN_RESUMIE |
                                AMHW_ZMF237_USB_INT_EN_EPIE);

    /* 使能端点中断 */
    for (i = 0; i < AM_USBD_MAX_EP_CNT; i++) {
        if (p_dev->device.endpoint_info[i].inuse == AM_TRUE)
            amhw_zmf237_usbd_ep_int_enable(p_usb, 1 << i);
    }

    /* 使能端点0的所有中断 */
    amhw_zmf237_usbd_ep0_int_enable(p_usb, AMHW_ZMF237_EP0_INT_EN_ALL);
    /* 使能其他端点的所有中断 */
    for (i = 0; i < AM_USBD_MAX_EP_CNT - 1; i++) {
        if (p_dev->device.endpoint_info[i + 1].inuse == AM_TRUE)
            amhw_zmf237_usbd_epx_int_enable(p_usb, (zmf237_usb_epx_t)i, AMHW_ZMF237_EPX_INT_EN_ALL);
    }
    ...
    /* 设置地址为0 */
    amhw_zmf237_usbd_addr_set(p_usb, 0);
    p_dev->device.device_address = 0;

    amhw_zmf237_usbd_active_set(p_usb, ZMF237_USB_ACTIVE);      /* 活跃 */
    amhw_zmf237_usbd_speed_set(p_usb, ZMF237_USB_SPEED_FULL);   /* 全速 */
    amhw_zmf237_usbd_connect_set(p_usb, ZMF237_USB_CONNECT);    /* 连接 */
...
static am_usb_status_t __usb_device_deinit (am_usbd_handle_t handle)
{
    ...
    amhw_zmf237_usbd_int_disable(p_usb, AMHW_ZMF237_USB_INT_EN_ALL); /* 禁能USB中断 */
    amhw_zmf237_usbd_ep_int_disable(p_usb, AMHW_ZMF237_EP_INT_EN_ALL);   /* 禁能端点中断 */
    amhw_zmf237_usbd_ep0_int_disable(p_usb, AMHW_ZMF237_EP0_INT_EN_ALL); /* 禁能端点0的中断 */
    amhw_zmf237_usbd_ep_disable(p_usb, AMHW_ZMF237_USB_EP_ALL);      /* 禁能所有端点 */

    /* 禁能端点的中断 */
    amhw_zmf237_usbd_epx_int_disable(p_usb, ZMF237_USB_EP1,
                                  AMHW_ZMF237_EPX_INT_EN_ALL);
    amhw_zmf237_usbd_epx_int_disable(p_usb, ZMF237_USB_EP2,
                                  AMHW_ZMF237_EPX_INT_EN_ALL);
    amhw_zmf237_usbd_epx_int_disable(p_usb, ZMF237_USB_EP3,
                                  AMHW_ZMF237_EPX_INT_EN_ALL);
    amhw_zmf237_usbd_epx_int_disable(p_usb, ZMF237_USB_EP4,
                                  AMHW_ZMF237_EPX_INT_EN_ALL);
                                          ...
                                          uint32_t         length)
{
    ...
    if (length == 0) {  /* 发送空包 */
        while (!amhw_zmf237_usbd_transfer_end(p_usb, (zmf237_usb_epx2_t)endpoint));
        amhw_zmf237_usbd_epx_transfer(p_usb, (zmf237_usb_epx2_t)endpoint, 0);
    } else {
        while (length) {        /* 如果一次不能完全传输则分多次传输 */
            ...
            if (buffer) {
                for (i = 0; i < send_once_size; i++) {
                    amhw_zmf237_usbd_epx_fifo_write(p_usb, (zmf237_usb_epx2_t)endpoint, *buffer++);
                }
            } else {
                for (i = 0; i < send_once_size; i++) {
                    amhw_zmf237_usbd_epx_fifo_write(p_usb, (zmf237_usb_epx2_t)endpoint, 0);
                }
            }
            amhw_zmf237_usbd_epx_transfer(p_usb, (zmf237_usb_epx2_t)endpoint, send_once_size);
        }
    }
...
static void __ep0_send_empty_packet (am_usbd_handle_t handle)
{
    ...
    case AM_USBD_CTRL_IN:
        while (!amhw_zmf237_usbd_transfer_end(p_usb, ZMF237_USB_EPX0));
        amhw_zmf237_usbd_epx_transfer(p_usb, ZMF237_USB_EPX0, 0);
                                          ...
                                          uint32_t                  length)
{
    ...
    avali_data_cnt = amhw_zmf237_usbd_epx_avail_size_get(p_usb, (zmf237_usb_epx2_t)endpoint);
    ...
    while (avali_data_cnt--) {
        *buffer++ = amhw_zmf237_usbd_epx_fifo_read(p_usb, (zmf237_usb_epx2_t)endpoint);
    }
...
static void __usb_device_setdefault_state(am_zlg227_usbd_dev_t *p_dev)
{
    ...
    amhw_zmf237_usbd_ep_enable(p_usb, AMHW_ZMF237_USB_EP_ALL);  /* 使能端点 */

    /* 清中断状态 */
    amhw_zmf237_usbd_int_state_clear(p_usb, AMHW_ZMF237_USB_INT_STATE_ALL);
    amhw_zmf237_usbd_ep0_int_state_clear(p_usb, AMHW_ZMF237_EP0_INT_STATE_ALL);
    amhw_zmf237_usbd_epx_int_state_clear(p_usb, ZMF237_USB_EP1,
                                         AMHW_ZMF237_EPX_INT_STATE_ALL);
    amhw_zmf237_usbd_epx_int_state_clear(p_usb, ZMF237_USB_EP2,
                                         AMHW_ZMF237_EPX_INT_STATE_ALL);
    amhw_zmf237_usbd_epx_int_state_clear(p_usb, ZMF237_USB_EP3,
                                         AMHW_ZMF237_EPX_INT_STATE_ALL);
    amhw_zmf237_usbd_epx_int_state_clear(p_usb, ZMF237_USB_EP4,
                                         AMHW_ZMF237_EPX_INT_STATE_ALL);

    /* 使能USB中断 */
    amhw_zmf237_usbd_int_enable(p_usb, AMHW_ZMF237_USB_INT_EN_ALL);

    /* 使能所有端点中断 */
    amhw_zmf237_usbd_ep_int_enable(p_usb, AMHW_ZMF237_EP_INT_EN_ALL);

    /* 使能端点0的所有中断 */
    amhw_zmf237_usbd_ep0_int_enable(p_usb, AMHW_ZMF237_EP0_INT_EN_ALL);

    /* 使能其他端点的所有中断 */
    amhw_zmf237_usbd_epx_int_enable(p_usb, ZMF237_USB_EP1,
                                 AMHW_ZMF237_EPX_INT_EN_ALL);
    amhw_zmf237_usbd_epx_int_enable(p_usb, ZMF237_USB_EP2,
                                 AMHW_ZMF237_EPX_INT_EN_ALL);
    amhw_zmf237_usbd_epx_int_enable(p_usb, ZMF237_USB_EP3,
                                 AMHW_ZMF237_EPX_INT_EN_ALL);
    amhw_zmf237_usbd_epx_int_enable(p_usb, ZMF237_USB_EP4,
                                 AMHW_ZMF237_EPX_INT_EN_ALL);
}
                                            ...
                                            void                     *param)
{
    ...
    switch ((int)type) {
        ...
        case AM_USBD_CONTROL_STOP:
            amhw_zmf237_usbd_connect_set(p_usb, ZMF237_USB_DISCONNECT); /* 断开连接 */
            error = AM_USB_STATUS_SUCCESS;
            break;
...
static void __usb_setup_handle (am_zlg227_usbd_dev_t *p_dev)
{
    ...
    if(p_usb_dev->state == AM_USBD_CTRL_IDLE) {  // 如果状态为空闲态
        amhw_zmf237_usbd_setupdata_get(p_usb, (uint8_t *)&(p_usb_dev->setup_data)); // 请求数据包
...
static void __usb_device_interrupt_endpoint (am_zlg227_usbd_dev_t *p_dev)
{
    ...
    ep_index = amhw_zmf237_usbd_ep_int_state_get(p_usb);     /* 获得发生中断的端点 */
    amhw_zmf237_usbd_ep_int_state_clear(p_usb, ep_index);    /* 清端点中断 */
    ...
    if (p_dev->int_ep_union.int_ep_flag_field.ep0) {
        ...
        /* 获得端点0发生的具体中断类型 */
        ep_int_type = amhw_zmf237_usbd_ep0_int_state_get(p_usb);
        amhw_zmf237_usbd_ep0_int_state_clear(p_usb, ep_int_type);
    ...
    for (i = 1; i < AM_USBD_MAX_EP_CNT; i++) {
        if (p_dev->device.endpoint_info[i].inuse == AM_TRUE) {
            if ((p_dev->int_ep_union.int_ep_flag >> i) & 1) {
                ...
                ep_int_type = amhw_zmf237_usbd_epx_int_state_get(p_usb, (zmf237_usb_epx_t)(i - 1));
                amhw_zmf237_usbd_epx_int_state_clear(p_usb, (zmf237_usb_epx_t)(i - 1), ep_int_type);
...
static void __usbd_isr_function(void *p_device)
{
    ...
    int_status = amhw_zmf237_usbd_int_state_get(p_usb);     /* 获得中断状态 */
    amhw_zmf237_usbd_int_state_clear(p_usb, int_status);    /* 清除中断状态 */

    /* 端点中断 */
    if (int_status & AMHW_ZMF237_USB_INT_STATE_EPINTF) {
        __usb_device_interrupt_endpoint(p_dev);
    }

    /* 总线复位0中断 */
    if (int_status & AMHW_ZMF237_USB_INT_STATE_RSTF) {
        am_usbd_init(&(p_dev->device));
    }

    /* 总线挂起 */
    if (int_status & AMHW_ZMF237_USB_INT_STATE_SUSPENDF) {
    }

    /* 总线唤醒 */
    if (int_status & AMHW_ZMF237_USB_INT_STATE_RESUMF) {
        amhw_zmf237_usbd_wakeup(p_usb);
    }

    /* 检测到SOF */
    if (int_status & AMHW_ZMF237_USB_INT_STATE_SOFF) {

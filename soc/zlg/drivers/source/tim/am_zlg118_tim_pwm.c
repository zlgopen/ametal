...
static int __tim_pwm_config (void          *p_drv,
                             ...
                             unsigned long  period_ns)
{
    am_zlg118_tim_pwm_dev_t *p_dev    = (am_zlg118_tim_pwm_dev_t *)p_drv;
    ...
    if(p_dev->p_devinfo->comp == 1) {
        ...
        /* 设置某一通道A的比较值 */
        amhw_zlg118_tim_mode23_ccrxy_set(p_hw_tim, (chan / 2) * 2, duty_c - 1);

        /* 设置某一通道B的比较值 */
        amhw_zlg118_tim_mode23_ccrxy_set(p_hw_tim, (chan / 2) * 2 + 1, duty_c - 1);

        /* 设置PWM输出极性设置 */
        amhw_zlg118_tim_mode23_compare_set(
           p_hw_tim,
           (amhw_zlg118_tim_compare_type_t)(((chan / 2) * 2) * 4),
           (amhw_zlg118_tim_compare_t)p_dev->p_devinfo->ocpolarity);

        amhw_zlg118_tim_mode23_compare_set(
            p_hw_tim,
            (amhw_zlg118_tim_compare_type_t)((((chan / 2) * 2) + 1) * 4),
            (amhw_zlg118_tim_compare_t)p_dev->p_devinfo->ocpolarity);
...
static int __tim_pwm_enable (void *p_drv, int chan)
{
    ...
    am_zlg118_tim_pwm_dev_t    *p_dev    = (am_zlg118_tim_pwm_dev_t *)p_drv;
    amhw_zlg118_tim_t          *p_hw_tim = (amhw_zlg118_tim_t *)p_dev->p_devinfo->tim_regbase;
    am_zlg118_tim_pwm_chaninfo_t *p_chaninfo = p_dev->p_devinfo->p_chaninfo;
    ...
    for(i = 0; i <= p_dev->chan_max; i++){
        if((p_chaninfo[i].channel & 0x7f) ==  chan){
            ...
            /* 独立PWM输出 */
                ...
                /* 禁能互补输出 */
                amhw_zlg118_tim_mode23_disable(p_hw_tim, AMHW_ZLG118_TIM_COMP);
...
static int __tim_pwm_disable (void *p_drv, int chan)
{
    ...
    /* 禁能通道PWM输出 */
    amhw_zlg118_tim_mode23_dtr_disable(p_hw_tim, AMHW_ZLG118_TIM_DTR_MOE);
...
/**
  * \brief pwm初始化
  */
void __tim_pwm_init (amhw_zlg118_tim_t       *p_hw_tim,
                     am_zlg118_tim_pwm_dev_t *p_dev,
                     amhw_zlg118_tim_type_t   type)
{
    ...
    for(i = 0; i < p_dev->chan_max;i = i + 2) {
        amhw_zlg118_tim_mode23_csa_compare(p_hw_tim, i);
    }
...
void am_zlg118_tim_pwm_deinit (am_pwm_handle_t handle)
{
    ...
    am_zlg118_tim_pwm_dev_t *p_dev    = (am_zlg118_tim_pwm_dev_t *)handle;
    amhw_zlg118_tim_t       *p_hw_tim = NULL;

...
{
    am_zsl42x_tim_timing_dev_t *p_dev    = (am_zsl42x_tim_timing_dev_t *)p_arg;
    amhw_zsl42x_tim_t          *p_hw_tim = (amhw_zsl42x_tim_t *)
                                           p_dev->p_devinfo->tim_regbase;

    /*
     * 判断是否是定时器溢出中断
     */
    if (amhw_zsl42x_tim_mode0_int_update_flag_check(p_hw_tim) == AM_TRUE ) {
        ...
        /* 清除溢出标志 */
        amhw_zsl42x_tim_mode0_int_update_flag_clr(p_hw_tim);
    }
}
...
{
    am_zsl42x_tim_timing_dev_t *p_dev = (am_zsl42x_tim_timing_dev_t *)p_drv;
                                      ...
                                      uint32_t prescale)
{
    ...
    if ((prescale != 0) && (prescale <= 256)) {
        ...
        if(prescale <= 1) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV1);
            p_dev->prescale_in = 1;
        } else if(prescale <= 2) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV2);
            p_dev->prescale_in = 2;
        } else if(prescale <= 4) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV4);
            p_dev->prescale_in = 4;
        } else if(prescale <= 8) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV8);
            p_dev->prescale_in = 8;
        } else if(prescale <= 16) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV16);
            p_dev->prescale_in = 16;
        } else if(prescale <= 32) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV32);
            p_dev->prescale_in = 32;
        } else if(prescale <= 64) {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV64);
            p_dev->prescale_in = 64;
        } else {
            amhw_zsl42x_tim_mode_clkdiv_set(p_hw_tim, AMHW_ZSL42x_TIM_CLK_DIV256);
                                      ...
                                      uint32_t *p_prescale)
{
    ...
    /* 获取分频值 */
    div = (uint16_t)amhw_zsl42x_tim_mode_clkdiv_get(p_hw_tim);
...
static int __tim_timing_count_get (void *p_drv, uint8_t chan, void *p_count)
{
    ...
    p_hw_tim = (amhw_zsl42x_tim_t *)p_dev->p_devinfo->tim_regbase;
    if(p_dev->p_devinfo->counter_type == AMHW_ZSL42x_TIM_MODE0_COUNTER_32) {
        *(uint32_t *)p_count = amhw_zsl42x_tim_cnt32_count_get(p_hw_tim);
    } else {
        *(uint32_t *)p_count = amhw_zsl42x_tim_cnt16_count_get(p_hw_tim);
    }
...
static int __tim_timing_rollover_get (void *p_drv,uint8_t chan,void *p_rollover)
{
    ...
    if(p_dev->p_devinfo->counter_type == AMHW_ZSL42x_TIM_MODE0_COUNTER_32) {
        ...
        *(uint32_t *)p_rollover = 0xffff - amhw_zsl42x_tim_arr_count_get(p_hw_tim);
    }
...
static int __tim_timing_disable (void *p_drv, uint8_t chan)
{
    ...
    /* 禁能更新中断 */
    amhw_zsl42x_tim_mode0_int_update_disable(p_hw_tim);
...
static int __tim_timing_enable (void *p_drv, uint8_t chan, void *p_count)
{
    ...
    if(p_dev->p_devinfo->counter_type == AMHW_ZSL42x_TIM_MODE0_COUNTER_16) {

        amhw_zsl42x_tim_arr_count_set(p_hw_tim, 65535 - count);
    }

    /* 设置计数器初值 CNT */
    if(p_dev->p_devinfo->counter_type == AMHW_ZSL42x_TIM_MODE0_COUNTER_16) {
        amhw_zsl42x_tim_cnt16_count_set(p_hw_tim, 65535 - count);
    } else {
        amhw_zsl42x_tim_cnt32_count_set(p_hw_tim, 0xffffffff - count);
    }

    /* 是否开启门控 */
    if(p_dev->p_devinfo->gate_enable == AMHW_ZSL42x_TIM_GATE_ENABLE) {
        amhw_zsl42x_tim_mode0_gate_enable(p_hw_tim);
    } else {
        amhw_zsl42x_tim_mode0_gate_disable(p_hw_tim);
    }

    /* 门控有效信号设置 */
    if(p_dev->p_devinfo->gate_state == AM_TRUE) {
        amhw_zsl42x_tim_mode0_gate_high(p_hw_tim);
    } else {
        amhw_zsl42x_tim_mode0_gate_low(p_hw_tim);
    }

    /* 计数时钟设置 */
    if(p_dev->p_devinfo->clk_src == AMHW_ZSL42x_TIM_CLK_SRC_TCLK) {
        amhw_zsl42x_tim_mode_clk_src_set(p_hw_tim,AMHW_ZSL42x_TIM_CLK_SRC_TCLK);
    } else {
        amhw_zsl42x_tim_mode_clk_src_set(p_hw_tim,AMHW_ZSL42x_TIM_CLK_SRC_ETR);
    }
    ...
    if (p_dev->pfn_callback != NULL) {
        ...
        /* 允许更新中断 */
        amhw_zsl42x_tim_mode0_int_update_enable(p_hw_tim);
    }
...
am_timer_handle_t am_zsl42x_tim_timing_init (
    am_zsl42x_tim_timing_dev_t             *p_dev,
    const am_zsl42x_tim_timing_devinfo_t  *p_devinfo)
{
    ...
    if(p_devinfo->counter_type == AMHW_ZSL42x_TIM_MODE0_COUNTER_32) {

        amhw_zsl42x_tim_mode0_counter_set(p_hw_tim,
                                          AMHW_ZSL42x_TIM_MODE0_COUNTER_32);
        ...
        amhw_zsl42x_tim_mode0_counter_set(p_hw_tim,
                                          AMHW_ZSL42x_TIM_MODE0_COUNTER_16);
    ...
    /* 设置定时器模式0 */
    amhw_zsl42x_tim_mode_set(p_hw_tim, AMHW_ZSL42x_TIM_MODE_TIMING);
...
void am_zsl42x_tim_timing_deinit (am_timer_handle_t handle)
{
    ...
    am_zsl42x_tim_timing_dev_t *p_dev    = (am_zsl42x_tim_timing_dev_t *)handle;
    amhw_zsl42x_tim_t          *p_hw_tim = NULL;

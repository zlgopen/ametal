...
/** \brief 中断未连接标识 */
#define AM_ZSN700_GPIO_INVALID_PIN_MAP    0xFF

const am_zsn700_gpio_devinfo_t *p_gpio_devinfo = NULL;
...
/** \bruef 指向GPIO设备的指针 */
am_zsn700_gpio_dev_t *__gp_gpio_dev;

/*******************************************************************************
  公共函数
*******************************************************************************/

static int __am_zsn700_peripheral_afio_clear(int pin)
{
    amhw_zsn700_gpio_t     *p_hw_gpio  = (amhw_zsn700_gpio_t *)p_gpio_devinfo->gpio_regbase;

    if (NULL == p_gpio_devinfo) {
        return -AM_ENXIO;
    }

    if (p_gpio_devinfo->p_afio[pin] == AMHW_ZSN700_AFIO_NO) {
        return -AM_ENXIO;
    }

    amhw_zsn700_gpio_afio_set(p_hw_gpio, AMHW_ZSN700_AFIO_NO, pin);

    if(amhw_zsn700_gpio_afio_get(p_hw_gpio, pin) == AMHW_ZSN700_AFIO_NO)
    {
        p_gpio_devinfo->p_afio[pin] = AMHW_ZSN700_AFIO_NO;
    }
...
{
    amhw_zsn700_gpio_t     *p_hw_gpio  = NULL;
    amhw_zsn700_gpiomode_t  pin_mode   = AMHW_ZSN700_GPIO_MODE_IPU;
    ...
    p_hw_gpio  = (amhw_zsn700_gpio_t *)p_gpio_devinfo->gpio_regbase;

    func = AM_GPIO_COM_FUNC_GET(flags);
    mode = AM_GPIO_COM_MODE_GET(flags);

    __am_zsn700_peripheral_afio_clear(pin);
    ...
    if (0 != func) {
        ...
        case AM_GPIO_OUTPUT_INIT_HIGH_VAL:
            ...
            /* 推挽输出 */
            amhw_zsn700_gpio_pin_mode_set(p_hw_gpio,
                                          AMHW_ZSN700_GPIO_MODE_OUT_PP,
                                          pin);

            /* 输出高电平 */
            amhw_zsn700_gpio_pin_out_high(p_hw_gpio, pin);
        ...
        case AM_GPIO_OUTPUT_INIT_LOW_VAL:
            ...
            /* 输出低电平 */
            amhw_zsn700_gpio_pin_out_low(p_hw_gpio, pin);
    ...
    if (mode != 0x00) {
        ...
        /* 获取输入输出方向 */
        dir = amhw_zsn700_gpio_pin_dir_get(p_hw_gpio, pin);
        ...
        case AM_GPIO_PULL_UP_VAL:
            if (dir == 1) {
                ...
                /* 上拉输入模式  */
                pin_mode = AMHW_ZSN700_GPIO_MODE_IPU;

            } else {

                /* 输出没有上拉模式，设置为推挽模式 */
                pin_mode = AMHW_ZSN700_GPIO_MODE_OUT_PP;
            }
            break;
        ...
        case AM_GPIO_PULL_DOWN_VAL:
            if (dir == 1) {
                ...
                /* 下拉输入模式  */
                pin_mode = AMHW_ZSN700_GPIO_MODE_IPD;
        ...
        case AM_GPIO_FLOAT_VAL:
            if (dir == 1) {
                pin_mode = AMHW_ZSN700_GPIO_MODE_IN_FLOATING;
            } else {

                /* 输出没有浮空模式，设置为开漏模式 */
                pin_mode = AMHW_ZSN700_GPIO_MODE_OUT_OD;
            }
            break;
        ...
        amhw_zsn700_gpio_pin_mode_set(p_hw_gpio, pin_mode, pin);
    }

    /* 平台相关 */
    if (0 != (flags & AM_ZSN700_GPIO_MODE)) {

        /* 输入输出相应模式设置 */

        switch (AM_ZSN700_GPIO_MODE_GET(flags)) {
        ...
        case 2:
            /* 模拟输入 */
            pin_mode = AMHW_ZSN700_GPIO_MODE_AIN;
            break;
        ...
        case 6:
            /* 模拟输出 */
            pin_mode = AMHW_ZSN700_GPIO_MODE_AOUT;
            break;
    ...
    /* 重映像设置 */
    if (0 != (flags & AM_ZSN700_GPIO_REMAP)) {

        __am_zsn700_peripheral_afio_clear(pin);

        amhw_zsn700_gpio_afio_set(
            p_hw_gpio,
            (amhw_zsn700_gpio_afio_t)AM_ZSN700_GPIO_REMAP_MODE_GET(flags),
            pin);
...
int am_gpio_get (int pin)
{
    ...
    if (0 == amhw_zsn700_gpio_pin_dir_get(p_hw_gpio, pin)) {
        return amhw_zsn700_gpio_pin_output_get(p_hw_gpio, pin);
    } else {
        return amhw_zsn700_gpio_pin_input_get(p_hw_gpio, pin);
    }
}
...
int am_gpio_toggle (int pin)
{
    ...
    t= amhw_zsn700_gpio_pin_output_get(p_hw_gpio, pin);
...
static void __port_a_int_isr (void * p_arg)
{
    ...
    for(pin = 0; pin < 16; pin++) {
        if (1 == amhw_zsn700_gpio_pin_int_flag_get(p_hw_gpio, pin)) {
            ...
            /* 清中断标志 */
            amhw_zsn700_gpio_pin_int_flag_clr(p_hw_gpio, pin);
        }
    }
}
                         ...
                         const am_zsn700_gpio_devinfo_t *p_devinfo)
{
    ...
    for (i = 0 ; i < p_devinfo->pin_count ; i++) {
        __am_zsn700_peripheral_afio_clear(i);
    }

    for (i = 0 ; i < p_devinfo->exti_num_max ; i++) {
        p_devinfo->p_infomap[i] = AM_ZSN700_GPIO_INVALID_PIN_MAP;
...
void am_zsn700_gpio_deinit (void)
{
    ...
    for (i = 0 ; i < p_gpio_devinfo->exti_num_max ; i++) {
        p_gpio_devinfo->p_infomap[i] = AM_ZSN700_GPIO_INVALID_PIN_MAP;
                            ...
                            void         *p_arg)
{
    ...
    if (p_gpio_devinfo->p_infomap[slot] == AM_ZSN700_GPIO_INVALID_PIN_MAP) {
                               ...
                               void         *p_arg)
{
    ...
    if (p_gpio_devinfo->p_infomap[slot] == pin) {
        ...
        p_gpio_devinfo->p_infomap[slot] = AM_ZSN700_GPIO_INVALID_PIN_MAP;
...
int am_gpio_trigger_on(int pin)
{
    ...
    if (p_gpio_devinfo->p_infomap[slot] == pin) {
        ...
        switch (__gp_gpio_dev->int_type) {
            ...
            case AM_GPIO_TRIGGER_HIGH:
                amhw_zsn700_gpio_pin_high_int_enable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_low_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_raising_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_falling_int_disable(p_hw_gpio,pin);
                break;

            case AM_GPIO_TRIGGER_LOW:
                amhw_zsn700_gpio_pin_high_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_low_int_enable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_raising_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_falling_int_disable(p_hw_gpio,pin);
                break;
            ...
            case AM_GPIO_TRIGGER_RISE:
                amhw_zsn700_gpio_pin_high_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_low_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_raising_int_enable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_falling_int_disable(p_hw_gpio,pin);
                break;
            ...
            case AM_GPIO_TRIGGER_FALL:
                ...
                amhw_zsn700_gpio_pin_low_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_raising_int_disable(p_hw_gpio,pin);
                amhw_zsn700_gpio_pin_falling_int_enable(p_hw_gpio,pin);
                break;

...
{
    amhw_zlg118_flash_key_set(ZLG118_FLASH, 0x5A5A);
    amhw_zlg118_flash_key_set(ZLG118_FLASH, 0xA5A5);
}
...
{
    if(rch_clk == AMHW_ZLG118_RCH_FRE_24MHz) {

        /* 设置RCH时钟频率校准值 （24MHz）*/
        amhw_zlg118_rcc_rch_trim_set(*((uint16_t*)rch_clk));
        *sysclk = 24000000;
    } else if(rch_clk == AMHW_ZLG118_RCH_FRE_22MHz) {

       /* 设置RCH时钟频率校准值 （22.12MHz）*/
       amhw_zlg118_rcc_rch_trim_set(*((uint16_t*)rch_clk));
       *sysclk = 22120000;
    } else if(rch_clk == AMHW_ZLG118_RCH_FRE_16MHz) {

       /* 设置RCH时钟频率校准值 （16MHz）*/
       amhw_zlg118_rcc_rch_trim_set(*((uint16_t*)rch_clk));
       *sysclk = 16000000;
    } else if(rch_clk == AMHW_ZLG118_RCH_FRE_8MHz) {
...

{
    ...
    if((p_dev->sys_clk) > 24000000) {
        __flash_unlock();
        amhw_zlg118_flash_read_waittime_set(ZLG118_FLASH,
    ...
    __flash_unlock();
    amhw_zlg118_flash_tnvs_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tpgs_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tprog_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tserase_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tmerase_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tprcv_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tsrcv_set(ZLG118_FLASH, hclk_unit);

    __flash_unlock();
    amhw_zlg118_flash_tmrcv_set(ZLG118_FLASH, hclk_unit);

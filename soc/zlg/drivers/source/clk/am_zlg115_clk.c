...
static void __pllin_hsi_init (void)
{
    ...
    if(__gp_dev->p_devinfo->input_clk == 48000000){
        ZLG116_RCC->cr &= ~((uint32_t)0x01 << 20);
...
const am_zlg115_clk_dev_t *am_zlg116_clk_devinfo_get (void)
{
    if (__gp_dev == NULL)  {
        return NULL;
    }
                        ...
                        const am_zlg115_clk_devinfo_t *p_devinfo)
{
    ...
    ZLG116_RCC->cfgr &= (uint32_t)0xF0FF0000;

    /* 复位禁能 HSEON, CSSON and PLLON bits */
    ZLG116_RCC->cr &= (uint32_t)0xFFF6FFFF;

    ZLG116_RCC->cr &= (uint32_t)0xFFFBFFFF;

    ZLG116_RCC->cir = 0x009F0000;
    ...
    ZLG116_FLASH->acr |= 0x10;

    amhw_zlg_flash_latency_set(ZLG116_FLASH, 2);
    ...
    amhw_zlg116_rcc_sys_clk_set ((amhw_zlg116_sys_clk_src)AMHW_ZLG115_SYSCLK_HSI);

    while (temp != 0x02) {
        temp = ZLG116_RCC->cfgr >> 2;
        temp &= 0x03;
    }

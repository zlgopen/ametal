...
am_local void __spi_cs_on  (am_zsl42x_spi_dma_dev_t *p_this, am_spi_device_t *p_dev);
am_local void __spi_cs_off (am_zsl42x_spi_dma_dev_t *p_this, am_spi_device_t *p_dev);

am_local int  __spi_hard_init (am_zsl42x_spi_dma_dev_t *p_this);
am_local int  __spi_config (am_zsl42x_spi_dma_dev_t *p_this);

am_local uint32_t __spi_speed_cfg (am_zsl42x_spi_dma_dev_t *p_dev,
                                   uint32_t                 target_baud);

am_local int  __spi_mst_sm_event (am_zsl42x_spi_dma_dev_t *p_dev, uint32_t event);
 ...
 */
am_local
void __spi_cs_on (am_zsl42x_spi_dma_dev_t *p_this, am_spi_device_t *p_dev)
{
    amhw_zsl42x_spi_t     *p_hw_spi = (amhw_zsl42x_spi_t *)(p_this->p_devinfo->spi_reg_base);
 ...
 */
am_local
void __spi_cs_off (am_zsl42x_spi_dma_dev_t   *p_this,
                   am_spi_device_t           *p_dev)
{
    amhw_zsl42x_spi_t     *p_hw_spi = (amhw_zsl42x_spi_t *)(p_this->p_devinfo->spi_reg_base);
 ...
 */
am_static_inline
void __spi_msg_in (am_zsl42x_spi_dma_dev_t *p_dev, struct am_spi_message *p_msg)
{
 ...
 */
am_static_inline
struct am_spi_message *__spi_msg_out (am_zsl42x_spi_dma_dev_t *p_dev)
{
...
{
    am_zsl42x_spi_dma_dev_t *p_this = (am_zsl42x_spi_dma_dev_t *)p_arg;
 ...
 */
am_local
int __spi_hard_init (am_zsl42x_spi_dma_dev_t *p_this)
{
    amhw_zsl42x_spi_t *p_hw_spi = (amhw_zsl42x_spi_t *)(p_this->p_devinfo->spi_reg_base);
...
void __dma_isr (void *p_arg, uint32_t stat)
{
    ...
    /* 中断发生 */
    if (stat == AM_ZSL42x_DMA_INT_COMPLETE)
    {
        /* 记录成功传送字节数 */
 ...
 */
am_local
int __spi_dma_trans (am_zsl42x_spi_dma_dev_t *p_dev)
{
    am_zsl42x_spi_dma_dev_t           *p_this      = (am_zsl42x_spi_dma_dev_t *)p_dev;
    const am_zsl42x_spi_dma_devinfo_t *p_devinfo   = p_this->p_devinfo;
    amhw_zsl42x_spi_t                 *p_hw_spi    = (amhw_zsl42x_spi_t *)(p_devinfo->spi_reg_base);
    ...
    /* 连接DMA中断服务函数 */
    am_zsl42x_dma_isr_connect(p_this->p_devinfo->dma_chan_tx,
                           __dma_isr, 
                           (void *)p_this);

    /* DMA发送通道配置 */
    dma_flags[0] = AMHW_ZSL42x_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
    ...
    /* DMA接收通道配置 */
    dma_flags[1] = AMHW_ZSL42x_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
                   AMHW_ZSL42x_DMA_CHAN_SIZE_8BIT              | /* 数据位宽 8bit */

                   AMHW_ZSL42x_DMA_CHAN_SRC_ADD_INC_DISABLE    | /* 源地址不自增 */
                   AMHW_ZSL42x_DMA_CHAN_RELOAD_SRC_ADD_ENABLE  | /* 源地址重载*/

                   AMHW_ZSL42x_DMA_CHAN_RELOAD_DST_ADD_DISABLE | /* 目标地址不重载 */
    ...
    if (p_cur_trans->p_rxbuf == NULL) {
        ...
        dma_flags[1] |= AMHW_ZSL42x_DMA_CHAN_DST_ADD_INC_DISABLE;

        /* 建立接收通道描述符 */
        am_zsl42x_dma_xfer_desc_build(&(p_this->g_desc[1]),       /* 通道描述符 */
    ...
    /* 存在接收数据 */
        ...
        /* 目标地址自增(接收数据buff地址自增) */
        dma_flags[1] |= AMHW_ZSL42x_DMA_CHAN_DST_ADD_INC_ENABLE;
    ...
    if (p_cur_trans->p_txbuf == NULL) {
        ...
        dma_flags[0] |= AMHW_ZSL42x_DMA_CHAN_SRC_ADD_INC_DISABLE;

        /* 建立发送通道描述符 */
        am_zsl42x_dma_xfer_desc_build(&(p_this->g_desc[0]),       /* 通道描述符 */
        ...
        /* 源地址自增(发送数据buff地址自增) */
        dma_flags[0] |= AMHW_ZSL42x_DMA_CHAN_SRC_ADD_INC_ENABLE;
    ...
    if (am_zsl42x_dma_xfer_desc_chan_cfg(&(p_this->g_desc[0]),
                                        AMHW_ZSL42x_DMA_MER_TO_PER,  /* 内存到外设 */
                                        p_this->p_devinfo->dma_chan_tx) == AM_ERROR) {
        return AM_ERROR;
    }

    if (am_zsl42x_dma_xfer_desc_chan_cfg(&(p_this->g_desc[1]),
                                        AMHW_ZSL42x_DMA_PER_TO_MER,  /* 外设到内存 */
    ...
    am_zsl42x_dma_block_data_size(p_this->p_devinfo->dma_chan_rx, 1);
    am_zsl42x_dma_block_data_size(p_this->p_devinfo->dma_chan_tx, 1);

    /* 设置DMA触发源*/
    if((p_devinfo->spi_reg_base) == ZSL42x_SPI0_BASE) {
        am_zsl42x_dma_chan_src_set(p_this->p_devinfo->dma_chan_rx,
                                   ZSL42x_DMA_SRC_TYPE_SPI0_RX);
        am_zsl42x_dma_chan_src_set(p_this->p_devinfo->dma_chan_tx,
                                   ZSL42x_DMA_SRC_TYPE_SPI0_TX);

    } else if((p_devinfo->spi_reg_base) == ZSL42x_SPI1_BASE) {

        am_zsl42x_dma_chan_src_set(p_this->p_devinfo->dma_chan_rx,
                                   ZSL42x_DMA_SRC_TYPE_SPI1_RX);

        am_zsl42x_dma_chan_src_set(p_this->p_devinfo->dma_chan_tx,
                                   ZSL42x_DMA_SRC_TYPE_SPI1_TX);
    }

    am_zsl42x_dma_chan_start(p_this->p_devinfo->dma_chan_rx);
    am_zsl42x_dma_chan_start(p_this->p_devinfo->dma_chan_tx);

    return AM_OK;
}

am_local
int __spi_config (am_zsl42x_spi_dma_dev_t *p_this)
{
    const am_zsl42x_spi_dma_devinfo_t *p_devinfo = p_this->p_devinfo;
    amhw_zsl42x_spi_t                 *p_hw_spi  = (amhw_zsl42x_spi_t *)(p_devinfo->spi_reg_base);
    ...
    /* 配置SPI模式 */
    amhw_zsl42x_spi_clk_mode_set(p_hw_spi, p_this->p_cur_spi_dev->mode);
...
int __spi_msg_start (void              *p_drv,
                     ...
                     am_spi_message_t  *p_msg)
{
    am_zsl42x_spi_dma_dev_t *p_this = (am_zsl42x_spi_dma_dev_t *)p_drv;
 ...
 */
am_local
int __spi_mst_sm_event (am_zsl42x_spi_dma_dev_t *p_dev, uint32_t event)
{
    volatile uint32_t new_event = __SPI_EVT_NONE;
    amhw_zsl42x_spi_t *p_hw_spi = (amhw_zsl42x_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    while (1) {
        ...
        case __SPI_ST_DMA_TRANS_DATA:    /* DMA发送数据 */
        {
            ...
            amhw_zsl42x_spi_dma_tx_enable(p_hw_spi);
            amhw_zsl42x_spi_dma_rx_enable(p_hw_spi);
 ...
 * \brief SPI 初始化
 */
am_spi_handle_t am_zsl42x_spi_dma_init (am_zsl42x_spi_dma_dev_t           *p_dev,
                                        const am_zsl42x_spi_dma_devinfo_t *p_devinfo)
{
 ...
 * \brief SPI 去除初始化
 */
void am_zsl42x_spi_dma_deinit (am_spi_handle_t handle)
{
    am_zsl42x_spi_dma_dev_t *p_dev    = (am_zsl42x_spi_dma_dev_t *)handle;
    amhw_zsl42x_spi_t       *p_hw_spi = (amhw_zsl42x_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    /* 禁能 SPI */
    amhw_zsl42x_spi_dma_tx_disable(p_hw_spi);
    amhw_zsl42x_spi_dma_rx_disable(p_hw_spi);
    amhw_zsl42x_spi_enable(p_hw_spi, AM_FALSE);
 ...
 */
am_local
uint32_t __spi_speed_cfg (am_zsl42x_spi_dma_dev_t *p_dev,
                          uint32_t                 target_speed)
{

    uint32_t clk, best_speed;  /* 计算出的速度 */
    uint8_t  i = 0;
    amhw_zsl42x_spi_baud_rate_t baud_div = AMHW_ZSL42x_SPI_BAUD_RATE_PCLK_0;

    amhw_zsl42x_spi_t *p_hw_spi = (amhw_zsl42x_spi_t *)
                                  (p_dev->p_devinfo->spi_reg_base);

    if(target_speed == 0) {
        baud_div = AMHW_ZSL42x_SPI_BAUD_RATE_PCLK_DIV2;
    } else {
    ...
    baud_div = (amhw_zsl42x_spi_baud_rate_t)(i - 1);

    amhw_zsl42x_spi_baud_rate_set(p_hw_spi, baud_div);

...
int __spi_dma_trans (am_zsn700_spi_dma_dev_t *p_dev)
{
    ...
    /* DMA发送通道配置 */
    dma_flags[0] = AMHW_ZSN700_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
    ...
    /* DMA接收通道配置 */
    dma_flags[1] = AMHW_ZSN700_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
                   AMHW_ZSN700_DMA_CHAN_SIZE_8BIT              | /* 数据位宽 8bit */

                   AMHW_ZSN700_DMA_CHAN_SRC_ADD_INC_DISABLE    | /* 源地址不自增 */
                   AMHW_ZSN700_DMA_CHAN_RELOAD_SRC_ADD_ENABLE  | /* 源地址重载*/

                   AMHW_ZSN700_DMA_CHAN_RELOAD_DST_ADD_DISABLE | /* 目标地址不重载 */
    ...
    if (p_cur_trans->p_rxbuf == NULL) {
        ...
        /* 建立接收通道描述符 */
        am_zsn700_dma_xfer_desc_build(&(p_this->g_desc[1]),       /* 通道描述符 */
    ...
    if (p_cur_trans->p_txbuf == NULL) {
        ...
        /* 建立发送通道描述符 */
        am_zsn700_dma_xfer_desc_build(&(p_this->g_desc[0]),       /* 通道描述符 */
    ...
    if (am_zsn700_dma_xfer_desc_chan_cfg(&(p_this->g_desc[0]),
                                        AMHW_ZSN700_DMA_MER_TO_PER,  /* 内存到外设 */

...
am_local void __spi_cs_on  (am_zsl42x_spi_int_dev_t *p_this, am_spi_device_t *p_dev);
am_local void __spi_cs_off (am_zsl42x_spi_int_dev_t *p_this, am_spi_device_t *p_dev);

am_local void __spi_write_data (am_zsl42x_spi_int_dev_t *p_dev);
am_local void __spi_read_data (am_zsl42x_spi_int_dev_t *p_dev);

am_local void __spi_irq_handler (void *p_arg);
am_local int  __spi_hard_init (am_zsl42x_spi_int_dev_t *p_this);
am_local int  __spi_config (am_zsl42x_spi_int_dev_t *p_this);

am_local uint32_t __spi_speed_cfg (am_zsl42x_spi_int_dev_t *p_dev,
                                   uint32_t                 target_baud);

am_local int  __spi_mst_sm_event (am_zsl42x_spi_int_dev_t *p_dev, uint32_t event);
/*******************************************************************************
  SPI驱动函数声明
*******************************************************************************/
 ...
 */
am_local
void __spi_cs_on (am_zsl42x_spi_int_dev_t *p_this, am_spi_device_t *p_dev)
{
 ...
 */
am_local
void __spi_cs_off (am_zsl42x_spi_int_dev_t *p_this,
                   am_spi_device_t      *p_dev)
{
    amhw_zsl42x_spi_t     *p_hw_spi = (amhw_zsl42x_spi_t *)(p_this->p_devinfo->spi_reg_base);
...
am_local
void __spi_write_data (am_zsl42x_spi_int_dev_t *p_dev)
{
    ...
    while(amhw_zsl42x_spi_flag_check (
            p_hw_spi, AMHW_ZSL42x_SPI_FLAG_TX_EMPTY) == AM_FALSE);
    /* tx_buf 有效 */
    if (p_trans->p_txbuf != NULL) {

        /** \brief 待发送数据的基址+偏移 */
        uint8_t *ptr = (uint8_t *)(p_trans->p_txbuf) + p_dev->data_ptr;
        amhw_zsl42x_spi_tx_data8_put(p_hw_spi, *ptr, p_dev->lsbfirst);

    /* tx_buf 无效 */
    } else {
        amhw_zsl42x_spi_tx_data8_put(p_hw_spi, 0x00, p_dev->lsbfirst);
    }
...
am_local
void __spi_read_data (am_zsl42x_spi_int_dev_t *p_dev)
{
    amhw_zsl42x_spi_t *p_hw_spi = (amhw_zsl42x_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    if (amhw_zsl42x_spi_flag_check (
            p_hw_spi, AMHW_ZSL42x_SPI_FLAG_RX_NOT_EMPTY) == AM_TRUE) {

        /* rx_buf 有效 */
        if (p_trans->p_rxbuf != NULL && p_dev->nbytes_to_recv) {
            *p_buf8 = amhw_zsl42x_spi_rx_data8_get(p_hw_spi, p_dev->lsbfirst);
        /* rx_buf 无效或者不需要接收数据 */
        } else {
            (void)amhw_zsl42x_spi_rx_data8_get(p_hw_spi, p_dev->lsbfirst);
        }
 ...
 */
am_static_inline
void __spi_msg_in (am_zsl42x_spi_int_dev_t *p_dev,
 ...
 */
am_static_inline
struct am_spi_message *__spi_msg_out (am_zsl42x_spi_int_dev_t *p_dev)
{
...
{
    am_zsl42x_spi_int_dev_t *p_this = (am_zsl42x_spi_int_dev_t *)p_arg;
 ...
 */
am_local
int __spi_hard_init (am_zsl42x_spi_int_dev_t *p_this)
{
    amhw_zsl42x_spi_t *p_hw_spi = (amhw_zsl42x_spi_t *)(p_this->p_devinfo->spi_reg_base);
...
{
    am_zsl42x_spi_int_dev_t  *p_dev    = (am_zsl42x_spi_int_dev_t *)p_arg;
    amhw_zsl42x_spi_t        *p_hw_spi = (amhw_zsl42x_spi_t *)(p_dev->p_devinfo->spi_reg_base);


    /* 发送数据 */
    if(amhw_zsl42x_spi_flag_check(
            p_hw_spi, AMHW_ZSL42x_SPI_FLAG_TX_EMPTY) == AM_TRUE) {

        amhw_zsl42x_spi_flag_clr(p_hw_spi);
...
am_local
int __spi_config (am_zsl42x_spi_int_dev_t *p_this)
{
    const am_zsl42x_spi_int_devinfo_t *p_devinfo = p_this->p_devinfo;
...
int __spi_msg_start (void              *p_drv,
                     ...
                     am_spi_message_t  *p_msg)
{
    am_zsl42x_spi_int_dev_t *p_this = (am_zsl42x_spi_int_dev_t *)p_drv;
 ...
 */
am_local
int __spi_mst_sm_event (am_zsl42x_spi_int_dev_t *p_dev, uint32_t event)
{
    amhw_zsl42x_spi_t *p_hw_spi = (amhw_zsl42x_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    while (1) {
        ...
        case __SPI_ST_MSG_START:    /* 消息开始 */
        {
            ...
            if (p_cur_msg) {
                ...
                /* 禁能中断 */
                amhw_zsl42x_spi_int_disable(p_hw_spi);
                p_dev->busy = AM_FALSE;
            }
            am_int_cpu_unlock(key);
        ...
        case __SPI_ST_M_SEND_DATA:    /* 发送数据 */
        {
            ...
            if (p_dev->data_ptr >= p_cur_trans->nbytes) {
                ...
                if (amhw_zsl42x_spi_flag_check (
                    ...
                    /* 打开接收中断 */
                    amhw_zsl42x_spi_int_enable(p_hw_spi);
                    amhw_zsl42x_spi_int_rx_not_empty_enable(p_hw_spi);
                } else {

                    /* 打开中断等待发送就绪 */
                    amhw_zsl42x_spi_int_enable(p_hw_spi);
                    amhw_zsl42x_spi_tx_empty_enable(p_hw_spi);
                }
            }
            break;
        }
        ...
        case __SPI_ST_M_RECV_DATA:   /* 接收数据 */
        {
            ...
            if (p_dev->nbytes_to_recv == 0) {
                ...
                /* 禁能发送及接收中断 */
                amhw_zsl42x_spi_int_disable(p_hw_spi);
                amhw_zsl42x_spi_tx_empty_disable(p_hw_spi);
                amhw_zsl42x_spi_int_rx_not_empty_disable(p_hw_spi);
 ...
 * \brief SPI初始化
 */
am_spi_handle_t am_zsl42x_spi_int_init (am_zsl42x_spi_int_dev_t           *p_dev,
                                        const am_zsl42x_spi_int_devinfo_t *p_devinfo)
{
 ...
 * \brief SPI 去除初始化
 */
void am_zsl42x_spi_int_deinit (am_spi_handle_t handle)
{
    am_zsl42x_spi_int_dev_t *p_dev    = (am_zsl42x_spi_int_dev_t *)handle;
    amhw_zsl42x_spi_t       *p_hw_spi = (amhw_zsl42x_spi_t *)(p_dev->p_devinfo->spi_reg_base);
 ...
 */
am_local
uint32_t __spi_speed_cfg (am_zsl42x_spi_int_dev_t *p_dev,

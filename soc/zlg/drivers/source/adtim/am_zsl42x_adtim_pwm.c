...
static int __adtim_pwm_config (void          *p_drv,
                               ...
                               unsigned long  period_ns)
{
    am_zsl42x_adtim_pwm_dev_t *p_dev      = (am_zsl42x_adtim_pwm_dev_t *)p_drv;
    ...
    if(fre_div <= 1) {
        ...
        basecnt.cntclkdiv  = AMHW_ZSL42x_ADTIM_PCLK_DIV2;
        fre_div = 2;
    } else if(fre_div <= 4) {
        basecnt.cntclkdiv  = AMHW_ZSL42x_ADTIM_PCLK_DIV4;
        fre_div = 4;
    } else if(fre_div <= 8) {
        basecnt.cntclkdiv = AMHW_ZSL42x_ADTIM_PCLK_DIV8;
        fre_div = 8;
    } else if(fre_div <= 16) {
        basecnt.cntclkdiv  = AMHW_ZSL42x_ADTIM_PCLK_DIV16;
        fre_div = 16;
    } else if(fre_div <= 64) {
        basecnt.cntclkdiv  = AMHW_ZSL42x_ADTIM_PCLK_DIV64;
        fre_div = 64;
    } else if(fre_div <= 256) {
        basecnt.cntclkdiv  = AMHW_ZSL42x_ADTIM_PCLK_DIV256;
        fre_div = 256;
    } else {
        basecnt.cntclkdiv  = AMHW_ZSL42x_ADTIM_PCLK_DIV1024;
        fre_div = 1024;
    }
    ...
    if(p_dev->p_devinfo->comp == 1) {
        ...
        amhw_zsl42x_adtim_setcomparevalue(p_hw_adtim, 
                                          AMHW_ZSL42x_ADTIM_COMPARE_A, 
                                          duty_c);
        amhw_zsl42x_adtim_setcomparevalue(p_hw_adtim, 
                                          AMHW_ZSL42x_ADTIM_COMPARE_B, 
                                          duty_c);

        portcfg.staout = (amhw_zsl42x_adtim_port_out_t)p_dev->p_devinfo->ocpolarity;
        portcfg.stpout = (amhw_zsl42x_adtim_port_out_t)p_dev->p_devinfo->ocpolarity;
...
static int __adtim_pwm_enable (void *p_drv, int chan)
{
    ...
    am_zsl42x_adtim_pwm_dev_t      *p_dev      = (am_zsl42x_adtim_pwm_dev_t *)
                                                 p_drv;
    amhw_zsl42x_adtim_t            *p_hw_adtim = (amhw_zsl42x_adtim_t *)
                                                 p_dev->p_devinfo->adtim_regbase;
    am_zsl42x_adtim_pwm_chaninfo_t *p_chaninfo = p_dev->p_devinfo->p_chaninfo;
...
static int __adtim_pwm_disable (void *p_drv, int chan)
{
    ...
    am_zsl42x_adtim_pwm_dev_t      *p_dev    = (am_zsl42x_adtim_pwm_dev_t *)p_drv;
    amhw_zsl42x_adtim_t            *p_hw_adtim = (amhw_zsl42x_adtim_t *)p_dev->p_devinfo->adtim_regbase;
    am_zsl42x_adtim_pwm_chaninfo_t *p_chaninfo = p_dev->p_devinfo->p_chaninfo;
...
/**
  ...
  */
am_pwm_handle_t am_zsl42x_adtim_pwm_init (am_zsl42x_adtim_pwm_dev_t      *p_dev,
                                     const am_zsl42x_adtim_pwm_devinfo_t *p_devinfo)
{
    amhw_zsl42x_adtim_basecnt_cfg_t basecnt;
    ...
    p_hw_adtim              = (amhw_zsl42x_adtim_t *)p_dev->p_devinfo->adtim_regbase;
...
void am_zsl42x_adtim_pwm_deinit (am_pwm_handle_t handle)
{

    am_zsl42x_adtim_pwm_dev_t *p_dev    = (am_zsl42x_adtim_pwm_dev_t *)handle;
    amhw_zsl42x_adtim_t       *p_hw_adtim = NULL;

 ...
 * \brief 串口模式（查询或中断）设置
 */
int __uart_mode_set (am_zlg118_uart_dev_t *p_dev, uint32_t new_mode);

/**
 * \brief 串口硬件设置
 */
int __uart_opt_set (am_zlg118_uart_dev_t *p_dev, uint32_t opts);

/**
 * \brief 流控接受器流控状态设置
 */
static int __uart_flow_rxstat_set (am_zlg118_uart_dev_t *p_dev, uint32_t ctrl);

/**
 * \brief 流控模式设置
 */
static int __uart_flow_mode_set (am_zlg118_uart_dev_t *p_dev, uint32_t mode);

/**
 * \brief 流控发送器流控状态获取
 */
static int __uart_flow_txstat_get (am_zlg118_uart_dev_t *p_dev);
...
{
    am_zlg118_uart_dev_t *p_dev     = (am_zlg118_uart_dev_t *)p_drv;
    ...
    case AM_UART_BAUD_SET:
        ...
        /* 只有在发送空的基础上才允许修改波特率 */
        while (amhw_zlg118_uart_flag_check(
                   p_hw_uart, AMHW_ZLG118_UART_FLAG_TX_EMPTY) == AM_FALSE);

        status = amhw_zlg118_uart_baudrate_set(
...
int __uart_tx_startup (void *p_drv)
{
    ...
    am_zlg118_uart_dev_t *p_dev     = (am_zlg118_uart_dev_t *)p_drv;
    amhw_zlg118_uart_t   *p_hw_uart = (amhw_zlg118_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    /* 使能发送完成中断 */
    amhw_zlg118_uart_int_enable(p_hw_uart, AMHW_ZLG118_UART_INT_TX_COMPLETE);
...
static int __uart_poll_putchar (void *p_drv, char outchar)
{
    ...
    if(amhw_zlg118_uart_flag_check(
        ...
        /* 发送一个字符 */
        amhw_zlg118_uart_data_write(p_hw_uart, outchar);
...
static int __uart_poll_getchar (void *p_drv, char *p_char)
{
    ...
    if(amhw_zlg118_uart_flag_check(
           p_hw_uart, AMHW_ZLG118_UART_FLAG_RX_COMPLETE) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {

        /* 接收一个字符 */
        *p_inchar = amhw_zlg118_uart_data_read(p_hw_uart);

        amhw_zlg118_uart_flag_clr(p_hw_uart,
 ...
 * \brief 串口选项配置
 */
int __uart_opt_set (am_zlg118_uart_dev_t *p_dev, uint32_t options)
{
    amhw_zlg118_uart_t *p_hw_uart = (amhw_zlg118_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    if (options & AM_UART_STOPB) {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZLG118_UART_STOP_2_0_BIT;
    } else {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZLG118_UART_STOP_1_0_BIT;
    }
    ...
    if (options & AM_UART_PARENB) {
        ...
        if (options & AM_UART_PARODD) {
            cfg_flags |= AMHW_ZLG118_UART_HW_PARITY_ODD;
        } else {
            cfg_flags |= AMHW_ZLG118_UART_HW_PARITY_EVEN;
        }
    } else {
        cfg_flags &= ~(0x3ul << 2);
        cfg_flags |= AMHW_ZLG118_UART_PARITY_NO;
    }

    /* 保存和生效配置 */
    amhw_zlg118_uart_stop_bit_sel(p_hw_uart  , (cfg_flags & (0x3ul << 14)));
    amhw_zlg118_uart_parity_bit_sel(p_hw_uart, (cfg_flags & (0x3ul << 2)));
...
static int __uart_flow_rxstat_set (am_zlg118_uart_dev_t *p_dev, uint32_t ctrl)
{
    ...
    p_hw_uart = (amhw_zlg118_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    /* 如果是硬件流控，配置RTS输出引脚电位 */
        ...
        if (AM_UART_FLOWSTAT_ON == ctrl) {
            amhw_zlg118_uart_data_write(p_hw_uart, AM_ZLG118_UART_XON);
        } else {
            amhw_zlg118_uart_data_write(p_hw_uart, AM_ZLG118_UART_XOFF);
        }
    }
...
static int __uart_flow_mode_set(am_zlg118_uart_dev_t *p_dev, uint32_t mode)
{
    ...
    if(AM_UART_FLOWCTL_NO == p_dev->flowctl_mode) {
        ...
        /* 硬件流控禁能     */
        amhw_zlg118_uart_disable(p_hw_uart, AMHW_ZLG118_UART_CTS);
        amhw_zlg118_uart_disable(p_hw_uart, AMHW_ZLG118_UART_RTS);
    ...
    /* 硬件流控 */
        ...
        /* 硬件流控使能     */
        amhw_zlg118_uart_enable(p_hw_uart, AMHW_ZLG118_UART_CTS);
        amhw_zlg118_uart_enable(p_hw_uart, AMHW_ZLG118_UART_RTS);
    } else {
 ...
 * \brief 串口接收中断服务
 */
void __uart_irq_rx_handler (am_zlg118_uart_dev_t *p_dev)
{
    amhw_zlg118_uart_t *p_hw_uart = (amhw_zlg118_uart_t *)p_dev->p_devinfo->uart_reg_base;
    char data;
 ...
 * \brief 串口发送中断服务
 */
void __uart_irq_tx_handler (am_zlg118_uart_dev_t *p_dev)
{
    amhw_zlg118_uart_t *p_hw_uart = (amhw_zlg118_uart_t *)p_dev->p_devinfo->uart_reg_base;
...
int __uart_connect (void *p_drv)
{
    ...
    am_int_enable(p_dev->p_devinfo->inum);
    amhw_zlg118_uart_int_enable(p_dev->p_devinfo->uart_reg_base,
                                      ...
                                      const am_zlg118_uart_devinfo_t *p_devinfo)
{
    ...
    /* 获取配置参数 */
    p_hw_uart                = (amhw_zlg118_uart_t  *)p_devinfo->uart_reg_base;
    ...
    p_dev->other_int_enable  = p_devinfo->other_int_enable  &
                               ~(AMHW_ZLG118_UART_INT_TX_COMPLETE |
                                 AMHW_ZLG118_UART_INT_RX_COMPLETE);
    ...
    if((p_devinfo->work_mode == 2) || (p_devinfo->work_mode == 3)) {
        ...
        if((p_devinfo->mut_addr.enable == AM_TRUE)) {
            ...
            amhw_zlg118_uart_slaver_addr_set(p_hw_uart,
            ...
            /* 多机地址自动识别使能 */
            amhw_zlg118_uart_enable(p_hw_uart, AMHW_ZLG118_UART_MUT_ADR_AUTO);
        }
    }
    ...
    if(p_devinfo->work_mode == AMHW_ZLG118_UART_WORK_MODE_0) {
        ...
        /* 模式0通信时钟分频系数设置无效 */
        amhw_zlg118_uart_clk_div_sel(p_hw_uart,
                                     AMHW_ZLG118_UART_CLK_DIV_MODE0_NO);
    } else {

        /* 设置通信时钟分频系数为最小 */
        amhw_zlg118_uart_clk_div_sel(p_hw_uart,
                                     AMHW_ZLG118_UART_CLK_DIV_MODE2_16);
    }
    ...
    /* 获取串口停止位配置选项 */
    if (p_devinfo->cfg_flags & (AMHW_ZLG118_UART_STOP_2_0_BIT)) {
    ...
    /* 设置波特率 */
    p_dev->baud_rate = amhw_zlg118_uart_baudrate_set(
                                     p_hw_uart,
                                     p_dev->clk_rate,
                                     p_devinfo->baud_rate);
    /* 默认轮询模式 */
...
void am_zlg118_uart_deinit (am_zlg118_uart_dev_t *p_dev)
{
    ...
    amhw_zlg118_uart_disable(p_hw_uart,AMHW_ZLG118_UART_RX);
    amhw_zlg118_uart_disable(p_hw_uart,AMHW_ZLG118_UART_CTS);
    amhw_zlg118_uart_disable(p_hw_uart,AMHW_ZLG118_UART_RTS);
    amhw_zlg118_uart_disable(p_hw_uart,AMHW_ZLG118_UART_DMA_TX);
    amhw_zlg118_uart_disable(p_hw_uart,AMHW_ZLG118_UART_DMA_RX);
    amhw_zlg118_uart_disable(p_hw_uart,AMHW_ZLG118_UART_MUT_ADR_AUTO);

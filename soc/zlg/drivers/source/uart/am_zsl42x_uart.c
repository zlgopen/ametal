...
/** \brief 串口模式（查询或中断）设置 */
static int __uart_mode_set (am_zsl42x_uart_dev_t *p_dev, uint32_t new_mode);

/** \brief 串口硬件设置 */
static int __uart_opt_set (am_zsl42x_uart_dev_t *p_dev, uint32_t opts);

/** \brief ZSL42x 串口驱动函数声明 */
...
/** \brief 串口设备表 */
static am_zsl42x_uart_dev_t *g_dev_plist[4];
...
{
    am_zsl42x_uart_dev_t *p_dev     = (am_zsl42x_uart_dev_t *)p_drv;
    amhw_zsl42x_uart_t   *p_hw_uart =
            (amhw_zsl42x_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    case AM_UART_BAUD_SET:
        ...
        /* 只有在发送空的基础上才允许修改波特率 */
        while (amhw_zsl42x_uart_flag_check(
                   p_hw_uart, AMHW_ZSL42x_UART_FLAG_TX_EMPTY) == AM_FALSE);

        status = amhw_zsl42x_uart_baudrate_set(
...
int __uart_tx_startup (void *p_drv)
{
    ...
    /* 中断发送状态 */
    if (p_hw_uart->scon  & AMHW_ZSL42x_UART_INT_TX_COMPLETE) {
        ...
        if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
            ...
            amhw_zsl42x_uart_data_write(p_hw_uart, data);

            /* 使能发送完成中断 */
            amhw_zsl42x_uart_int_enable(p_hw_uart, AMHW_ZSL42x_UART_INT_TX_COMPLETE);
        }
    }
...
static int __uart_poll_putchar (void *p_drv, char outchar)
{
    ...
    if(amhw_zsl42x_uart_flag_check(
           p_hw_uart, AMHW_ZSL42x_UART_FLAG_TX_EMPTY) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {
        ...
        /* 发送一个字符 */
        amhw_zsl42x_uart_data_write(p_hw_uart, outchar);
...
static int __uart_poll_getchar (void *p_drv, char *p_char)
{
    ...
    if(amhw_zsl42x_uart_flag_check(
           p_hw_uart, AMHW_ZSL42x_UART_FLAG_RX_COMPLETE) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {

        /* 接收一个字符 */
        *p_inchar = amhw_zsl42x_uart_data_read(p_hw_uart);

        amhw_zsl42x_uart_flag_clr(p_hw_uart,
    ...
    if (new_mode == AM_UART_MODE_INT) {
        if (p_dev->p_devinfo->inum == 6){
            am_int_connect(p_dev->p_devinfo->inum,
                           ...
                           (void *)g_dev_plist);
            if (p_dev->p_devinfo->uart_reg_base == ZSL42x_UART0_BASE) {
                g_dev_plist[0] = p_dev;
            } else {
                g_dev_plist[2] = p_dev;
            }
            ...
            am_int_connect(p_dev->p_devinfo->inum,
                           ...
                           (void *)g_dev_plist);
            if (p_dev->p_devinfo->uart_reg_base == ZSL42x_UART1_BASE) {
                g_dev_plist[1] = p_dev;
            } else {
                g_dev_plist[3] = p_dev;
            }
        ...
        /* 使能接收完成中断 */
        amhw_zsl42x_uart_int_enable(p_hw_uart, AMHW_ZSL42x_UART_INT_RX_COMPLETE);
    } else {
 ...
 * \brief 串口选项配置
 */
static int __uart_opt_set (am_zsl42x_uart_dev_t *p_dev, uint32_t options)
{
    amhw_zsl42x_uart_t *p_hw_uart =
            (amhw_zsl42x_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    if (options & AM_UART_STOPB) {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZSL42x_UART_STOP_2_0_BIT;
    } else {
        cfg_flags &= ~(0x3ul << 14);
        cfg_flags |= AMHW_ZSL42x_UART_STOP_1_0_BIT;
    }
    ...
    if (options & AM_UART_PARENB) {
        ...
        if (options & AM_UART_PARODD) {
            cfg_flags |= AMHW_ZSL42x_UART_HW_PARITY_ODD;
        } else {
            cfg_flags |= AMHW_ZSL42x_UART_HW_PARITY_EVEN;
        }
        amhw_zsl42x_uart_mode_sel(p_hw_uart, AMHW_ZSL42x_UART_WORK_MODE_3);
    } else {
        cfg_flags &= ~(0x3ul << 2);
        cfg_flags |= AMHW_ZSL42x_UART_PARITY_NO;
        amhw_zsl42x_uart_mode_sel(p_hw_uart, AMHW_ZSL42x_UART_WORK_MODE_1);
    }

    /* 保存和生效配置 */
    amhw_zsl42x_uart_stop_bit_sel(p_hw_uart  , (cfg_flags & (0x3ul << 14)));
    amhw_zsl42x_uart_parity_bit_sel(p_hw_uart, (cfg_flags & (0x3ul << 2)));
 ...
 * \brief 串口接收中断服务
 */
static void __uart_irq_rx_handler (am_zsl42x_uart_dev_t *p_dev)
{
    amhw_zsl42x_uart_t *p_hw_uart =
            (amhw_zsl42x_uart_t *)p_dev->p_devinfo->uart_reg_base;
    char data;
    ...
    /* 是否为接收Rx中断 */
    if (amhw_zsl42x_uart_flag_check(
            p_hw_uart, AMHW_ZSL42x_UART_FLAG_RX_COMPLETE) == AM_TRUE) {

        amhw_zsl42x_uart_flag_clr(p_hw_uart, AMHW_ZSL42x_UART_FLAG_RX_COMPLETE);

        /* 获取新接收数据 */
        data = amhw_zsl42x_uart_data_read(p_hw_uart);
 ...
 * \brief 串口发送中断服务
 */
static void __uart_irq_tx_handler (am_zsl42x_uart_dev_t *p_dev)
{
    amhw_zsl42x_uart_t *p_hw_uart =
            (amhw_zsl42x_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    if (amhw_zsl42x_uart_flag_check(
            p_hw_uart, AMHW_ZSL42x_UART_FLAG_TX_COMPLETE) == AM_TRUE) {

        amhw_zsl42x_uart_flag_clr(p_hw_uart, AMHW_ZSL42x_UART_FLAG_TX_COMPLETE);
        ...
        if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
            ...
            /* 没有数据传送就关闭发送中断 */
            amhw_zsl42x_uart_int_disable(p_hw_uart,
                                         AMHW_ZSL42x_UART_FLAG_TX_COMPLETE);
 ...
 * \brief 选择串口动作
 */
static void __uart_irq_sel (am_zsl42x_uart_dev_t *p_dev)
{
    ...
    /* 判断中断类型 */
    uart_int_stat = amhw_zsl42x_uart_flag_get(p_hw_uart);

    if (amhw_zsl42x_uart_flag_check(
            p_hw_uart, AMHW_ZSL42x_UART_FLAG_RX_COMPLETE) == AM_TRUE) {
        __uart_irq_rx_handler(p_dev);
    } else if (amhw_zsl42x_uart_flag_check(
            p_hw_uart, AMHW_ZSL42x_UART_FLAG_TX_COMPLETE) == AM_TRUE) {
    ...
    if ((p_dev->other_int_enable & uart_int_stat) != 0) {
        ...
        if (p_dev->pfn_err != NULL) {
            p_dev->pfn_err(p_dev->err_arg,
                           AM_ZSL42x_UART_ERRCODE_UART_OTHER_INT,
                           (void *)p_hw_uart,
                           1);
        }
    }
...
{
    int i;
    am_zsl42x_uart_dev_t  *p_dev = NULL;
 ...
 * \brief 串口初始化函数
 */
am_uart_handle_t am_zsl42x_uart_init (am_zsl42x_uart_dev_t           *p_dev,
                                    const am_zsl42x_uart_devinfo_t *p_devinfo)
{
    amhw_zsl42x_uart_t  *p_hw_uart;
    ...
    /* 获取配置参数 */
    p_hw_uart                = (amhw_zsl42x_uart_t  *)p_devinfo->uart_reg_base;
    ...
    p_dev->other_int_enable  = p_devinfo->other_int_enable  &
                               ~(AMHW_ZSL42x_UART_INT_TX_COMPLETE |
                                 AMHW_ZSL42x_UART_INT_RX_COMPLETE);
    ...
    if (AM_TRUE == p_devinfo->asyn_hd) {
        amhw_zsl42x_uart_single_line_half_enable(p_hw_uart);
    }

    /* 获取串口检验方式配置选项 */
    if ((p_devinfo->cfg_flags & (0xc)) == (AMHW_ZSL42x_UART_HW_PARITY_ODD)) {
        p_dev->options |= AM_UART_PARENB | AM_UART_PARODD;
    } else if ((p_devinfo->cfg_flags & (0xc)) == (AMHW_ZSL42x_UART_HW_PARITY_EVEN)){
    ...
    /* 获取串口停止位配置选项 */
    if (p_devinfo->cfg_flags & (AMHW_ZSL42x_UART_STOP_2_0_BIT)) {
    ...
    /* 设置波特率 */
    p_dev->baud_rate = amhw_zsl42x_uart_baudrate_set(
                                     p_hw_uart,
                                     p_dev->clk_rate,
                                     p_devinfo->baud_rate);
    /* 默认轮询模式 */
 ...
 * \brief 串口去初始化
 */
void am_zsl42x_uart_deinit (am_zsl42x_uart_dev_t *p_dev)
{
    amhw_zsl42x_uart_t *p_hw_uart =
            (amhw_zsl42x_uart_t *)p_dev->p_devinfo->uart_reg_base;
    ...
    amhw_zsl42x_uart_disable(p_hw_uart,AMHW_ZSL42x_UART_RX);
    amhw_zsl42x_uart_disable(p_hw_uart,AMHW_ZSL42x_UART_CTS);
    amhw_zsl42x_uart_disable(p_hw_uart,AMHW_ZSL42x_UART_RTS);
    amhw_zsl42x_uart_disable(p_hw_uart,AMHW_ZSL42x_UART_DMA_TX);
    amhw_zsl42x_uart_disable(p_hw_uart,AMHW_ZSL42x_UART_DMA_RX);
    amhw_zsl42x_uart_disable(p_hw_uart,AMHW_ZSL42x_UART_MUT_ADR_AUTO);

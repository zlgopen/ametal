                                     ...
                                     uint32_t            nbytes)
{
    ...
    while (len--) {
         ...
         amhw_zlg118_uart_data_write(p_hw_uart, (uint8_t)(*p_txbuf++));
    }
                                        ...
                                        uint32_t            nbytes)
{
    ...
    while (len--) {
        ...
        *p_rxbuf++ = (uint8_t)amhw_zlg118_uart_data_read(p_hw_uart);\
                                   ...
                                   uint32_t            baud)
{
    ...
    /* 获取时钟模式 */
    mode = amhw_zlg118_uart_mode_get(p_hw_uart);

    /* 模式0情况下，波特率固定为 PCLK/12 */
    if(mode == AMHW_ZLG118_UART_WORK_MODE_0) {

    	baud = (uint32_t)((float)uart_clk / 12.0);

    /* 模式2情况下，波特率 = 时钟 / 分频系数 */
    } else if (mode == AMHW_ZLG118_UART_WORK_MODE_2) {
        /* 获取分频系数设置位的数字量 */
        clk_bit = amhw_zlg118_uart_clk_div_get(p_hw_uart);
    ...
    } else if ((mode == AMHW_ZLG118_UART_WORK_MODE_1) ||
               (mode == AMHW_ZLG118_UART_WORK_MODE_3) ||
               (mode == AMHW_ZLG118_UART_WORK_MODE_4)) {

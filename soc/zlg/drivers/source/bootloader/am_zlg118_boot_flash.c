...
static int __boot_flash_erase_region(void     *p_drv,
                                     ...
                                     uint32_t  length)
{
    am_zlg118_boot_flash_dev_t *p_dev = (am_zlg118_boot_flash_dev_t *)p_drv;
    ...
    if((length & (sector_size - 1)) != 0) {
     ...
     uint32_t flash_addr_end = p_dev->p_devinfo->base_info.flash_start_addr+ \
     ...
     for (i = 0 ; i < erase_sector_count; i++) {
         ...
         key = am_int_cpu_lock();
         ret = am_zlg118_flash_sector_erase(
             (amhw_zlg118_flash_t *)p_dev->amhw_zlg118_flash, i * sector_size + start_addr);
                                ...
                                uint32_t   size)
{
    ...
    for(i = 0; i < program_sector_count; i++) {
        ...
        key = am_int_cpu_lock();
        ret = am_zlg118_flash_sector_program((amhw_zlg118_flash_t *)p_dev->amhw_zlg118_flash,
 ...
 */
am_boot_flash_handle_t am_zlg118_boot_flash_init(am_zlg118_boot_flash_dev_t        *p_dev,
                                              am_zlg118_boot_flash_devinfo_t *p_devinfo)
{
    if(p_dev == NULL || p_devinfo == NULL) {
        return NULL;
    }
    p_dev->amhw_zlg118_flash = (amhw_zlg118_flash_t *)p_devinfo->flash_reg_rebese;

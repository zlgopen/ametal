...
static int __boot_flash_erase_region(void     *p_drv,
                                     ...
                                     uint32_t  length)
{
    am_zsn700_boot_flash_dev_t *p_dev = (am_zsn700_boot_flash_dev_t *)p_drv;
    ...
    if((length & (sector_size - 1)) != 0) {
     ...
     for (i = 0 ; i < erase_sector_count; i++) {
         ...
         key = am_int_cpu_lock();
         ret = am_zsn700_flash_sector_erase(
             (amhw_zsn700_flash_t *)p_dev->amhw_zsn700_flash, i * sector_size + start_addr);
                                ...
                                uint32_t   size)
{
    ...
    for(i = 0; i < program_sector_count; i++) {
        ...
        key = am_int_cpu_lock();
        ret = am_zsn700_flash_sector_program((amhw_zsn700_flash_t *)p_dev->amhw_zsn700_flash,
 ...
 */
am_boot_flash_handle_t am_zsn700_boot_flash_init(am_zsn700_boot_flash_dev_t        *p_dev,
                                              am_zsn700_boot_flash_devinfo_t *p_devinfo)
{
    if(p_dev == NULL || p_devinfo == NULL) {
        return NULL;
    }
    p_dev->amhw_zsn700_flash = (amhw_zsn700_flash_t *)p_devinfo->flash_reg_rebese;

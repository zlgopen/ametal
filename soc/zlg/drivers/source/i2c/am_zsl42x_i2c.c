...
/** \brief I2C 硬件初始化 */
static int __i2c_hard_init (am_zsl42x_i2c_dev_t *p_dev);

/** \brief I2C 中断处理函数 */
static void __i2c_irq_handler (void *p_arg);

/** \brief I2C 状态机函数 */
static int __i2c_mst_sm_event (am_zsl42x_i2c_dev_t *p_dev, uint32_t event);
...
am_static_inline
struct am_i2c_message *__i2c_msg_out (am_zsl42x_i2c_dev_t *p_dev);
...
/** \brief I2C 硬件初始化 */
am_local int __i2c_hard_init (am_zsl42x_i2c_dev_t *p_dev)
{
    amhw_zsl42x_i2c_t *p_hw_i2c = NULL;
    ...
    p_hw_i2c = (amhw_zsl42x_i2c_t *)p_dev->p_devinfo->i2c_regbase;
    ...
    if (p_dev->p_devinfo->speed < 100000) {
        amhw_zsl42x_i2c_cr_clear(p_hw_i2c, AMHW_ZSL42x_I2C_HIGH_SPEED);
    } else {
        amhw_zsl42x_i2c_cr_set(p_hw_i2c, AMHW_ZSL42x_I2C_HIGH_SPEED);
    }
    amhw_zsl42x_i2c_tm_set(p_hw_i2c, count);
    amhw_zsl42x_i2c_tm_enable(p_hw_i2c);
...
/** \brief I2C 硬件重新初始化 */
am_local int __i2c_hard_re_init (am_zsl42x_i2c_dev_t *p_dev)
{
...
{
    am_zsl42x_i2c_dev_t *p_dev = (am_zsl42x_i2c_dev_t *)p_arg;
    uint32_t             key   = 0;
 ...
 * \return 无
 */
am_local void __softimer_start (am_zsl42x_i2c_dev_t *p_dev)
{
 ...
 * \return 无
 */
am_local void __softimer_stop (am_zsl42x_i2c_dev_t *p_dev)
{
    if (p_dev->p_devinfo->timeout_ms == 0) {
        return;
    }
 ...
 */
am_static_inline
void __i2c_msg_in (am_zsl42x_i2c_dev_t *p_dev, struct am_i2c_message *p_msg)
{
...
static void __i2c_irq_handler (void *p_arg)
{
    am_zsl42x_i2c_dev_t *p_dev    = NULL;
    amhw_zsl42x_i2c_t   *p_hw_i2c = NULL;
    ...
    p_dev    = (am_zsl42x_i2c_dev_t *)p_arg;
    p_hw_i2c = (amhw_zsl42x_i2c_t *)p_dev->p_devinfo->i2c_regbase;

    amhw_zsl42x_i2c_status_get(p_hw_i2c);
}
...
{
    am_zsl42x_i2c_dev_t *p_dev    = (am_zsl42x_i2c_dev_t *)p_drv;
    int key;
 ...
 * \brief I2C初始化
 */
am_i2c_handle_t am_zsl42x_i2c_init (am_zsl42x_i2c_dev_t           *p_dev,
                                 const am_zsl42x_i2c_devinfo_t *p_devinfo)
{
 ...
 * \brief I2C解初始化
 */
void am_zsl42x_i2c_deinit (am_i2c_handle_t handle)
{
    amhw_zsl42x_i2c_t   *p_hw_i2c = NULL;
    am_zsl42x_i2c_dev_t *p_dev    = NULL;
    ...
    p_dev    = (am_zsl42x_i2c_dev_t *)handle;
    p_hw_i2c = (amhw_zsl42x_i2c_t *)p_dev->p_devinfo->i2c_regbase;

    p_dev->i2c_serv.p_funcs = NULL;
    p_dev->i2c_serv.p_drv   = NULL;

    amhw_zsl42x_i2c_disable (p_hw_i2c);
...
static int __i2c_mst_sm_event (am_zsl42x_i2c_dev_t *p_dev, uint32_t event)
{
    ...
    /* 发送起始条件 */
    amhw_zsl42x_i2c_cr_set(p_hw_i2c, AMHW_ZSL42x_I2C_START_ENABLE);

    while(1) {
        /* 获取中断标志 */
        while(0 == amhw_zsl42x_i2c_irq_get(p_hw_i2c)) {
            p_dev->state = __I2C_ST_MSG_START;
        }

        /* 获取I2C状态 */
        state = amhw_zsl42x_i2c_status_get(p_hw_i2c);
        ...
        if(p_cur_trans[p_cur_msg->done_num].flags & AM_I2C_M_RD) {  /* 读操作 */
            ...
            case 0x10: /* 已发送起始条件 */
                /* 清除起始条件 */
                amhw_zsl42x_i2c_cr_clear(p_hw_i2c,
                                       AMHW_ZSL42x_I2C_START_ENABLE);

                /* 传输从机地址和读命令 */
                amhw_zsl42x_i2c_dat_write(p_hw_i2c,
            ...
            case 0x40: /* 已发送SLA +R,已接收ACK */
                ...
                if(p_cur_trans[p_cur_msg->done_num].nbytes > 1) {
                    amhw_zsl42x_i2c_cr_set(p_hw_i2c,
                                           AMHW_ZSL42x_I2C_REPLY_ENABLE);
                }
                break;

            case 0x48: /* 已发送SLA +R,已接收非ACK */
                amhw_zsl42x_i2c_cr_set(p_hw_i2c, AMHW_ZSL42x_I2C_STOP_ENABLE);
                amhw_zsl42x_i2c_cr_set(p_hw_i2c, AMHW_ZSL42x_I2C_START_ENABLE);
                break;
            ...
            case 0x50:/* 已接收数据字节，ACK已返回 */
                ...
                p_cur_trans[p_cur_msg->done_num].p_buf[p_dev->data_ptr++] = \
                    amhw_zsl42x_i2c_dat_read(p_hw_i2c);
            ...
            /* 清除中断标志位 */
            amhw_zsl42x_i2c_cr_clear(p_hw_i2c, AMHW_ZSL42x_I2C_INT_FLAG);
            ...
            case 0x10: /* 已发送起始条件 */
                ...
                /* 传输从机地址和写命令 */
                amhw_zsl42x_i2c_dat_write(p_hw_i2c, (p_cur_trans->addr << 1));
                break;

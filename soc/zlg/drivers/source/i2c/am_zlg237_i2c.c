...
am_local int __i2c_hard_init (am_zlg_i2c_dev_t *p_dev)
{
     ...
     */
    amhw_zlg237_i2c_en(p_hw_i2c,DISABLE);
    tmpreg = 0;

    if (p_dev->i2c_clock_speed <= 100000) {    /** <brief 最高为 100khz */
            /** <brief 标准模式速度计算 */
        ...
        /** <brief 启动I2C外设 */
        amhw_zlg237_i2c_en(p_hw_i2c,ENABLE);
        ...
        /** \breif I2C OAR1 配置 */
        p_hw_i2c->i2c_oar1 = (p_dev->i2c_acknowledgedaddress | (p_dev->i2c_ownaddress1 <<1 ) );

        /* 默认不使用 10bit地址*/
        amhw_zlg237_i2c_endual(p_hw_i2c,DISABLE);
        amhw_zlg237_i2c_iten_mode_set(p_hw_i2c, I2C_CR2_ITERREN, DISABLE);
        amhw_zlg237_i2c_iten_mode_set(p_hw_i2c, I2C_CR2_ITEVTEN, DISABLE);
        amhw_zlg237_i2c_iten_mode_set(p_hw_i2c, I2C_CR2_ITBUFEN, DISABLE);
        /** 启动 I2C外设  */
amhw_zlg237_i2c_en(p_hw_i2c,ENABLE);

/*		 检测BUSY位是否等 0
		while(amhw_zlg237_i2c_checkflagstaus(p_hw_i2c,AMHW_ZLGI2C_FLAG_BUSY));

		 产生起始条件 切换至主模式
		amhw_zlg237_i2c_genstrat(p_hw_i2c);*/
...
static void __i2c_irq_handler (void *p_arg)
{
    ...
    am_i2c_transfer_t *p_cur_trans   = p_dev->p_cur_trans;
    p_hw_i2c = (amhw_zlg237_i2c_t *)p_dev->p_devinfo->i2c_regbase;
...
static int __i2c_mst_sm_event (am_zlg_i2c_dev_t *p_dev, uint32_t event)
{
    ...
    while (1) {
        ...
        case __I2C_ST_IDLE:
        {
            ...
            while(amhw_zlg237_i2c_checkflagstaus(p_hw_i2c,AMHW_ZLG_I2C_FLAG_BUSY));
        }
        ...
        case __I2C_ST_SEND_SLA_ADDR:
        {
            ...
            if ( p_cur_trans->flags & AM_I2C_M_RD) {
                amhw_zlg237_i2c_send7bit_address(p_hw_i2c, p_dev->slave_addr, 1);
                __I2C_NEXT_STATE(__I2C_ST_M_RECV_DATA,
                        __I2C_EVT_NONE);
            } else {
                amhw_zlg237_i2c_send7bit_address(p_hw_i2c, p_dev->slave_addr, 0);
                __I2C_NEXT_STATE(__I2C_ST_M_SEND_DATA,
                        __I2C_EVT_NONE);
            }
            /* 等待EV6事件标志位 */
            ...
            /* 打开TXE RXNE中断使能 */
            amhw_zlg237_i2c_iten_mode_set(p_hw_i2c, I2C_CR2_ITEVTEN, ENABLE);
            amhw_zlg237_i2c_iten_mode_set(p_hw_i2c, I2C_CR2_ITBUFEN, ENABLE);
...
//                    /* EV8_2事件 ： 在最后一个数据发送完成 产生ACK信号后，设置停止位  ，以清除EV8_2 事件 */
//                    amhw_zlg237_i2c_genstop(p_hw_i2c, ENABLE);
                }
                else {

...
#include "am_hk32f103rbt6_spi_int.h"
#include "ametal.h"
#include "am_int.h"
#include "am_gpio.h"
#include "am_delay.h"
...
am_local void __spi_cs_on  (am_hk32f103rbt6_spi_int_dev_t *p_this,
                            am_spi_device_t         *p_dev);
am_local void __spi_cs_off (am_hk32f103rbt6_spi_int_dev_t *p_this,
                            am_spi_device_t         *p_dev);

am_local void __spi_write_data (am_hk32f103rbt6_spi_int_dev_t *p_dev);
am_local void __spi_read_data (am_hk32f103rbt6_spi_int_dev_t *p_dev);

am_local void __spi_irq_handler (void *p_arg);
am_local int  __spi_hard_init (am_hk32f103rbt6_spi_int_dev_t *p_this);
am_local int  __spi_config (am_hk32f103rbt6_spi_int_dev_t *p_this);

am_local int  __spi_mst_sm_event (am_hk32f103rbt6_spi_int_dev_t *p_dev,
                                  uint32_t                 event);
/*******************************************************************************
  SPI驱动函数声明
*******************************************************************************/
 ...
 */
am_local
void __spi_cs_on (am_hk32f103rbt6_spi_int_dev_t *p_this, am_spi_device_t *p_dev)
{
 ...
 */
am_local
void __spi_cs_off (am_hk32f103rbt6_spi_int_dev_t *p_this,
                   am_spi_device_t      *p_dev)
{
    if (p_this->p_tgl_dev == p_dev) {
        p_this->p_tgl_dev = NULL;
...
am_local
void __spi_write_data (am_hk32f103rbt6_spi_int_dev_t *p_dev)
{
    ...
    while(amhw_hk32f103rbt6_spi_status_flag_check (p_hw_spi, AMHW_HK32F103RBT6_SPI_TX_EMPTY_FLAG) == AM_FALSE);
    ...
    if (p_trans->p_txbuf != NULL) {
        if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
            ...
            amhw_hk32f103rbt6_spi_tx_put(p_hw_spi, *ptr);
    ...
    /* tx_buf 无效 */
        ...
        /* 待发送数据无效 直接发0 */
        if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
            amhw_hk32f103rbt6_spi_tx_put(p_hw_spi, 0x00);
        } else {
            amhw_hk32f103rbt6_spi_tx_put(p_hw_spi, 0x0000);
        }
    }
...
am_local
void __spi_read_data (am_hk32f103rbt6_spi_int_dev_t *p_dev)
{
    amhw_hk32f103rbt6_spi_t *p_hw_spi = (amhw_hk32f103rbt6_spi_t *)(p_dev->p_devinfo->spi_reg_base);
    ...
    if (amhw_hk32f103rbt6_spi_status_flag_check (p_hw_spi, AMHW_HK32F103RBT6_SPI_RX_NOT_EMPTY_FLAG) == AM_TRUE) {

        /* rx_buf 有效 */
        if (p_trans->p_rxbuf != NULL && p_dev->nbytes_to_recv) {
            if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
                 *p_buf8 = amhw_hk32f103rbt6_spi_rx_get(p_hw_spi);
            } else {
                 *p_buf32 = amhw_hk32f103rbt6_spi_rx_get(p_hw_spi);
            }

        /* rx_buf 无效或者不需要接收数据 */
        } else {
            if ((p_dev->p_cur_spi_dev->bits_per_word) <= 8) {
                (void)amhw_hk32f103rbt6_spi_rx_get(p_hw_spi);
 ...
 */
am_static_inline
void __spi_msg_in (am_hk32f103rbt6_spi_int_dev_t *p_dev,
 ...
 */
am_static_inline
struct am_spi_message *__spi_msg_out (am_hk32f103rbt6_spi_int_dev_t *p_dev)
{
...
{
    am_hk32f103rbt6_spi_int_dev_t *p_this = (am_hk32f103rbt6_spi_int_dev_t *)p_arg;
 ...
 */
am_local
int __spi_hard_init (am_hk32f103rbt6_spi_int_dev_t *p_this)
{
...
{
    am_hk32f103rbt6_spi_int_dev_t  *p_dev    = (am_hk32f103rbt6_spi_int_dev_t *)p_arg;
    amhw_hk32f103rbt6_spi_t     *p_hw_spi    = (amhw_hk32f103rbt6_spi_t *)
                                         (p_dev->p_devinfo->spi_reg_base);

    /* 发送数据 */
    if(amhw_hk32f103rbt6_spi_status_flag_check(
        p_hw_spi, AMHW_HK32F103RBT6_SPI_TX_EMPTY_FLAG) == AM_TRUE) {
    ...
    /* 接收数据 */
    if(amhw_hk32f103rbt6_spi_status_flag_check(
        p_hw_spi, AMHW_HK32F103RBT6_SPI_RX_NOT_EMPTY_FLAG) == AM_TRUE) {
...
am_local
int __spi_config (am_hk32f103rbt6_spi_int_dev_t *p_this)
{
    const am_hk32f103rbt6_spi_int_devinfo_t *p_devinfo = p_this->p_devinfo;
    amhw_hk32f103rbt6_spi_t              *p_hw_spi     = (amhw_hk32f103rbt6_spi_t *)
...
int __spi_msg_start (void              *p_drv,
                     ...
                     am_spi_message_t  *p_msg)
{
    am_hk32f103rbt6_spi_int_dev_t *p_this = (am_hk32f103rbt6_spi_int_dev_t *)p_drv;
 ...
 */
am_local
int __spi_mst_sm_event (am_hk32f103rbt6_spi_int_dev_t *p_dev, uint32_t event)
{
    amhw_hk32f103rbt6_spi_t *p_hw_spi = (amhw_hk32f103rbt6_spi_t *)
                                  (p_dev->p_devinfo->spi_reg_base);
    ...
    while (1) {
        ...
        case __SPI_ST_M_SEND_DATA:    /* 发送数据 */
        {
            ...
            if (p_dev->data_ptr >= p_cur_trans->nbytes) {
                ...
                if (amhw_hk32f103rbt6_spi_status_flag_check (
                    ...
                    /* Enable RX interrupt */
                    amhw_hk32f103rbt6_spi_int_enable(
                        p_hw_spi,AMHW_HK32F103RBT6_SPI_INT_RX_NOT_EMPTY);


                } else {

                    /* 打开中断等待发送就绪 */
                    amhw_hk32f103rbt6_spi_int_enable(p_hw_spi,
                                               AMHW_HK32F103RBT6_SPI_INT_TX_EMPTY);
                }
            }
            break;
        }
        ...
        case __SPI_ST_M_RECV_DATA:   /* 接收数据 */
        {
            ...
            if (p_dev->nbytes_to_recv == 0) {
                ...
                /* 禁能发送及接收中断 */
                amhw_hk32f103rbt6_spi_int_disable(p_hw_spi,
                                            AMHW_HK32F103RBT6_SPI_INT_TX_EMPTY);
                amhw_hk32f103rbt6_spi_int_disable(p_hw_spi,
                                            AMHW_HK32F103RBT6_SPI_INT_RX_NOT_EMPTY);
 ...
 * \brief SPI初始化
 */
am_spi_handle_t am_hk32f103rbt6_spi_int_init (
    am_hk32f103rbt6_spi_int_dev_t           *p_dev,
    const am_hk32f103rbt6_spi_int_devinfo_t *p_devinfo)
{
 ...
 * \brief SPI 去除初始化
 */
void am_hk32f103rbt6_spi_int_deinit (am_spi_handle_t handle)
{
    am_hk32f103rbt6_spi_int_dev_t *p_dev    = (am_hk32f103rbt6_spi_int_dev_t *)handle;
    amhw_hk32f103rbt6_spi_t       *p_hw_spi = (amhw_hk32f103rbt6_spi_t *)
                                        (p_dev->p_devinfo->spi_reg_base);

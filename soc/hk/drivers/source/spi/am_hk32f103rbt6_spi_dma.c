...
am_local void __spi_cs_on  (am_hk32f103rbt6_spi_dma_dev_t *p_this,
                            am_spi_device_t         *p_dev);
am_local void __spi_cs_off (am_hk32f103rbt6_spi_dma_dev_t *p_this,
                            am_spi_device_t         *p_dev);

am_local int  __spi_hard_init (am_hk32f103rbt6_spi_dma_dev_t *p_this);
am_local int  __spi_config (am_hk32f103rbt6_spi_dma_dev_t *p_this);

am_local int  __spi_mst_sm_event (am_hk32f103rbt6_spi_dma_dev_t *p_dev,
                                  uint32_t                 event);
 ...
 */
am_local
void __spi_cs_on (am_hk32f103rbt6_spi_dma_dev_t *p_this, am_spi_device_t *p_dev)
{
 ...
 */
am_local
void __spi_cs_off (am_hk32f103rbt6_spi_dma_dev_t   *p_this,
                   am_spi_device_t           *p_dev)
{
    if (p_this->p_tgl_dev == p_dev) {
        p_this->p_tgl_dev = NULL;
 ...
 */
am_static_inline
void __spi_msg_in (am_hk32f103rbt6_spi_dma_dev_t *p_dev, struct am_spi_message *p_msg)
{
 ...
 */
am_static_inline
struct am_spi_message *__spi_msg_out (am_hk32f103rbt6_spi_dma_dev_t *p_dev)
{
...
{
    am_hk32f103rbt6_spi_dma_dev_t *p_this = (am_hk32f103rbt6_spi_dma_dev_t *)p_arg;
 ...
 */
am_local
int __spi_hard_init (am_hk32f103rbt6_spi_dma_dev_t *p_this)
{
    amhw_hk32f103rbt6_spi_t *p_hw_spi =
        (amhw_hk32f103rbt6_spi_t *)(p_this->p_devinfo->spi_reg_base);
    ...
    /* NSS采用软件管理*/
    amhw_hk32f103rbt6_spi_ssm_enable(p_hw_spi);

    /* 关闭硬件SPI的NSS引脚输出*/
    amhw_hk32f103rbt6_spi_ssout_disable(p_hw_spi);

    /* 软件管理下，NSS内部为高，与引脚无关。特别注意，无论硬件、软件管理，配置成主机模式，NSS必须为高*/
    amhw_hk32f103rbt6_spi_ssi_set(p_hw_spi,AMHW_HK32F103RBT6_SPI_SSI_TO_NSS_ENABLE);

    /* 配置为主机模式 */
    amhw_hk32f103rbt6_spi_master_salver_set(p_hw_spi, AMHW_HK32F103RBT6_SPI_MASTER);

    /* 配置时钟相位和极性 */
    amhw_hk32f103rbt6_spi_clk_mode_set(p_hw_spi, AMHW_HK32F103RBT6_SPI_CLK_MODE_1 );

    /* 帧格式设置，先发送MSB*/
    amhw_hk32f103rbt6_spi_lsbfirst_set(p_hw_spi, AMHW_HK32F103RBT6_SPI_LSB_FIRST_SEND_MSB);

    /* 分频系数为64*/
    amhw_hk32f103rbt6_spi_baudratefre_set(p_hw_spi,
                                    AMHW_HK32F103RBT6_SPI_BAUDRATE_PRESCALER_128);
...
void __dma_isr (void *p_arg, uint32_t stat)
{
    ...
    if (stat == AM_HK32F103RBT6_DMA_INT_NORMAL)
    {
        ...
        /* 全速运行时，必要的延时，等待接收最后几个数据，否则会丢失最后几个数据*/
        am_udelay(100);
 ...
 */
am_local
int __spi_dma_trans (am_hk32f103rbt6_spi_dma_dev_t *p_dev)
{
    am_hk32f103rbt6_spi_dma_dev_t           *p_this      = (am_hk32f103rbt6_spi_dma_dev_t *)
                                                     p_dev;
    const am_hk32f103rbt6_spi_dma_devinfo_t *p_devinfo   = p_this->p_devinfo;
    ...
    /* 连接DMA中断服务函数 */
    am_hk32f103rbt6_dma_isr_connect(p_this->p_devinfo->dma_chan_tx, 
                           __dma_isr, 
                           (void *)p_this);
    ...
    /* DMA发送通道配置 */
    dma_flags[0] =  AMHW_HK32F103RBT6_DMA_CHAN_PRIORITY_HIGH         |  /* 中断优先级 高 */
                    AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_8BIT         |  /* 内存数据宽度1字节 */
                    AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_8BIT         |  /* 外设数据宽度1字节 */
                    AMHW_HK32F103RBT6_DMA_CHAN_PER_ADD_INC_DISABLE   |  /* 外设地址不自增 */
                    AMHW_HK32F103RBT6_DMA_CHAN_CIRCULAR_MODE_DISABLE ;  /* 关闭循环模式 */

    /* DMA接收通道配置 */
    dma_flags[1] = AMHW_HK32F103RBT6_DMA_CHAN_PRIORITY_HIGH         |  /* 中断优先级 高 */
                   AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_8BIT         |  /* 内存数据宽度1字节 */
                   AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_8BIT         |  /* 外设数据宽度1字节 */
                   AMHW_HK32F103RBT6_DMA_CHAN_PER_ADD_INC_DISABLE   |  /* 外设地址不自增 */
                   AMHW_HK32F103RBT6_DMA_CHAN_CIRCULAR_MODE_DISABLE ;  /* 关闭循环模式 */
    ...
    if (p_cur_trans->bits_per_word > 8) {
        dma_flags[0] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_16BIT;
        dma_flags[0] |= AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_16BIT;
        dma_flags[1] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_16BIT;
        dma_flags[1] |= AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_16BIT;
    } else {
        dma_flags[0] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_8BIT;
        dma_flags[0] |= AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_8BIT;
        dma_flags[1] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_8BIT;
        dma_flags[1] |= AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_8BIT;
    }
    ...
    if (p_cur_trans->p_rxbuf == NULL) {
        ...
        /* 内存地址不自增 */
        dma_flags[1] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_ADD_INC_DISABLE;

        /* 建立接收通道描述符 */
        am_hk32f103rbt6_dma_xfer_desc_build(&(p_this->g_desc[1]),        /* 通道描述符 */
    ...
    /* 存在接收数据 */
        ...
        /* 内存地址自增 */
        dma_flags[1] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_ADD_INC_ENABLE;
    ...
    if (p_cur_trans->p_txbuf == NULL) {
        ...
        /* 内存地址不自增 */
        dma_flags[0] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_ADD_INC_DISABLE;
        ...
        /* 内存地址自增 */
        dma_flags[0] |= AMHW_HK32F103RBT6_DMA_CHAN_MEM_ADD_INC_ENABLE;
    ...
    if (am_hk32f103rbt6_dma_xfer_desc_chan_cfg(&(p_this->g_desc[1]),
    ...
    am_hk32f103rbt6_dma_chan_start(p_this->p_devinfo->dma_chan_rx);
...
am_local
int __spi_config (am_hk32f103rbt6_spi_dma_dev_t *p_this)
{
    const am_hk32f103rbt6_spi_dma_devinfo_t *p_devinfo = p_this->p_devinfo;
    amhw_hk32f103rbt6_spi_t              *p_hw_spi     = (amhw_hk32f103rbt6_spi_t *)
                                                   (p_devinfo->spi_reg_base);
    ...
    if (p_this->p_cur_spi_dev->mode & AM_SPI_LSB_FIRST) {
        mode_flag |= AMHW_HK32F103RBT6_SPI_LSB_FIRST_SEND_LSB;
    }
    ...
    /* 配置SPI模式（时钟相位和极性） */
    amhw_hk32f103rbt6_spi_clk_mode_set(p_hw_spi, 0x3 & p_this->p_cur_spi_dev->mode);

    /* 配置SPI工作特性 */
    amhw_hk32f103rbt6_spi_lsbfirst_set(p_hw_spi, mode_flag);
    ...
    if (p_this->p_cur_spi_dev->bits_per_word == 8) {
        ...
        amhw_hk32f103rbt6_spi_lsbfirst_set(p_hw_spi, mode_flag);
        amhw_hk32f103rbt6_spi_flex_length_set(p_hw_spi,
    ...
    /* 设置SPI速率 */
    amhw_hk32f103rbt6_spi_baudratefre_set(p_hw_spi, p_this->p_devinfo->baud_div);
...
int __spi_msg_start (void              *p_drv,
                     ...
                     am_spi_message_t  *p_msg)
{
    am_hk32f103rbt6_spi_dma_dev_t *p_this = (am_hk32f103rbt6_spi_dma_dev_t *)p_drv;
 ...
 */
am_local
int __spi_mst_sm_event (am_hk32f103rbt6_spi_dma_dev_t *p_dev, uint32_t event)
{
    amhw_hk32f103rbt6_spi_t *p_hw_spi = (amhw_hk32f103rbt6_spi_t *)
    ...
    while (1) {
        ...
        case __SPI_ST_MSG_START:    /* 消息开始 */
        {
            ...
            if (p_cur_msg) {
                ...
                /* 禁能所有中断 */
                amhw_hk32f103rbt6_spi_int_disable(p_hw_spi, AMHW_HK32F103RBT6_SPI_INT_ALL);

                amhw_hk32f103rbt6_spi_dma_tx_disable(p_hw_spi);
                amhw_hk32f103rbt6_spi_dma_rx_disable(p_hw_spi);
...
{
    am_hk32f103rbt6_spi_dma_dev_t *p_dev    = (am_hk32f103rbt6_spi_dma_dev_t *)handle;
    amhw_hk32f103rbt6_spi_t       *p_hw_spi = (amhw_hk32f103rbt6_spi_t *)
                                        (p_dev->p_devinfo->spi_reg_base);

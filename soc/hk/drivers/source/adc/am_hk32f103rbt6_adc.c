...
#define __ADC_HW_DECL(p_hw_adc, p_drv)   \
            amhw_hk32f103rbt6_adc_t *p_hw_adc =   \
            ((am_hk32f103rbt6_adc_dev_t *)p_drv)->p_devinfo->p_hw_adc

#define __ADC_VREF_GET(p_drv)  (((am_hk32f103rbt6_adc_dev_t *)p_drv)->p_devinfo->vref)
 ...
 * \brief ADC数据转换完成中断
 */
void __hk32f103rbt6_adc_irq_handle (void *p_arg)
{
    am_hk32f103rbt6_adc_dev_t    *p_dev      = (am_hk32f103rbt6_adc_dev_t *)p_arg;
    amhw_hk32f103rbt6_adc_t      *p_hw_adc   =  NULL;
    ...
    p_hw_adc =  (amhw_hk32f103rbt6_adc_t *)(p_dev->p_devinfo->adc_reg_base);
    ...
    if (p_dev->conv_cnt < p_desc->length) {
        ...
        /* 以清除转换结束中断标志 */
        amhw_hk32f103rbt6_adc_status_flag_clr(p_hw_adc,AMHW_HK32F103RBT6_ADC_END_FLAG);

        /* 读取数据 */
        adc_dat = amhw_hk32f103rbt6_adc_regular_data_get (p_hw_adc);
        ...
        if (p_dev->conv_cnt >= p_desc->length) {
            ...
            if (p_dev->desc_index == p_dev->desc_num) {
                ...
                if (p_dev->count != 0 && p_dev->seq_cnt == p_dev->count) {
                    ...
                    amhw_hk32f103rbt6_adc_status_flag_clr(p_hw_adc,
                                                    AMHW_HK32F103RBT6_ADC_END_FLAG);
        ...
        if (amhw_hk32f103rbt6_adc_status_flag_check(
            p_hw_adc,AMHW_HK32F103RBT6_ADC_END_FLAG) == AM_TRUE) {
            amhw_hk32f103rbt6_adc_status_flag_clr(p_hw_adc,AMHW_HK32F103RBT6_ADC_END_FLAG);
        }
...
{
    am_hk32f103rbt6_adc_dev_t *p_dev = NULL;
    amhw_hk32f103rbt6_adc_t   *p_hw_adc;
    ...
    p_dev    = (am_hk32f103rbt6_adc_dev_t *)p_drv;
    p_hw_adc = (amhw_hk32f103rbt6_adc_t *)p_dev->p_devinfo->adc_reg_base;
 ...
 */
static void __adc_int_work_startup (am_hk32f103rbt6_adc_dev_t       *p_dev,
                                    amhw_hk32f103rbt6_adc_channel_t  chan)
{
    amhw_hk32f103rbt6_adc_t *p_hw_adc = (amhw_hk32f103rbt6_adc_t *)
                           ...
                           void                *p_arg)
{
    ...
    __adc_int_work_startup(p_dev,(amhw_hk32f103rbt6_adc_channel_t)chan); /* 中断工作模式启动配置 */
...
static int __fn_adc_stop (void *p_drv, int chan)
{
    am_hk32f103rbt6_adc_dev_t *p_dev    = (am_hk32f103rbt6_adc_dev_t *)p_drv;
    amhw_hk32f103rbt6_adc_t   *p_hw_adc = (amhw_hk32f103rbt6_adc_t *)
                                    (p_dev->p_devinfo->adc_reg_base);
    ...
    amhw_hk32f103rbt6_adc_int_disable(p_hw_adc, AMHW_HK32F103RBT6_ADC_INT_END);
    amhw_hk32f103rbt6_adc_disable(p_hw_adc);
...
static int __fn_adc_rate_get (void          *p_drv,
                              ...
                              uint32_t      *p_rate)
{
    am_hk32f103rbt6_adc_dev_t *p_dev;
    amhw_hk32f103rbt6_adc_t   *p_hw_adc;
    ...
    i = amhw_hk32f103rbt6_adc_smpr_get(p_hw_adc,
                                (amhw_hk32f103rbt6_adc_channel_t)p_dev->chan);
    sample = sample_time[i];

    /* ADC分频设置位于RCC驱动中 */
    adc_clkdiv = 2 * (amhw_hk32f103rbt6_rcc_adc_div_get() + 1);
                              ...
                              uint32_t       rate)
{
    ...
    amhw_hk32f103rbt6_rcc_adc_div_set (adc_rate_err.adc_div / 2 - 1);
    amhw_hk32f103rbt6_adc_smpr_set (p_hw_adc,
          (amhw_hk32f103rbt6_adc_sample_time_t)adc_rate_err.sample_time,
          (amhw_hk32f103rbt6_adc_channel_t)chan);
...
{
    /* 仅支持12位分辨率 */
    return AMHW_HK32F103RBT6_ADC_DATA_VALID_12BIT;
}
 ...
 * \brief ADC初始化
 */
am_adc_handle_t am_hk32f103rbt6_adc_init (am_hk32f103rbt6_adc_dev_t           *p_dev,
                                    const am_hk32f103rbt6_adc_devinfo_t *p_devinfo)
{
    uint16_t bit = 0;
    ...
    /* 对齐方式--右对齐*/
    amhw_hk32f103rbt6_adc_data_alignment_set(p_hw_adc,AM_ADC_DATA_ALIGN_RIGHT);
    ...
    if( p_dev->p_devinfo->temp_mode == 0) {
        ...
        amhw_hk32f103rbt6_adc_tsvrefe_enable(p_hw_adc);
    }
 ...
 * \brief ADC去初始化
 */
void am_hk32f103rbt6_adc_deinit (am_adc_handle_t handle)
{
    am_hk32f103rbt6_adc_dev_t *p_dev    = (am_hk32f103rbt6_adc_dev_t *)handle;
    amhw_hk32f103rbt6_adc_t   *p_hw_adc = NULL;
    ...
    /* 关闭所有中断 */
    amhw_hk32f103rbt6_adc_int_disable(p_hw_adc, AMHW_HK32F103RBT6_ADC_INT_ALL);

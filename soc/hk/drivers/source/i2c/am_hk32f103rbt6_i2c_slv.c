...
#include "ametal.h"
#include "am_hk32f103rbt6_i2c_slv.h"
...
/** \brief I2C硬件初始化 */
static int __i2c_slv_hard_init(am_hk32f103rbt6_i2c_slv_dev_t *p_dev);

/** \brief I2C中断处理函数 */
static void __hk32f103rbt6_i2c_slv_irq_handler (void *p_arg);
...
static int __i2c_slv_setup (void *p_drv, am_i2c_slv_device_t *p_i2c_slv_dev)
{
    ...
    am_hk32f103rbt6_i2c_slv_dev_t *p_dev      = NULL;
    amhw_hk32f103rbt6_i2c_t       *p_hw_i2c   = NULL;
    ...
    p_dev    = (am_hk32f103rbt6_i2c_slv_dev_t *)p_drv;
    p_hw_i2c = (amhw_hk32f103rbt6_i2c_t *) (p_dev->p_devinfo->i2c_regbase);

    /* 获取总的从设备个数 */
    hk32f103rbt6_i2c_slv_dev_num            = p_dev->hk32f103rbt6_i2c_slv_dev_num;
    ...
    for ( i = 0; i < hk32f103rbt6_i2c_slv_dev_num; i++) {
    ...
    /* 从机设备都用关了，无法开启该从机设备 */
    if ( i == hk32f103rbt6_i2c_slv_dev_num) {
        return -AM_EAGAIN;
    }
...
//    amhw_hk32f103rbt6_i2c_disable (p_hw_i2c);
    ...
    if ( p_i2c_slv_dev->dev_flags & AM_I2C_SLV_GEN_CALL_ACK) {
        amhw_hk32f103rbt6_i2c_engc(p_hw_i2c, ENABLE);
    } else {
        amhw_hk32f103rbt6_i2c_engc(p_hw_i2c, DISABLE);
    }
...
static int __i2c_slv_shutdown (void *p_drv, am_i2c_slv_device_t *p_i2c_slv_dev)
{
    am_hk32f103rbt6_i2c_slv_dev_t *p_dev    = (am_hk32f103rbt6_i2c_slv_dev_t *)p_drv;
    amhw_hk32f103rbt6_i2c_t    *p_hw_i2c_slv = NULL;

    if ( (p_dev              == NULL) ||
         (p_i2c_slv_dev      == NULL)) {
        return -AM_EINVAL;
    }
    p_hw_i2c_slv = (amhw_hk32f103rbt6_i2c_t *) p_dev->p_devinfo->i2c_regbase;
    ...
    amhw_hk32f103rbt6_i2c_disable(p_hw_i2c_slv);
...
{
    am_hk32f103rbt6_i2c_slv_dev_t *p_dev = ( am_hk32f103rbt6_i2c_slv_dev_t *) p_arg;
 ...
 * \brief 从机接受数据
 */
void __hk32f103rbt6_i2c_slv_irq_rx_handler (am_hk32f103rbt6_i2c_slv_dev_t *p_dev)
{
    ...
    /* 接受数据 */
    uint8_t rx_data = amhw_hk32f103rbt6_i2c_recv_data (p_hw_i2c);
 ...
 * \brief 从机发送数据
 */
void __hk32f103rbt6_i2c_slv_irq_tx_handler (am_hk32f103rbt6_i2c_slv_dev_t *p_dev)
{
    amhw_hk32f103rbt6_i2c_t   *p_hw_i2c  = (amhw_hk32f103rbt6_i2c_t *)
                                     p_dev->p_devinfo->i2c_regbase;
    am_i2c_slv_device_t *p_i2c_slv = p_dev->p_i2c_slv_dev[0];
    ...
    amhw_hk32f103rbt6_i2c_send_data(p_hw_i2c, tx_data);
...
static void __hk32f103rbt6_i2c_slv_irq_handler (void *p_arg)
{
    ...
    am_hk32f103rbt6_i2c_slv_dev_t *p_dev              = (am_hk32f103rbt6_i2c_slv_dev_t *)
                                                   p_arg;
    amhw_hk32f103rbt6_i2c_t       *p_hw_i2c_slv       = (amhw_hk32f103rbt6_i2c_t *)
                                                  p_dev->p_devinfo->i2c_regbase;
    am_i2c_slv_device_t     *p_i2c_slv_dev      = p_dev->p_i2c_slv_dev[0];
    uint16_t                 i2c_slv_int_status = amhw_hk32f103rbt6_i2c_getitstatus(
                                                      p_hw_i2c_slv);


    /* EV1 事件: 收到从地址*/
    if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__ADDR) {

        /* 读sr2 清除中断标志位 */
        amhw_hk32f103rbt6_i2c_read_reg(p_hw_i2c_slv->i2c_sr2);
        ...
        /* 收到广播呼叫从地址  */
        if (amhw_hk32f103rbt6_i2c_checkflagstaus(p_hw_i2c_slv,
                AMHW_HK32F103RBT6_I2C_FLAG_GENCALL)) {
    ...
    if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__BTF) {
        if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__RXNE) {

            /* 开始接收数据 */
            __hk32f103rbt6_i2c_slv_irq_rx_handler (p_dev);
        } else if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__TXE) {

            /* 开始发送数据 */
            __hk32f103rbt6_i2c_slv_irq_tx_handler (p_dev);
        }
    }
     ...
     */
    if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__TXE) {
    ...
    /* 传输数据结束时 */
    if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__STOPF) {
        if (i2c_slv_int_status & (uint16_t)AMHW_HK32F103RBT6_INT_FLAG__AF) {
            /* EV3_2 从发送器清除 AF */
            amhw_hk32f103rbt6_i2c_clearflag(p_hw_i2c_slv, AMHW_HK32F103RBT6_I2C_FLAG_AF);
        } else {

            /* EV4  */
            amhw_hk32f103rbt6_i2c_read_reg(p_hw_i2c_slv->i2c_sr1);
                                      ...
                                      const am_hk32f103rbt6_i2c_slv_devinfo_t *p_devinfo)
{
    ...
    p_dev->hk32f103rbt6_i2c_slv_dev_num =1;  /* 只能生成一个从机设备 */
    ...
    /* 连接中断 */
    am_int_connect(p_dev->p_devinfo->inum, __hk32f103rbt6_i2c_slv_irq_handler,
                       (void *)p_dev);
...
void am_hk32f103rbt6_i2c_slv_deinit (am_i2c_slv_handle_t handle)
{
    ...
    p_dev    = (am_hk32f103rbt6_i2c_slv_dev_t *)handle;
    p_hw_i2c_slv = (amhw_hk32f103rbt6_i2c_t *)p_dev->p_devinfo->i2c_regbase;
    ...
    am_int_disconnect(p_dev->p_devinfo->inum, __hk32f103rbt6_i2c_slv_irq_handler,
                         (void *)p_dev);

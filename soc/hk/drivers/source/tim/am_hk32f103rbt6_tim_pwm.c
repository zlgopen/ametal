...
static int __tim_pwm_config (void          *p_drv,
                             ...
                             unsigned long  period_ns)
{
    am_hk32f103rbt6_tim_pwm_dev_t *p_dev    = (am_hk32f103rbt6_tim_pwm_dev_t *)p_drv;
...

   {
    ...
    /* 写入分频值 */
    amhw_hk32f103rbt6_tim_prescale_set(p_hw_tim, pre_reg);
    ...
    amhw_hk32f103rbt6_tim_arr_set(p_hw_tim, period_c - 1);

    /* 使能定ARR预装载 */
    amhw_hk32f103rbt6_tim_arpe_enable(p_hw_tim);

    /* Low level time */
    duty_c = period_c - duty_c;

    /* 填充的值为PWM输出低电平的时间 */
    amhw_hk32f103rbt6_tim_ccr_ouput_reload_val_set(p_hw_tim, duty_c - 1, chan);

    /* 选择该通道为输出 */
    amhw_hk32f103rbt6_tim_ccs_set(p_hw_tim, 0, chan);

    /* 选择该通道的PWM模式 */
    amhw_hk32f103rbt6_tim_ocm_set(p_hw_tim, p_dev->p_devinfo->pwm_mode, chan);

    /* PWM输出通道预装载使能 */
    amhw_hk32f103rbt6_tim_ccs_ocpe_enable(p_hw_tim, chan);

    if ((p_dev->p_devinfo->tim_type == AMHW_HK32F103RBT6_TIM_TYPE0) ||
        (p_dev->p_devinfo->tim_type == AMHW_HK32F103RBT6_TIM_TYPE2) ||
        (p_dev->p_devinfo->tim_type == AMHW_HK32F103RBT6_TIM_TYPE3)) {

        /* 设置CR2寄存器确定N极极性 */
    }
    /* 使能互补输出高电平有效 */
    amhw_hk32f103rbt6_tim_ccne_set(p_hw_tim, p_dev->p_devinfo->ocpolarity, chan);

    /* 设置比较输出通道高电平极性有效 */
    amhw_hk32f103rbt6_tim_ccp_output_set(p_hw_tim, p_dev->p_devinfo->ocpolarity, chan);
...
static int __tim_pwm_enable (void *p_drv, int chan)
{
    ...
    am_hk32f103rbt6_tim_pwm_dev_t    *p_dev    = (am_hk32f103rbt6_tim_pwm_dev_t *)p_drv;
    amhw_hk32f103rbt6_tim_t          *p_hw_tim = (amhw_hk32f103rbt6_tim_t *)p_dev->p_devinfo->tim_regbase;
    am_hk32f103rbt6_tim_pwm_chaninfo_t *p_chaninfo = p_dev->p_devinfo->p_chaninfo;
    ...
    for(i = 0; i <= p_dev->p_devinfo->channels_num; i++){
        if((p_chaninfo[i].channel & 0x7f) ==  chan){
            ...
            if((p_chaninfo[i].channel & 0x80) == 0x80){
                /* 使能互补输出 */
                amhw_hk32f103rbt6_tim_ccne_enable(p_hw_tim, chan);
                enable_flag = 1;
            }else{
                /* 使能通道PWM输出 */
                amhw_hk32f103rbt6_tim_cce_output_enable(p_hw_tim, chan);
                enable_flag = 1;
            }
        }
    }
    ...
    if ((p_dev->p_devinfo->tim_type == AMHW_HK32F103RBT6_TIM_TYPE0) ||
        ...
        amhw_hk32f103rbt6_tim_bdtr_enable(p_hw_tim, AMHW_HK32F103RBT6_TIM_MOE);
    }
...
static int __tim_pwm_disable (void *p_drv, int chan)
{
    ...
    /* 禁能通道PWM输出 */
    amhw_hk32f103rbt6_tim_cce_output_disable(p_hw_tim, chan);
    ...
    if ((p_dev->p_devinfo->tim_type == AMHW_HK32F103RBT6_TIM_TYPE0) ||
        ...
        amhw_hk32f103rbt6_tim_bdtr_disable(p_hw_tim, AMHW_HK32F103RBT6_TIM_MOE);
    }
...
/**
  * \brief pwm初始化
  */
void __tim_pwm_init (amhw_hk32f103rbt6_tim_t     *p_hw_tim,
...
/**
  * \brief tim pwm服务初始化
  */
am_pwm_handle_t am_hk32f103rbt6_tim_pwm_init (am_hk32f103rbt6_tim_pwm_dev_t           *p_dev,
                                     const am_hk32f103rbt6_tim_pwm_devinfo_t *p_devinfo)
{
...
void am_hk32f103rbt6_tim_pwm_deinit (am_pwm_handle_t handle)
{

    am_hk32f103rbt6_tim_pwm_dev_t *p_dev    = (am_hk32f103rbt6_tim_pwm_dev_t *)handle;
    amhw_hk32f103rbt6_tim_t       *p_hw_tim = NULL;

 ...
 * \endinternal
 */
#include "am_hk32f103rbt6_usart.h"
#include "am_delay.h"
#include "am_clk.h"
#include "am_int.h"
#include "am_gpio.h"
 ...
 * \brief 串口模式（查询或中断）设置
 */
int __usart_mode_set (am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t new_mode);

/**
 * \brief 串口硬件设置
 */
int __usart_opt_set (am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t opts);

/**
 * \brief 流控接受器流控状态设置
 */
static int __usart_flow_rxstat_set (am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t ctrl);

/**
 * \brief 流控模式设置
 */
static int __usart_flow_mode_set (am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t mode);

/**
 * \brief 流控发送器流控状态获取
 */
static int __usart_flow_txstat_get (am_hk32f103rbt6_usart_dev_t *p_dev);

/* HK32F103RBT6 串口驱动函数声明 */
...
{
    am_hk32f103rbt6_usart_dev_t *p_dev     = (am_hk32f103rbt6_usart_dev_t *)p_drv;
    amhw_hk32f103rbt6_usart_t   *p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;
    ...
    case AM_UART_BAUD_SET:
        ...
        while (amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart, AMHW_HK32F103RBT6_USART_TX_COMPLETE_FLAG) == AM_FALSE);

        status = amhw_hk32f103rbt6_usart_baudrate_set(p_hw_usart,
        ...
        if (status > 0) {
            ...
            /* 寄存器需要重新使能 UART 才能生效 */
            amhw_hk32f103rbt6_usart_disable(p_hw_usart);
...
static int __usart_poll_putchar (void *p_drv, char outchar)
{
    ...
    if(amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart, AMHW_HK32F103RBT6_USART_TX_EMPTY_FLAG) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {
        ...
        /* 发送一个字符 */
        amhw_hk32f103rbt6_usart_data_write(p_hw_usart, outchar);
...
static int __usart_poll_getchar (void *p_drv, char *p_char)
{
    ...
    if(amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart, AMHW_HK32F103RBT6_USART_RX_NOT_EMPTY_FLAG) == AM_FALSE) {
        return (-AM_EAGAIN);
    } else {

        /* 接收一个字符 */
        *p_inchar = amhw_hk32f103rbt6_usart_data_read(p_hw_usart);
    }
 ...
 * \brief 串口选项配置
 */
int __usart_opt_set (am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t options)
{
    amhw_hk32f103rbt6_usart_t *p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;
    uint8_t             cfg_flags = 0;
    ...
    if (options & AM_UART_STOPB) {
        cfg_flags &= ~(0x1 << 2);
        cfg_flags |= AMHW_HK32F103RBT6_USART_STOP_20_BIT;
    } else {
        cfg_flags &= ~(0x1 << 2);
        cfg_flags |= AMHW_HK32F103RBT6_USART_STOP_10_BIT;
    }
    ...
    if (options & AM_UART_PARENB) {
        ...
        if (options & AM_UART_PARODD) {
            cfg_flags |= AMHW_HK32F103RBT6_USART_PARITY_ODD;
        } else {
            cfg_flags |= AMHW_HK32F103RBT6_USART_PARITY_EVEN;
        }
    } else {
        cfg_flags &= ~(0x3 << 0);
        cfg_flags |= AMHW_HK32F103RBT6_USART_PARITY_NO;
    }

    /* 保存和生效配置 */
    amhw_hk32f103rbt6_usart_stop_bit_sel(p_hw_usart, (cfg_flags & 0x4));
    amhw_hk32f103rbt6_usart_parity_bit_sel(p_hw_usart, (cfg_flags & 0x3));
...
static int __usart_flow_rxstat_set (am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t ctrl)
{
    amhw_hk32f103rbt6_usart_t *p_hw_usart = NULL;

    if (NULL == p_dev) {
        return -AM_EINVAL;
    }

    p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;
    ...
    /* 如果是硬件流控，配置RTS输出引脚电位 */
        ...
        if (AM_UART_FLOWSTAT_ON == ctrl) {
            amhw_hk32f103rbt6_usart_data_write(p_hw_usart, AM_HK32F103RBT6_USART_XON);
        } else {
            amhw_hk32f103rbt6_usart_data_write(p_hw_usart, AM_HK32F103RBT6_USART_XOFF);
        }
    }
...
static int __usart_flow_mode_set(am_hk32f103rbt6_usart_dev_t *p_dev, uint32_t mode)
{
    ...
    if(AM_UART_FLOWCTL_NO == p_dev->flowctl_mode) {
        ...
        amhw_hk32f103rbt6_usart_rts_disable(p_hw_usart);
        amhw_hk32f103rbt6_usart_cts_disable(p_hw_usart);
        ...
        amhw_hk32f103rbt6_usart_rts_enable(p_hw_usart);
        amhw_hk32f103rbt6_usart_cts_enable(p_hw_usart);
    } else {
        amhw_hk32f103rbt6_usart_rts_disable(p_hw_usart);
        amhw_hk32f103rbt6_usart_cts_disable(p_hw_usart);
        am_gpio_pin_cfg(p_dev->p_devinfo->gpio_rts[0], AM_GPIO_INPUT);
...
static int __usart_flow_txstat_get (am_hk32f103rbt6_usart_dev_t *p_dev)
{
    ...
    if(AM_UART_FLOWCTL_HW == p_dev->flowctl_mode) {
        ...
        /* cts的状态发生变化时，状态位有效 */
        if(amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart,AMHW_HK32F103RBT6_USART_CTS_FLAG) == AM_TRUE) {

            /* 清除cts状态位 */
            amhw_hk32f103rbt6_usart_status_flag_clr(p_hw_usart,AMHW_HK32F103RBT6_USART_CTS_FLAG);
 ...
 * \brief 串口接收中断服务
 */
void __usart_irq_rx_handler (am_hk32f103rbt6_usart_dev_t *p_dev)
{
    amhw_hk32f103rbt6_usart_t *p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;
    char data;

    /* 是否为接收Rx中断 */
    if (amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart,AMHW_HK32F103RBT6_USART_RX_NOT_EMPTY_FLAG) == AM_TRUE) {
 ...
 * \brief 串口发送中断服务
 */
void __usart_irq_tx_handler (am_hk32f103rbt6_usart_dev_t *p_dev)
{
    amhw_hk32f103rbt6_usart_t *p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;

    char data;

    if (amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart,AMHW_HK32F103RBT6_USART_TX_EMPTY_FLAG) == AM_TRUE) {

        amhw_hk32f103rbt6_usart_status_flag_clr(p_hw_usart, AMHW_HK32F103RBT6_USART_TX_EMPTY_FLAG);
        /* 获取发送数据并发送 */
        if ((p_dev->pfn_txchar_get(p_dev->txget_arg, &data)) == AM_OK) {
            amhw_hk32f103rbt6_usart_data_write(p_hw_usart, data);
        } else {
...
{
    am_hk32f103rbt6_usart_dev_t  *p_dev     = (am_hk32f103rbt6_usart_dev_t *)p_arg;
    amhw_hk32f103rbt6_usart_t    *p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;
    ...
    } else if (amhw_hk32f103rbt6_usart_status_flag_check(p_hw_usart,AMHW_HK32F103RBT6_USART_TX_EMPTY_FLAG) == AM_TRUE) {
    ...
    if ((p_dev->other_int_enable & usart_int_stat) != 0) {
        ...
        if (p_dev->pfn_err != NULL) {
            p_dev->pfn_err(p_dev->err_arg,
                           AM_HK32F103RBT6_USART_ERRCODE_USART_OTHER_INT,
                           (void *)p_hw_usart,
                           1);
        }
    }
...
int __usart_connect (void *p_drv)
{
    ...
    am_int_enable(p_dev->p_devinfo->inum);
    amhw_hk32f103rbt6_usart_int_enable(p_dev->p_devinfo->usart_reg_base,
 ...
 * \brief 串口初始化函数
 */
am_uart_handle_t am_hk32f103rbt6_usart_init (am_hk32f103rbt6_usart_dev_t           *p_dev,
                                       const am_hk32f103rbt6_usart_devinfo_t *p_devinfo)
{
    amhw_hk32f103rbt6_usart_t  *p_hw_usart;
    uint32_t              tmp;
    ...
    /* 获取配置参数 */
    p_hw_usart                = (amhw_hk32f103rbt6_usart_t  *)p_devinfo->usart_reg_base;
    ...
    /* 设置波特率 */
    p_dev->baud_rate = amhw_hk32f103rbt6_usart_baudrate_set(
        p_hw_usart,
        am_clk_rate_get(p_dev->p_devinfo->clk_num),
        p_devinfo->baud_rate);
 ...
 * \brief 串口去初始化
 */
void am_hk32f103rbt6_usart_deinit (am_hk32f103rbt6_usart_dev_t *p_dev)
{
    amhw_hk32f103rbt6_usart_t *p_hw_usart = (amhw_hk32f103rbt6_usart_t *)p_dev->p_devinfo->usart_reg_base;

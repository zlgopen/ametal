...
static int __crc_init (void *p_cookie, am_crc_pattern_t *p_pattern)
{
    am_hk32f103rbt6_crc_dev_t *p_dev      = (am_hk32f103rbt6_crc_dev_t *)p_cookie;
    amhw_hk32f103rbt6_crc_t   *p_hw_crc   = NULL;
    ...
    p_hw_crc = (amhw_hk32f103rbt6_crc_t *)(p_dev->p_devinfo->crc_reg_base);

    /* 重置寄存器CRC_DATA 为0xFFFF FFFF */
    amhw_hk32f103rbt6_crc_reset(p_hw_crc);
...
static int __crc_cal (void *p_cookie, const uint8_t *p_data, uint32_t nbytes)
{
    ...
    uint32_t tdata;
    am_hk32f103rbt6_crc_dev_t *p_dev    = (am_hk32f103rbt6_crc_dev_t *)p_cookie;
    amhw_hk32f103rbt6_crc_t   *p_hw_crc = (amhw_hk32f103rbt6_crc_t *)(p_dev->p_devinfo->crc_reg_base);
    ...
    for (i = 0; i < nbytes; i+=4) {
        ...
        amhw_hk32f103rbt6_crc_32bit_write_data(p_hw_crc, tdata);
    }
...
static int __crc_final (void *p_cookie, uint32_t *p_value)
{
   ...
   *p_value = amhw_hk32f103rbt6_crc_32bit_read_data(p_hw_crc);
 ...
 * \brief CRC初始化
 */
am_crc_handle_t am_hk32f103rbt6_crc_init (am_hk32f103rbt6_crc_dev_t           *p_dev,
                                 const am_hk32f103rbt6_crc_devinfo_t *p_devinfo)
{
 ...
 * \brief CRC解初始化
 */
void am_hk32f103rbt6_crc_deinit (am_crc_handle_t handle)
{
    am_hk32f103rbt6_crc_dev_t *p_dev    = (am_hk32f103rbt6_crc_dev_t *)handle;

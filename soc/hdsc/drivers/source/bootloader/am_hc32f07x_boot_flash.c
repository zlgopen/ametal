...
static int __boot_flash_erase_region(void     *p_drv,
                                     ...
                                     uint32_t  length)
{
    am_hc32f07x_boot_flash_dev_t *p_dev = (am_hc32f07x_boot_flash_dev_t *)p_drv;
    ...
    if((length & (sector_size - 1)) != 0) {
     ...
     for (i = 0 ; i < erase_sector_count; i++) {
         ...
         key = am_int_cpu_lock();
         ret = am_hc32f07x_flash_sector_erase(
             (amhw_hc32f07x_flash_t *)p_dev->amhw_hc32f07x_flash, i * sector_size + start_addr);
                                ...
                                uint32_t   size)
{
    ...
    for(i = 0; i < program_sector_count; i++) {
        ...
        key = am_int_cpu_lock();
        ret = am_hc32f07x_flash_sector_program((amhw_hc32f07x_flash_t *)p_dev->amhw_hc32f07x_flash,
 ...
 */
am_boot_flash_handle_t am_hc32f07x_boot_flash_init(am_hc32f07x_boot_flash_dev_t        *p_dev,
                                              am_hc32f07x_boot_flash_devinfo_t *p_devinfo)
{
    if(p_dev == NULL || p_devinfo == NULL) {
        return NULL;
    }
    p_dev->amhw_hc32f07x_flash = (amhw_hc32f07x_flash_t *)p_devinfo->flash_reg_rebese;

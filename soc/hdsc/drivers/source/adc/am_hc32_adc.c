...
#define __ADC_HW_DECL(p_hw_adc, p_drv)   \
            ...
            ((am_hc32_adc_dev_t *)p_drv)->p_devinfo->p_hw_adc

#define __ADC_VREF_GET(p_drv)  (((am_hc32_adc_dev_t *)p_drv)->p_devinfo->vref)
 ...
 * \brief ADC数据转换完成中断
 */
void __hc32_adc_irq_handle (void *p_arg)
{
    am_hc32_adc_dev_t    *p_dev      = (am_hc32_adc_dev_t *)p_arg;
    amhw_hc32_adc_t      *p_hw_adc   =  NULL;
    ...
    p_hw_adc =  (amhw_hc32_adc_t *)(p_dev->p_devinfo->adc_reg_base);
    ...
    if (p_dev->conv_cnt < p_desc->length) {
        ...
        /* 读取数据 */
        adc_dat = amhw_hc32_adc_data_get (p_hw_adc);
...
{
    am_hc32_adc_dev_t *p_dev = NULL;
    
    if (NULL == p_drv) {
        return -AM_EINVAL;
    }

    p_dev    = (am_hc32_adc_dev_t *)p_drv;

    /* 连接中断函数 */
    am_int_connect(p_dev->p_devinfo->inum,
                   __hc32_adc_irq_handle,
                   (void *)p_dev);
 ...
 */
static void __adc_int_work_startup (am_hc32_adc_dev_t       *p_dev,
                                    amhw_hc32_adc_chan_sel_t chan)
{
    amhw_hc32_adc_t *p_hw_adc = (amhw_hc32_adc_t *)
    ...
    if(p_dev->p_devinfo->p_ioinfo != NULL) {
        ...
        /* 链接ADC采样通道和引脚 */
        amhw_hc32_adc_sgl_chan_sel(p_hw_adc,
        ...
        if(p_dev->p_devinfo->p_ioinfo[chan].chan_sel <
            ...
            /* ADC输入信号放大器使能 */
            amhw_hc32_adc_inamplify_enable(p_hw_adc);

            if(p_dev->p_devinfo->p_ioinfo[chan].chan_sel ==
               AMHW_HC32_CHAN_INSIDE_TEMP_SENSOR) {
        ...
        /* 中断使能*/
        amhw_hc32_adc_int_enable(p_hw_adc);
                           ...
                           void                *p_arg)
{
    ...
    __adc_int_work_startup(p_dev,
                           (amhw_hc32_adc_chan_sel_t)chan);/* 中断工作模式启动配置 */
...
static int __fn_adc_stop (void *p_drv, int chan)
{
    am_hc32_adc_dev_t *p_dev    = (am_hc32_adc_dev_t *)p_drv;
    amhw_hc32_adc_t   *p_hw_adc = (amhw_hc32_adc_t *)
                                    (p_dev->p_devinfo->adc_reg_base);
    ...
    /* ADC中断禁能 */
    amhw_hc32_adc_int_disable(p_hw_adc);
                              ...
                              uint32_t      *p_rate)
{
    ...
    i = amhw_hc32_adc_sample_period_get(p_hw_adc);

    sample = sample_time[i];

    adc_clkdiv = (0x1ul << amhw_hc32_adc_pclk_div_get(p_hw_adc));
                              ...
                              uint32_t       rate)
{
    ...
    /* 设置转换周期 */
    amhw_hc32_adc_sample_period_sel(
        p_hw_adc, 
        (amhw_hc32_adc_sample_period_t)adc_rate_err.sample_time);

    /* 设置ADC时钟分频 */
    amhw_hc32_adc_pclk_div_sel(p_hw_adc, 
                                 (amhw_hc32_adc_pclk_div_t)adc_rate_err.adc_div);
                                    ...
                                    const am_hc32_adc_devinfo_t *p_devinfo)
{
    ...
    if(p_devinfo->vref_choice == AMHW_HC32_ADC_REFVOL_OUTSIDE_PB1) {

        am_gpio_pin_cfg(PIOB_1, PIOB_1_GPIO | PIOB_1_AIN);

        /* 内部参考电压禁能 */
        amhw_hc32_adc_inrefvol_disable(p_hw_adc);

        /* 设置参考电压源 */
        amhw_hc32_adc_refvol_sel(p_hw_adc, p_devinfo->vref_choice);
    ...
    /* 使能单次转换模式 */
    amhw_hc32_adc_mode_sel(p_hw_adc, AMHW_HC32_ADC_MODE_SINGLE);
...
{
    am_hc32_adc_dev_t *p_dev    = (am_hc32_adc_dev_t *)handle;
    amhw_hc32_adc_t   *p_hw_adc = NULL;

...
/* 设备结构体指针 */
static
am_hc32_lptim_timing_dev_t *__gp_dev[PUBLIC_INUM_DEV_NUM] = {NULL, NULL};
...
{
    amhw_hc32_lptim_t *p_hw_tim = NULL;

    if(__gp_dev[0] != NULL) {

        p_hw_tim = (amhw_hc32_lptim_t *)__gp_dev[0]->p_devinfo->lptim_regbase;
        if (amhw_hc32_lptim_int_flag_check(p_hw_tim) == AM_TRUE ) {
           ...
           /* 清除溢出标志 */
           amhw_hc32_lptim_int_flag_clr(p_hw_tim);
       }
    }

    if(__gp_dev[1] != NULL) {

        p_hw_tim = (amhw_hc32_lptim_t *)__gp_dev[1]->p_devinfo->lptim_regbase;
        if (amhw_hc32_lptim_int_flag_check(p_hw_tim) == AM_TRUE ) {
...
static const am_timer_info_t * __lptim_timing_info_get (void *p_drv)
{
    am_hc32_lptim_timing_dev_t *p_dev = (am_hc32_lptim_timing_dev_t *)p_drv;

    if(p_dev->p_devinfo->lptim_regbase == HC32_LPTIM0_BASE) {
...
{
    am_hc32_lptim_timing_dev_t *p_dev = (am_hc32_lptim_timing_dev_t *)p_drv;
    amhw_hc32_lptim_t          *p_hw_tim;

    if ((p_drv == NULL) || (chan != 0)) {
        return -AM_EINVAL;
    }

    p_hw_tim = (amhw_hc32_lptim_t *)p_dev->p_devinfo->lptim_regbase;

    if ((prescale != 0) && (prescale <= 256)) {

        if(prescale == 1) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim, AMHW_HC32_LPTIM_CLK_DIV1);
        } else if(prescale == 2) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim, AMHW_HC32_LPTIM_CLK_DIV2);
        } else if(prescale == 4) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim, AMHW_HC32_LPTIM_CLK_DIV4);
        } else if(prescale == 8) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim, AMHW_HC32_LPTIM_CLK_DIV8);
        } else if(prescale == 16) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim,
                                          AMHW_HC32_LPTIM_CLK_DIV16);
        } else if(prescale == 32) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim,
                                          AMHW_HC32_LPTIM_CLK_DIV32);
        } else if(prescale == 64) {
            amhw_hc32_lptim_clk_div_set(p_hw_tim,
                                          AMHW_HC32_LPTIM_CLK_DIV64);
        } else {
            amhw_hc32_lptim_clk_div_set(p_hw_tim,
                                          AMHW_HC32_LPTIM_CLK_DIV256);
        }
...
static int __lptim_timing_prescale_get (void     *p_drv,
                                        ...
                                        uint32_t *p_prescale)
{
    amhw_hc32_lptim_clk_div_t   div   = AMHW_HC32_LPTIM_CLK_DIV1;
    am_hc32_lptim_timing_dev_t *p_dev = (am_hc32_lptim_timing_dev_t *)p_drv;
    amhw_hc32_lptim_t          *p_hw_tim;
    ...
    /* 获取分频值 */
    div = amhw_hc32_lptim_clk_div_get(p_hw_tim);

    switch(div) {
    case AMHW_HC32_LPTIM_CLK_DIV1:
        *p_prescale = 1;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV2:
        *p_prescale = 2;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV4:
        *p_prescale = 4;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV8:
        *p_prescale = 8;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV16:
        *p_prescale = 16;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV32:
        *p_prescale = 32;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV64:
        *p_prescale = 64;
        break;
    case AMHW_HC32_LPTIM_CLK_DIV256:
        *p_prescale = 256;
        break;
    default:
        break;
...
static int __lptim_timing_count_get (void *p_drv, uint8_t chan, void *p_count)
{
    ...
    *(uint32_t *)p_count = amhw_hc32_lptim_cnt16_count_get(p_hw_tim);
...
static int __lptim_timing_rollover_get (void *p_drv,uint8_t chan,void *p_rollover)
{
    ...
    *(uint32_t *)p_rollover = 0xffff - amhw_hc32_lptim_arr_count_get(p_hw_tim);
...
static int __lptim_timing_disable (void *p_drv, uint8_t chan)
{
    ...
    /* 禁能中断 */
    amhw_hc32_lptim_int_disable(p_hw_tim);

    /* 禁能定时器 */
    amhw_hc32_lptim_disable(p_hw_tim);
...
static int __lptim_timing_enable (void *p_drv, uint8_t chan, void *p_count)
{
    ...
    /* 等待同步完成 */
    while(amhw_hc32_lptim_write_into_arr_stat(p_hw_tim) == AM_FALSE);

    amhw_hc32_lptim_arr_count_set(p_hw_tim, 65535 - (*(uint16_t *)p_count));

    /* 是否开启门控 */
    if(p_dev->p_devinfo->gate_enable == AM_TRUE) {
        amhw_hc32_lptim_gate_enable(p_hw_tim);
    } else {
        amhw_hc32_lptim_gate_disable(p_hw_tim);
    }

    /* 门控有效信号设置 */
    if(p_dev->p_devinfo->gate_state == AM_TRUE) {
        amhw_hc32_lptim_gate_high(p_hw_tim);
    } else {
        amhw_hc32_lptim_gate_low(p_hw_tim);
    }

    /* TOG、TOGN信号输出设置 */
    if(p_dev->p_devinfo->tog_state == AM_TRUE) {
        amhw_hc32_lptim_tog_enable(p_hw_tim);
    } else {
        amhw_hc32_lptim_tog_disable(p_hw_tim);
    }
    ...
    if (p_dev->pfn_callback != NULL) {
        ...
        /* 中断使能 */
        amhw_hc32_lptim_int_enable(p_hw_tim);
    }

    /* 使能定时器LPTIM允许计数 */
    amhw_hc32_lptim_enable(p_hw_tim);
...
am_timer_handle_t am_hc32_lptim_timing_init (
    am_hc32_lptim_timing_dev_t             *p_dev,
    const am_hc32_lptim_timing_devinfo_t   *p_devinfo)
{
    ...
    /* 计数/计时功能选择 */
    amhw_hc32_lptim_function_sel(p_hw_tim, p_devinfo->fun_sel);
    ...
    if(p_devinfo->mode == AMHW_HC32_LPTIM_MODE_RELOAD) {

        amhw_hc32_lptim_mode_sel(p_hw_tim, AMHW_HC32_LPTIM_MODE_RELOAD);
        ...
        amhw_hc32_lptim_mode_sel(p_hw_tim, AMHW_HC32_LPTIM_MODE_NORELOAD);
    ...
    /* 时钟源设置 */
    amhw_hc32_lptim_clk_src_sel(p_hw_tim, p_dev->p_devinfo->clk_src);

    if(p_dev->p_devinfo->clk_src == AMHW_HC32_LPTIM_CLK_SRC_XTL) {
    ...
    } else if(p_dev->p_devinfo->clk_src == AMHW_HC32_LPTIM_CLK_SRC_RCL) {
...
void am_hc32_lptim_timing_deinit (am_timer_handle_t handle)
{
    ...
    am_hc32_lptim_timing_dev_t *p_dev    = (am_hc32_lptim_timing_dev_t *)handle;
    amhw_hc32_lptim_t          *p_hw_tim = NULL;

...
static int __i2c_slv_setup (void *p_drv, am_i2c_slv_device_t *p_i2c_slv_dev)
{
    ...
    /* 关闭I2C控制器，配置参数 */
    amhw_hc32_i2c_disable (p_hw_i2c);

    /* 判断是否 需要响应 广播地址 */
    if ( p_i2c_slv_dev->dev_flags & AM_I2C_SLV_GEN_CALL_ACK) {
        amhw_hc32_i2c_gen_call_ack(p_hw_i2c);
    } else {
        amhw_hc32_i2c_gen_call_nack(p_hw_i2c);
    }
     ...
     */
    amhw_hc32_i2c_slave_addr_set (p_hw_i2c, p_i2c_slv_dev->dev_addr);
...
static void __i2c_slv_timing_callback (void *p_arg)
{
    ...
    amhw_hc32_i2c_slave_addr_set(p_hw_i2c, p_dev->p_i2c_slv_dev[0]->dev_addr);

...
static int __crc_init (void *p_cookie, am_crc_pattern_t *p_pattern)
{
    am_hc32_crc_dev_t *p_dev      = (am_hc32_crc_dev_t *)p_cookie;
    amhw_hc32_crc_t   *p_hw_crc   = NULL;
    ...
    p_hw_crc = (amhw_hc32_crc_t *)(p_dev->p_devinfo->crc_reg_base);

    /* 根据模型选择校验方式（crc-16 或  crc-32） */
    if (p_dev->p_pattern->width == 32){

        amhw_hc32_crc_cal_way(p_hw_crc, AM_HC32_CRC_32);

        /* 初始化结果寄存器值 */
        amhw_hc32_crc_res_32data_init(p_hw_crc);
    }else{

        amhw_hc32_crc_cal_way(p_hw_crc, AM_HC32_CRC_16);

        /* 初始化结果寄存器值 */
        amhw_hc32_crc_res_16data_init(p_hw_crc);
    }
...
static int __crc_cal (void *p_cookie, const uint8_t *p_data, uint32_t nbytes)
{
    ...
    uint32_t tdata;
    am_hc32_crc_dev_t *p_dev    = (am_hc32_crc_dev_t *)p_cookie;
    amhw_hc32_crc_t   *p_hw_crc =
        (amhw_hc32_crc_t *)(p_dev->p_devinfo->crc_reg_base);
    ...
    for (i = 0; i < nbytes; i+=4) {
        ...
        amhw_hc32_crc_32bit_write_data(p_hw_crc, tdata);
    }
...
static int __crc_final (void *p_cookie, uint32_t *p_value)
{
    ...
    if (p_dev->p_pattern->width == 32){
        ...
        *p_value = amhw_hc32_crc_32bit_read_data(p_hw_crc);
    }else {

        *p_value = amhw_hc32_crc_16bit_read_data(p_hw_crc);
    }
...
{
    am_hc32_crc_dev_t *p_dev    = (am_hc32_crc_dev_t *)handle;

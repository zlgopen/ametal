...
am_local const uint32_t __g_hc32_flash_pcgtimer_4mhz[] = \
                                    ...
                                    uint8_t             data_size)
{
    ...
    __hc32_flash_unlocksector(p_hw_flash, addr);
    ...
    __hc32_flash_locksector(p_hw_flash, addr);
...
{
    AM_HC32_FLASH_BYPASS(p_hw_flash);
    amhw_hc32_flash_read_waittime_set(p_hw_flash, waitcycle);
                                    ...
                                    uint32_t           addr)
{
    ...
    __hc32_flash_unlockall(p_hw_flash);
    ...
    *((volatile uint8_t*)addr) = 0;
    ...
    __hc32_flash_lockall(p_hw_flash);
...
int32_t am_hc32_flash_all_sector_erase (amhw_hc32_flash_t *p_hw_flash)
{
    ...
    /* set op */
    timeout = AM_HC32_FLASH_TIMEOUT_ERASE;
    while(AMHW_HC32_FLASH_CHIP_ERASE != amhw_hc32_flash_opt_get(p_hw_flash)) {
        if(timeout--) {
            amhw_hc32_flash_opt_set(p_hw_flash, AMHW_HC32_FLASH_CHIP_ERASE);
        } else {
            return -AM_EAGAIN;
        }
    }

                                     ...
                                     uint32_t            nbytes)
{
    ...
    while (len--) {
         ...
         amhw_hc32_lpuart_data_write(p_hw_lpuart, (uint8_t)(*p_txbuf++));
    }
                                   ...
                                   uint32_t            baud)
{
    ...
    uint8_t                        clk_div  = 0;
    uint32_t                       val_set  = 0;
    amhw_hc32_lpuart_work_mode_t mode     = AMHW_HC32_LPUART_WORK_MODE_0;
    ...
    /* 获取时钟模式 */
    mode = amhw_hc32_lpuart_mode_get(p_hw_lpuart);

    /* 模式0情况下，波特率固定为 PCLK/12 */
    if(mode == AMHW_HC32_LPUART_WORK_MODE_0) {

    	baud = (uint32_t)((float)lpuart_clk / 12.0);

    /* 模式2情况下，波特率 = 时钟 / 分频系数 */
    } else if (mode == AMHW_HC32_LPUART_WORK_MODE_2) {

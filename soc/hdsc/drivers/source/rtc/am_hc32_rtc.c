...
am_local int __rtc_time_get (void *p_drv, am_tm_t *p_tm)
{
    ...
    amhw_hc32_rtc_t   *p_hw_rtc = (amhw_hc32_rtc_t  *)p_dev->p_devinfo->rtc_regbase;
    ...
    p_tm->tm_sec  = amhw_hc32_rtc_sec_get(p_hw_rtc);
    p_tm->tm_min  = amhw_hc32_rtc_min_get(p_hw_rtc);
    p_tm->tm_hour = amhw_hc32_rtc_hour_get(p_hw_rtc);
    p_tm->tm_mday = amhw_hc32_rtc_day_get(p_hw_rtc);
    p_tm->tm_wday = amhw_hc32_rtc_week_get(p_hw_rtc);
    p_tm->tm_mon  = amhw_hc32_rtc_mon_get(p_hw_rtc);
    p_tm->tm_year = amhw_hc32_rtc_year_get(p_hw_rtc);
...
am_local int __rtc_time_set (void *p_drv, am_tm_t *p_tm)
{
    ...
    amhw_hc32_rtc_sec_set(p_hw_rtc, p_tm->tm_sec);
    amhw_hc32_rtc_min_set(p_hw_rtc, p_tm->tm_min);
    amhw_hc32_rtc_hour_set(p_hw_rtc, p_tm->tm_hour);
    amhw_hc32_rtc_day_set(p_hw_rtc, p_tm->tm_mday);
    amhw_hc32_rtc_week_set(p_hw_rtc, p_tm->tm_wday);
    amhw_hc32_rtc_mon_set(p_hw_rtc, p_tm->tm_mon);
    amhw_hc32_rtc_year_set(p_hw_rtc, p_tm->tm_year);
...
am_local void __rtc_sec_isr (void *p_arg)
{
    ...
    p_hw_rtc =(amhw_hc32_rtc_t *)p_dev->p_devinfo->rtc_regbase;
 ...
 * \brief 获取 RTC 计数器值
 */
am_err_t am_hc32_rtc_cnt_get (am_rtc_handle_t handle,
                                uint8_t        *sec,
                                uint8_t        *min,
                                uint8_t        *hour,
                                uint8_t        *mday,
                                ...
                                uint8_t        *year)
{
    ...
    p_dev    = (am_hc32_rtc_dev_t *)handle->p_drv;
    p_hw_rtc =(amhw_hc32_rtc_t *)p_dev->p_devinfo->rtc_regbase;
    ...
    *sec  = amhw_hc32_rtc_sec_get(p_hw_rtc);
    *min  = amhw_hc32_rtc_min_get(p_hw_rtc);
    *hour = amhw_hc32_rtc_hour_get(p_hw_rtc);
    *mday = amhw_hc32_rtc_day_get(p_hw_rtc);
    *wday = amhw_hc32_rtc_week_get(p_hw_rtc);
    *mon  = amhw_hc32_rtc_mon_get(p_hw_rtc);
    *year = amhw_hc32_rtc_year_get(p_hw_rtc);
 ...
 * \brief 设置 RTC 计数器值
 */
am_err_t am_hc32_rtc_cnt_set (am_rtc_handle_t handle,
                                uint8_t         sec,
                                uint8_t         min,
                                uint8_t         hour,
                                uint8_t         mday,
                                ...
                                uint8_t         year)
{
    ...
    amhw_hc32_rtc_sec_set(p_hw_rtc, sec);
    amhw_hc32_rtc_min_set(p_hw_rtc, min);
    amhw_hc32_rtc_hour_set(p_hw_rtc, hour);
    amhw_hc32_rtc_day_set(p_hw_rtc, mday);
    amhw_hc32_rtc_week_set(p_hw_rtc, wday);
    amhw_hc32_rtc_mon_set(p_hw_rtc, mon);
    amhw_hc32_rtc_year_set(p_hw_rtc, year);
 ...
 * \brief 设置 RTC 中断回调
 */
am_err_t am_hc32_rtc_callback_set (am_rtc_handle_t handle,
                                     uint8_t         type,
                                     ...
                                     void           *p_arg)
{
    ...
    if (AM_HC32_RTC_CALLBACK_PERIOD == type) {
        p_dev->pfn_callback[0] = pfn_callback;
        p_dev->p_arg[0] = p_arg;
    } else if (AM_HC32_RTC_CALLBACK_ALARM == type) {
 ...
 * \brief RTC周期中断使能
 */
am_err_t am_hc32_rtc_period_int_enable(
              am_rtc_handle_t                   handle,
              amhw_hc32_rtc_period_int_type_t type,
              amhw_hc32_rtc_period_int_time_t set_s,
              float                             set_c)
{
    am_hc32_rtc_dev_t *p_dev    = NULL;
    amhw_hc32_rtc_t   *p_hw_rtc = NULL;
    ...
    /* 开启周期中断 */
    if (AMHW_HC32_RTC_PERIOD_INT_TYPE_CUSTOM == type) {

        /* 启用自定义周期时间设定*/
        amhw_hc32_rtc_period_int_time_set_enable(p_hw_rtc, AM_TRUE);

        /* 设定时间 */
        amhw_hc32_rtc_period_int_time_set(p_hw_rtc, set_c);
    } else {
        ...
        /* 设定时间 */
        amhw_hc32_rtc_period_int_time_sel(p_hw_rtc, set_s);
    }
    ...
    p_dev->int_state |= AM_HC32_RTC_CALLBACK_PERIOD;
 ...
 * \brief RTC闹钟中断使能
 */
am_err_t am_hc32_rtc_alarm_clock_int_enable(am_rtc_handle_t handle,
                                              uint8_t         sec,
                                              uint8_t         min,
                                              uint8_t         hour,
                                              uint8_t         wday)
{
    ...
    /* 闹钟禁能 */
    amhw_hc32_rtc_alarm_clock_enable(p_hw_rtc, AM_FALSE);

    /* 闹钟中断使能 */
    amhw_hc32_rtc_alarm_clock_int_enable(p_hw_rtc, AM_TRUE);

    /* 设置闹钟秒、时、分、时、周几 */
    amhw_hc32_rtc_sec_alarm_clock_set(p_hw_rtc, sec);
    amhw_hc32_rtc_min_alarm_clock_set(p_hw_rtc, min);
    amhw_hc32_rtc_hour_alarm_clock_set(p_hw_rtc, hour);
    amhw_hc32_rtc_week_alarm_clock_set(p_hw_rtc, wday);
                                    ...
                                    uint8_t         type)
{
    ...
    if(type == AM_HC32_RTC_CALLBACK_ALARM) {
        /* 闹钟禁能 */
        amhw_hc32_rtc_alarm_clock_enable(p_hw_rtc, AM_FALSE);

        /* 闹钟中断禁能*/
        amhw_hc32_rtc_alarm_clock_int_enable(p_hw_rtc, AM_FALSE);

        p_dev->int_state &= ~AM_HC32_RTC_CALLBACK_ALARM;

    } else if(type == AM_HC32_RTC_CALLBACK_PERIOD) {
        /* 禁用自定义周期时间设定,启用系统的时间选项 */
        amhw_hc32_rtc_period_int_time_set_enable(p_hw_rtc, AM_FALSE);

        /* 设定时间 */
        amhw_hc32_rtc_period_int_time_sel(p_hw_rtc,
...
{
    amhw_hc32_rtc_t   *p_hw_rtc  = NULL;
    am_hc32_clk_dev_t *p_clk_dev = NULL;

    if ((NULL == p_dev) || (NULL == p_devinfo)) {
        return NULL;
    }

    p_hw_rtc                = (amhw_hc32_rtc_t *)p_devinfo->rtc_regbase;
    ...
    case AMHW_HC32_RTC_CLK_SRC_XTL_32768Hz:
        /* 外部晶振驱动能力选择 */
    ...
    case AMHW_HC32_RTC_CLK_SRC_RCL_32KHz:

        /* 设置RCL时钟频率校准值 （38.4KHz）*/
        amhw_hc32_rcc_rcl_trim_set(*((uint16_t*)AMHW_HC32_RCl_FRE_32768Hz));
    ...
    case AMHW_HC32_RTC_CLK_SRC_XTH_4MHz_DIV128:
        p_clk_dev = (am_hc32_clk_dev_t *)am_hc32_clk_devinfo_get();
    ...
    case AMHW_HC32_RTC_CLK_SRC_XTH_8MHz_DIV256:
        p_clk_dev = (am_hc32_clk_dev_t *)am_hc32_clk_devinfo_get();
        if(p_clk_dev->p_devinfo->xth_osc != 4000000) {
            return NULL;
        }
    ...
    case AMHW_HC32_RTC_CLK_SRC_XTH_16MHz_DIV512:
        p_clk_dev = (am_hc32_clk_dev_t *)am_hc32_clk_devinfo_get();
        if(p_clk_dev->p_devinfo->xth_osc != 4000000) {
            return NULL;
        }
    ...
    case AMHW_HC32_RTC_CLK_SRC_XTH_32MHz_DIV1024:
    ...
    /* RTC 时钟源选择 */
    amhw_hc32_rtc_clk_src_sel(p_hw_rtc, p_devinfo->rtc_clk_sour);
    ...
    if(p_devinfo->ctrl_1hz_out.enable == AM_TRUE) {
        ...
        amhw_hc32_rtc_1hz_out_enable(p_hw_rtc, AM_TRUE);

        /* 不处于范围内，直接关闭补偿功能 */
        if(p_devinfo->ctrl_1hz_out.ppm < (-274.6) ||
           p_devinfo->ctrl_1hz_out.ppm > (212.6)) {
            amhw_hc32_rtc_compensate_enable(p_hw_rtc, AM_FALSE);
        ...
        /* 处于补偿范围内 */
            ...
            if(p_devinfo->ctrl_1hz_out.adjust_enable == AM_TRUE) {
                amhw_hc32_rtc_1hz_high_enable(p_hw_rtc, AM_TRUE);
                amhw_hc32_rcc_rtcclk_adjust_set(
                    p_devinfo->ctrl_1hz_out.adjust_clk);
            } else {
                amhw_hc32_rtc_1hz_high_enable(p_hw_rtc, AM_FALSE);
            }

            /* 使能补偿，设定补偿值 */
            amhw_hc32_rtc_compensate_set(p_hw_rtc,
                                           p_devinfo->ctrl_1hz_out.ppm);
            amhw_hc32_rtc_compensate_enable(p_hw_rtc, AM_TRUE);
        }

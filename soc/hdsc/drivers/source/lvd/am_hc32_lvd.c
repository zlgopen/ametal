...
void __lvd_irq_handler (void *p_cookie)
{
    ...
    am_hc32_lvd_dev_t *p_dev = (am_hc32_lvd_dev_t *)p_cookie;
    p_hw_lvd = (amhw_hc32_lvd_t *)(p_dev->p_devinfo->reg_base);

    if (amhw_hc32_int_status_check (p_hw_lvd)){
...
int am_hc32_lvd_enable (am_hc32_lvd_handle_t      handle)
{
    ...
    p_hw_lvd = (amhw_hc32_lvd_t *)handle->p_devinfo->reg_base;
                                       ...
                                       const am_hc32_lvd_devinfo_t *p_devinfo)
{
    ...
    /* cr寄存器清零 */
    amhw_hc32_lvd_cr_clr (p_hw_lvd);

    /* 触发条件使能 */
    amhw_hc32_lvd_tri_enable (p_hw_lvd, p_dev->p_devinfo->tri_type);

    /* 数字滤波时间选择 */
    amhw_hc32_lvd_deb_time_sel (p_hw_lvd, p_dev->p_devinfo->deb_time);

    /* 使能数字滤波 */
    amhw_hc32_lvd_deb_time_enable (p_hw_lvd);

    /* 阈值电压选择 */
    amhw_hc32_lvd_vids_sel (p_hw_lvd, p_dev->p_devinfo->vids);

    /* 监测来源选择 */
    amhw_hc32_lvd_src_sel (p_hw_lvd, p_dev->p_devinfo->src_sel);
    ...
    if (p_devinfo->tri_act == AMHW_HC32_LVD_TRI_ACT_NVIC_INT){
        ...
        /* 使能中断 */
        amhw_hc32_lvd_int_enable (p_hw_lvd);
...
void am_hc32_lvd_deinit (am_hc32_lvd_handle_t     handle)
{
    ...
    amhw_hc32_lvd_t *p_hw_lvd =
                          (amhw_hc32_lvd_t *)(p_dev->p_devinfo->reg_base);
    ...
    /* 禁能LVD */
    amhw_hc32_lvd_disable (p_hw_lvd);

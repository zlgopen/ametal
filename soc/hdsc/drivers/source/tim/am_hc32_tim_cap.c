...
/** \brief 禁能捕获通道 */
static int __hc32_tim_cap_disable (void *p_drv, int chan);
...
static const struct am_cap_drv_funcs __g_tim_cap_drv_funcs = {
    __hc32_tim_cap_config,
    __hc32_tim_cap_enable,
    __hc32_tim_cap_disable,
    __hc32_tim_cap_reset,
    __hc32_tim_cap_count_to_time,
};
                                  ...
                                  void              *p_arg)
{
    ...
    amhw_hc32_tim_filter_type_t filter_type = AMHW_HC32_TIM_FLITER_TYPE_FLTA0;
    ...
    /* 分频系数设置 */
    amhw_hc32_tim_mode_clkdiv_set(p_hw_tim, p_dev->p_devinfo->clk_div);

    filter_type = (amhw_hc32_tim_filter_type_t)(chan * 4);
...
static int __hc32_tim_cap_enable (void *p_drv, int chan)
{
    ...
    am_hc32_tim_cap_dev_t    *p_dev    = (am_hc32_tim_cap_dev_t *)p_drv;
    amhw_hc32_tim_t          *p_hw_tim = (amhw_hc32_tim_t *)p_dev->p_devinfo->tim_regbase;
    am_hc32_tim_cap_ioinfo_t *p_ioinfo = p_dev->p_devinfo->p_ioinfo;
    ...
    /* 中断连接并使能 */
    am_int_connect(p_dev->p_devinfo->inum, __hc32_tim_cap_irq_handler, (void *)p_dev);
...
static int __hc32_tim_cap_disable (void *p_drv, int chan)
{
    ...
    /* 断开NVIC中断回调函数 */
    am_int_disconnect(p_dev->p_devinfo->inum, __hc32_tim_cap_irq_handler, (void *)p_dev);
                                         ...
                                         unsigned int *p_time_ns)
{
    ...
    reg_pre = (uint16_t)amhw_hc32_tim_mode_clkdiv_get(p_hw_tim);
...
static void __hc32_tim_cap_irq_handler (void *p_arg)
{
    am_hc32_tim_cap_dev_t *p_dev     = (am_hc32_tim_cap_dev_t *)p_arg;
    amhw_hc32_tim_t       *p_hw_tim  = (amhw_hc32_tim_t *)p_dev->p_devinfo->tim_regbase;
    ...
    for(i = 0; i < p_dev->chan_max;i = i + 2) {
        if ((amhw_hc32_tim_mode23_int_flag_check(p_hw_tim, (i / 2) + 2)) == AM_TRUE) {
    ...
    for(i = 1; i < p_dev->chan_max;i = i + 2) {
        if ((amhw_hc32_tim_mode23_int_flag_check(p_hw_tim, (i / 2) + 5)) == AM_TRUE) {
                          ...
                          amhw_hc32_tim_type_t   type)
{
    ...
    for(i = 0; i < p_dev->chan_max;i = i + 2) {
        amhw_hc32_tim_mode23_csa_cap(p_hw_tim, i);
    }

    /* 设置通道为捕获模式 （CH0B、CH1B、CH2B */
    for(i = 1; i < p_dev->chan_max;i = i + 2) {
        amhw_hc32_tim_mode23_csb_cap(p_hw_tim, i);
    }
                                      ...
                                      const am_hc32_tim_cap_devinfo_t *p_devinfo)
{
    ...
    /* 捕获初始化 */
    __hc32_tim_cap_init(p_hw_tim, p_dev, p_devinfo->tim_type);

    return &(p_dev->cap_serv);
}

void am_hc32_tim_cap_deinit (am_cap_handle_t handle)
{
    uint8_t i = 0;

    am_hc32_tim_cap_dev_t *p_dev    = (am_hc32_tim_cap_dev_t *)handle;
    amhw_hc32_tim_t       *p_hw_tim = NULL;

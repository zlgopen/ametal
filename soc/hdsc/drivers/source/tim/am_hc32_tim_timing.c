...
{
    am_hc32_tim_timing_dev_t *p_dev    = (am_hc32_tim_timing_dev_t *)p_arg;
    amhw_hc32_tim_t          *p_hw_tim = (amhw_hc32_tim_t *)
...
{
    am_hc32_tim_timing_dev_t *p_dev = (am_hc32_tim_timing_dev_t *)p_drv;
                                      ...
                                      uint32_t *p_prescale)
{
    ...
    /* 获取分频值 */
    div = (uint16_t)amhw_hc32_tim_mode_clkdiv_get(p_hw_tim);
...
static int __tim_timing_count_get (void *p_drv, uint8_t chan, void *p_count)
{
    ...
    p_hw_tim = (amhw_hc32_tim_t *)p_dev->p_devinfo->tim_regbase;
    if(p_dev->p_devinfo->counter_type == AMHW_HC32_TIM_MODE0_COUNTER_32) {
        *(uint32_t *)p_count = amhw_hc32_tim_cnt32_count_get(p_hw_tim);
    } else {
        *(uint32_t *)p_count = amhw_hc32_tim_cnt16_count_get(p_hw_tim);
    }
...
static int __tim_timing_rollover_get (void *p_drv,uint8_t chan,void *p_rollover)
{
    ...
    if(p_dev->p_devinfo->counter_type == AMHW_HC32_TIM_MODE0_COUNTER_32) {
        ...
        *(uint32_t *)p_rollover = 0xffff - amhw_hc32_tim_arr_count_get(p_hw_tim);
    }
...
static int __tim_timing_disable (void *p_drv, uint8_t chan)
{
    ...
    /* 禁能更新中断 */
    amhw_hc32_tim_mode0_int_update_disable(p_hw_tim);
...
static int __tim_timing_enable (void *p_drv, uint8_t chan, void *p_count)
{
    ...
    if(p_dev->p_devinfo->counter_type == AMHW_HC32_TIM_MODE0_COUNTER_16) {

        amhw_hc32_tim_arr_count_set(p_hw_tim, 65535 - count);
    }

    /* 设置计数器初值 CNT */
    if(p_dev->p_devinfo->counter_type == AMHW_HC32_TIM_MODE0_COUNTER_16) {
        amhw_hc32_tim_cnt16_count_set(p_hw_tim, 65535 - count);
    } else {
        amhw_hc32_tim_cnt32_count_set(p_hw_tim, 0xffffffff - count);
    }

    /* 是否开启门控 */
    if(p_dev->p_devinfo->gate_enable == AMHW_HC32_TIM_GATE_ENABLE) {
        amhw_hc32_tim_mode0_gate_enable(p_hw_tim);
    } else {
        amhw_hc32_tim_mode0_gate_disable(p_hw_tim);
    }

    /* 门控有效信号设置 */
    if(p_dev->p_devinfo->gate_state == AM_TRUE) {
        amhw_hc32_tim_mode0_gate_high(p_hw_tim);
    } else {
        amhw_hc32_tim_mode0_gate_low(p_hw_tim);
    }

    /* 计数时钟设置 */
    if(p_dev->p_devinfo->clk_src == AMHW_HC32_TIM_CLK_SRC_TCLK) {
        amhw_hc32_tim_mode_clk_src_set(p_hw_tim,AMHW_HC32_TIM_CLK_SRC_TCLK);
    } else {
        amhw_hc32_tim_mode_clk_src_set(p_hw_tim,AMHW_HC32_TIM_CLK_SRC_ETR);
    }
...
am_timer_handle_t am_hc32_tim_timing_init (
    am_hc32_tim_timing_dev_t             *p_dev,
    const am_hc32_tim_timing_devinfo_t  *p_devinfo)
{
    ...
    if(p_devinfo->counter_type == AMHW_HC32_TIM_MODE0_COUNTER_32) {

        amhw_hc32_tim_mode0_counter_set(p_hw_tim,
                                          AMHW_HC32_TIM_MODE0_COUNTER_32);
...
/******************************************************************************/
void am_hc32_tim_timing_deinit (am_timer_handle_t handle)
{

    am_hc32_tim_timing_dev_t *p_dev    = (am_hc32_tim_timing_dev_t *)handle;
    amhw_hc32_tim_t          *p_hw_tim = NULL;

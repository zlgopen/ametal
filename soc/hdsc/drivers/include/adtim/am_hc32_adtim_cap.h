...
#ifndef __AM_HC32_ADTIM_CAP_H
#define __AM_HC32_ADTIM_CAP_H
...
/**
 * \addtogroup am_hc32_if_adtim_cap
 * \copydoc am_hc32_adtim_cap.h
 * @{
 */

#define  AM_HC32_ADTIM_CAP_CHA  0x00   /**< \brief CAP通道A标识符*/
#define  AM_HC32_ADTIM_CAP_CHB  0x01   /**< \brief CAP通道B标识符*/

/**
 * \brief TIM捕获功能相关的GPIO信息
 */
typedef struct am_hc32_adtim_cap_ioinfo {
...
} am_hc32_adtim_cap_ioinfo_t;

/**
 * \brief TIM捕获功能相关的设备信息
 */
typedef struct am_hc32_adtim_cap_devinfo {
    uint32_t                      tim_regbase;   /**< \brief TIM寄存器块基址 */
    uint8_t                       inum;          /**< \brief TIM对应的中断号 */
    amhw_hc32_adtim_pclk_div_t  clk_div;       /**< \brief 时钟分频系数 */
    uint8_t                       channels_num;  /**< \brief 使用的通道数，最大为2 */
    am_hc32_adtim_cap_ioinfo_t *p_ioinfo;      /**< \brief 指向捕获管脚信息结构体 */
...
} am_hc32_adtim_cap_devinfo_t;

/**
 * \brief TIM捕获功能设备
 */
typedef struct am_hc32_adtim_cap_dev {
 ...
 */
am_cap_handle_t am_hc32_adtim_cap_init (am_hc32_adtim_cap_dev_t              *p_dev,
                                        const am_hc32_adtim_cap_devinfo_t *p_devinfo);

/**
 * \brief 不使用TIM捕获功能时，解初始化TIM捕获功能，释放相关资源
 *
 * \param[in] handle : am_hc32_adtim_cap_init() 初始化函数获得的cap服务句柄
 *
 * \return 无
 */

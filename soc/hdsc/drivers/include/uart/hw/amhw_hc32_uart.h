...
#ifndef __AMHW_HC32_UART_H
#define __AMHW_HC32_UART_H
...
/**
 * \addtogroup amhw_hc32_if_uart
 * \copydoc amhw_hc32_uart.h
 * @{
 */
 ...
 */
am_static_inline
am_bool_t amhw_hc32_uart_data_8th_get (amhw_hc32_uart_t *p_hw_uart)
{
 ...
 */
am_static_inline
uint8_t amhw_hc32_uart_data_read (amhw_hc32_uart_t *p_hw_uart)
{
...
am_static_inline
void amhw_hc32_uart_single_line_half_disable (amhw_hc32_uart_t *p_hw_uart)
{
...
am_static_inline
void amhw_hc32_uart_single_line_half_enable (amhw_hc32_uart_t *p_hw_uart)
{
    p_hw_uart->scon |= (0x1ul << 22);
}


#define AMHW_HC32_UART_INT_FRAME_ERR   (1ul << 21)   /* 帧错误中断使能位 */
#define AMHW_HC32_UART_INT_CTS_TRIGGER (1ul << 20)   /* CTS翻转中断使能位 */
#define AMHW_HC32_UART_INT_PARITY_ERR  (1ul << 13)   /* 奇偶校验错误中断使能位 */
#define AMHW_HC32_UART_INT_TX_EMPTY    (1ul << 8)    /* TX空中断使能位  */
#define AMHW_HC32_UART_INT_TX_COMPLETE (1ul << 1)    /* 发送中断使能位 */
#define AMHW_HC32_UART_INT_RX_COMPLETE (1ul << 0)    /* 接收中断使能位 */
#define AMHW_HC32_UART_INT_ALL         0xFF          /* 所有中断标志 */

/**
 * \brief UART 相关中断  禁能
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] type      : 中断宏定义  AMHW_HC32_UART_INT_FRAME_ERR
 *                               。
 *                               。
 *                               。
 *                               AMHW_HC32_UART_INT_ALL
 *
 * \return 无
 */
...
#define AMHW_HC32_UART_DMA_RX       (1ul << 16) /* 接收DMA使能位 */
#define AMHW_HC32_UART_MUT_ADR_AUTO (1ul << 5)  /* 多机地址自动识别使能位 */
#define AMHW_HC32_UART_RX           (1ul << 4)  /* 接收使能
                                                   * Mode0:     0：发送; 1：接收
 ...
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] type      : 中断宏定义  AMHW_HC32_UART_CTS
 *                               。
 *                               。
 *                               。
 *                               AMHW_HC32_UART_RX
...
#define AMHW_HC32_UART_STOP_1_0_BIT   (0ul << 14)  /* 1个停止位 */
#define AMHW_HC32_UART_STOP_1_5_BIT   (1ul << 14)  /* 1.5个停止位 */
#define AMHW_HC32_UART_STOP_2_0_BIT   (2ul << 14)  /* 2个停止位 */

/**
 * \brief 停止位设置
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] stop_bit  : 停止位宏定义   AMHW_HC32_UART_STOP_1_0_BIT 或
 *                                 AMHW_HC32_UART_STOP_1_5_BIT 或
 *                                 AMHW_HC32_UART_STOP_2_0_BIT
 *
 * \return none
 */
am_static_inline
...
#define AMHW_HC32_UART_SOFT           (0ul << 2)  /* 软件读写SBUF[8] */
#define AMHW_HC32_UART_HW_PARITY_EVEN (1ul << 2)  /* 硬件偶校验 */
#define AMHW_HC32_UART_HW_PARITY_ODD  (2ul << 2)  /* 硬件奇校验 */
#define AMHW_HC32_UART_PARITY_NO      (3ul << 2)  /* 保留 */
/**
 * \brief UART parity bit select
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] parity    : 奇偶校验宏定义       AMHW_HC32_UART_SOFT              或
...
typedef enum {
    AMHW_HC32_UART_CLK_DIV_MODE0_NO  = 0,    /* 模式0，采样分频无效 */
    AMHW_HC32_UART_CLK_DIV_MODE13_16 = 0,    /* 模式1/3，16采样分频 */
    AMHW_HC32_UART_CLK_DIV_MODE13_8  = 1,    /* 模式1/3，8采样分频 */
    AMHW_HC32_UART_CLK_DIV_MODE2_32  = 0,    /* 模式2，32采样分频 */
    AMHW_HC32_UART_CLK_DIV_MODE2_16  = 1,    /* 模式2，16采样分频 */
}amhw_hc32_uart_clk_div_t;
/**
 * \brief UART 采样分频设置
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] parity    : 分频系数amhw_hc32_uart_clk_div_t枚举量
 *
 * \return none
 */
am_static_inline
 ...
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 *
 * \return 分频系数设定值（具体的分频值需要结合工作模式一起判定，如amhw_hc32_uart_clk_div_t枚举量）
 */
am_static_inline
uint8_t amhw_hc32_uart_clk_div_get (amhw_hc32_uart_t *p_hw_uart)
{
 ...
 */
am_static_inline
uint8_t amhw_hc32_uart_slaver_addr_get (amhw_hc32_uart_t *p_hw_uart)
{
 ...
 */
am_static_inline
uint8_t amhw_hc32_uart_slaver_addr_mask_get (amhw_hc32_uart_t *p_hw_uart)
{
    return (p_hw_uart->saden & 0xFF);
}

#define AMHW_HC32_UART_FLAG_CTS          (1ul << 6)   /* CTS高低电平标志位,硬件清零 */
#define AMHW_HC32_UART_FLAG_CTS_TRIGGER  (1ul << 5)   /* CTS翻转标志位,软件清0 */
#define AMHW_HC32_UART_FLAG_PARITY_ERR   (1ul << 4)   /* 奇偶校验错误标志位,软件清0 */
#define AMHW_HC32_UART_FLAG_TX_EMPTY     (1ul << 3)   /* 发送空标志位,硬件清零 */
 ...
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] flag      : 标志宏定义   AMHW_HC32_UART_FLAG_CTS          或
 *                                AMHW_HC32_UART_FLAG_CTS_TRIGGER  或
 *                                AMHW_HC32_UART_FLAG_PARITY_ERR   或
 *                                AMHW_HC32_UART_FLAG_TX_EMPTY     或
 *                                AMHW_HC32_UART_FLAG_FRAME_ERR    或
 *                                AMHW_HC32_UART_FLAG_TX_COMPLETE  或
 *                                AMHW_HC32_UART_FLAG_RX_COMPLETE  或
 *
 * \return AM_TRUE  : 标志有效
 *         AM_FALSE : 标志无效
 */
 ...
 *
 * \param[in] p_hw_uart : 指向UART寄存器结构体的指针
 * \param[in] flag      : 标志宏定义   AMHW_HC32_UART_FLAG_CTS_TRIGGER  或
 *                                AMHW_HC32_UART_FLAG_PARITY_ERR   或
 *                                AMHW_HC32_UART_FLAG_FRAME_ERR    或
 *                                AMHW_HC32_UART_FLAG_TX_COMPLETE  或
 *                                AMHW_HC32_UART_FLAG_RX_COMPLETE  或
 ...
 */
am_static_inline
uint32_t amhw_hc32_uart_flag_get(amhw_hc32_uart_t *p_hw_uart)
{

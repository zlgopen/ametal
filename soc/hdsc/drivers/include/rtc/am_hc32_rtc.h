...
#define  AM_HC32_RTC_CALLBACK_PERIOD  (1 << 0) /**< \brief RTC周期中断回调函数编号 */
#define  AM_HC32_RTC_CALLBACK_ALARM   (1 << 1) /**< \brief RTC闹钟中断回调函数编号 */

/**
 * \brief HC32 1Hz输出控制结构体
 */
typedef struct {
    ...
    am_bool_t adjust_enable;          /* 在ppm处于范围内时（即开启补偿的情况下），
                                       ...
                                       */
    amhw_hc32_rtcclk_adjust adjust_clk; /* 高速时钟补偿时钟选择 ，建议和PCLK频率相同  */
} am_hc32_rtc_1hz_out_t;
...
typedef struct am_hc32_rtc_devinfo {
    ...
    amhw_hc32_rtc_clk_src_t rtc_clk_sour;          /**< \brief RTC 时钟源 */
    int                       rtc_inum;              /**< \brief RTC 中断号 */
    am_hc32_rtc_1hz_out_t   ctrl_1hz_out;
...
typedef struct am_hc32_rtc_dev {
    ...
    const am_hc32_rtc_devinfo_t *p_devinfo;       /**< \brief RTC设备信息 */
}am_hc32_rtc_dev_t;
 ...
 *
 * \param[in] handle       RTC 标准服务操作句柄
 * \param[in] type         中断类型，可选 AM_HC32_RTC_CALLBACK_SECOND
 ...
 * \param[in] type   周期中断类型，见枚举定义amhw_hc32_rtc_period_int_type_t
 * \param[in] set_s  周期类型为系统定义时，所设置周期中断产生时间amhw_hc32_rtc_period_int_time_t
 ...
 *
 * \param[in] handle RTC 标准服务操作句柄
 * \param[in] type   中断类型，可选 AM_HC32_RTC_CALLBACK_PERIOD

 ...
 */
typedef struct {
    uint32_t chan;                           /**< \brief 对应的通道号 */
    amhw_hc32f460_adc_chan_sel_t chan_sel;   /**< \brief 对应的通道类型 */
    uint32_t gpio;                           /**< \brief 对应的GPIO管脚号 */
    uint32_t func;                           /**< \brief 对应的GPIO功能设置 */
    uint32_t dfunc;                          /**< \brief 对应的GPIO功能默认设置 */
    amhw_hc32f460_adc_chan_sel_t d_chan_sel;   /**< \brief 默认的通道类型 */
 ...
 * 默认去初始化序列A
 *
 * \param[in] handle : am_hc32f460_adc_init() 初始化函数获得的ADC服务句柄
 *
 * \return 无
 */

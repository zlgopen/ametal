...
typedef enum am_hc32_pcnt_polar {
    HC32_PCNT_S0PNOINVERT = (0u),   /* S0通道极性不取反 */
    HC32_PCNT_S0PINVERT   = (1u),   /* S0通道取反 */
    HC32_PCNT_S1PNOINVERT = (2u),   /* S1通道极性不取反 */
    HC32_PCNT_S1PINVERT   = (3u)    /* S1通道取反 */
...
typedef enum am_hc32_pcnt_mode {
    HC32_PCNT_SINGLE  = (1u), /* 单通道脉冲计数模式 */
    HC32_PCNT_SPECIAL = (2u), /* 双通道非正交脉冲计数模式 */
    HC32_PCNT_DOUBLE  = (3u)  /* 双通道正交脉冲计数模式 */
} am_hc32_pcnt_mode_t;

/* 脉冲计数模式选择枚举 */
typedef enum am_hc32_pcnt_dir {
    HC32_PCNT_UP   = (0u), /* 向上计数模式 */
    HC32_PCNT_DOWN = (1u), /* 向下计数模式 */
} am_hc32_pcnt_dir_t;

/* 中断标志枚举 */
typedef enum am_hc32_pcnt_int {
    HC32_PCNT_INT_S1E = (1u << 7), /* S1脉冲解码错误 */
    HC32_PCNT_INT_S0E = (1u << 6), /* S0脉冲解码错误 */
    HC32_PCNT_INT_BB  = (1u << 5), /* 中间无低状态 */
    HC32_PCNT_INT_FE  = (1u << 4), /* 不正确非交编码帧 */
...
typedef enum am_hc32_pcnt_cmd {
    HC32_PCNT_B2C = (4u), /* 立即将BUF中的值同步到CNT */
    HC32_PCNT_B2T = (2u), /* 立即将BUF中的值同步到TOP */
    HC32_PCNT_T2C = (1)   /* 立即将TOP中的值同步到CNT */
} am_hc32_pcnt_cmd_t;

/* 输出选项 */
typedef enum am_hc32_pcnt_dgb {
    HC32_PCNT_DGB_0   = (0),  /* 固定为0 */
    HC32_PCNT_DGB_SA  = (4u), /* 脉冲同步之后的输出（S0A/S1A） */
    HC32_PCNT_DGB_SP  = (2u), /* 脉冲极性选择之后的输出（S0P/S1P） */
    HC32_PCNT_DGB_SPF = (1)   /* 脉冲滤波之后的输出（S0PF/S1PF） */
} am_hc32_pcnt_dgb_t;

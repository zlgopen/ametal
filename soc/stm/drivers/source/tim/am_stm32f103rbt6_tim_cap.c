...
/** \brief 捕获参数配置 */
static int __stm32f103rbt6_tim_cap_config (void              *p_cookie,
                                 int                chan,
                                 unsigned int       flags,
...
/** \brief 使能捕获通道 */
static int __stm32f103rbt6_tim_cap_enable (void *p_drv, int chan);

/** \brief 禁能捕获通道 */
static int __stm32f103rbt6_tim_cap_disable (void *p_drv, int chan);

/** \brief 复位捕获通道计数值 */
static int __stm32f103rbt6_tim_cap_reset (void *p_drv, int chan);

static int __stm32f103rbt6_tim_cap_count_to_time (void         *p_drv,
                                        int           chan,
                                        unsigned int  count1,
                                        unsigned int  count2,
                                        unsigned int *p_time_ns);

static void __stm32f103rbt6_tim_cap_irq_handler (void *p_arg);

/** \brief 捕获服务驱动函数 */
static const struct am_cap_drv_funcs __g_tim_cap_drv_funcs = {
    __stm32f103rbt6_tim_cap_config,
    __stm32f103rbt6_tim_cap_enable,
    __stm32f103rbt6_tim_cap_disable,
    __stm32f103rbt6_tim_cap_reset,
    __stm32f103rbt6_tim_cap_count_to_time,
};

/******************************************************************************/

/** \brief 配置一个输入捕获通道 */
static int __stm32f103rbt6_tim_cap_config (void              *p_drv,
                                 int                chan,
                                 unsigned int       options,
                                 am_cap_callback_t  pfn_callback,
                                 void              *p_arg)
{
    am_stm32f103rbt6_tim_cap_dev_t *p_dev    = (am_stm32f103rbt6_tim_cap_dev_t *)p_drv;
    ...
    amhw_stm32f103rbt6_tim_icpsc_set(p_hw_tim, 0, chan);

    /* 选择输入端通道映射不反相 */
    amhw_stm32f103rbt6_tim_ccs_set(p_hw_tim, 1, chan);

    /* 设置输入滤波器的分频值为0(默认不使用滤波) */
    amhw_stm32f103rbt6_tim_icf_set(p_hw_tim, AMHW_STM32F103RBT6_TIM_ICF_FSAMPLING0, chan);

    /*  选择上升沿触发 */
    if ((options & AM_CAP_TRIGGER_RISE) == AM_CAP_TRIGGER_RISE) {
        amhw_stm32f103rbt6_tim_ccp_captrigger_set(p_hw_tim,
...
static int __stm32f103rbt6_tim_cap_enable (void *p_drv, int chan)
{
    am_stm32f103rbt6_tim_cap_dev_t    *p_dev    = (am_stm32f103rbt6_tim_cap_dev_t *)p_drv;
    amhw_stm32f103rbt6_tim_t          *p_hw_tim = (amhw_stm32f103rbt6_tim_t *)p_dev->p_devinfo->tim_regbase;
    am_stm32f103rbt6_tim_cap_ioinfo_t *p_ioinfo = p_dev->p_devinfo->p_ioinfo;
    ...
    /* 中断连接并使能 */
    am_int_connect(p_dev->p_devinfo->inum, __stm32f103rbt6_tim_cap_irq_handler, (void *)p_dev);
    am_int_enable(p_dev->p_devinfo->inum);

    /* 设置自动重装寄存器的值 */
    amhw_stm32f103rbt6_tim_arr_set(p_hw_tim, 0xffffffff);
    ...
    /* 允许捕获计数器的值到捕获寄存器中 */
    amhw_stm32f103rbt6_tim_cce_cap_enable(p_hw_tim, chan);
...
static int __stm32f103rbt6_tim_cap_disable (void *p_drv, int chan)
{
    ...
    /* 断开NVIC中断回调函数 */
    am_int_disconnect(p_dev->p_devinfo->inum, __stm32f103rbt6_tim_cap_irq_handler, (void *)p_dev);
    am_int_disable(p_dev->p_devinfo->inum);

    /* 禁能捕获中断 */
    amhw_stm32f103rbt6_tim_int_disable(p_hw_tim, (1UL << (chan + 1)));
                                        ...
                                        unsigned int *p_time_ns)
{
    ...
    reg_pre = (uint16_t)amhw_stm32f103rbt6_tim_prescale_get(p_hw_tim);
...
static void __stm32f103rbt6_tim_cap_irq_handler (void *p_arg)
{
    am_stm32f103rbt6_tim_cap_dev_t *p_dev     = (am_stm32f103rbt6_tim_cap_dev_t *)p_arg;
    amhw_stm32f103rbt6_tim_t       *p_hw_tim  = (amhw_stm32f103rbt6_tim_t *)p_dev->p_devinfo->tim_regbase;
    ...
    for (i = 1; i <= chans_num; i++) {
        if ((amhw_stm32f103rbt6_tim_status_flg_get(p_hw_tim, 1UL << i)) != 0) {

            callback_func = p_dev->callback_info[i - 1].callback_func;

            /* 得到对应通道的值 */
            value = amhw_stm32f103rbt6_tim_ccr_cap_val_get(p_hw_tim, i - 1);
...
/**
  * \brief 捕获初始化
  */
void __stm32f103rbt6_tim_cap_init (amhw_stm32f103rbt6_tim_t     *p_hw_tim,
...
am_cap_handle_t am_stm32f103rbt6_tim_cap_init (am_stm32f103rbt6_tim_cap_dev_t           *p_dev,
                                        const am_stm32f103rbt6_tim_cap_devinfo_t *p_devinfo)
{
    ...
    /* 捕获初始化 */
    __stm32f103rbt6_tim_cap_init(p_hw_tim, p_devinfo->tim_type);

    return &(p_dev->cap_serv);
}

void am_stm32f103rbt6_tim_cap_deinit (am_cap_handle_t handle)
{
    uint8_t i = 0;

    am_stm32f103rbt6_tim_cap_dev_t *p_dev    = (am_stm32f103rbt6_tim_cap_dev_t *)handle;
    amhw_stm32f103rbt6_tim_t       *p_hw_tim = NULL;

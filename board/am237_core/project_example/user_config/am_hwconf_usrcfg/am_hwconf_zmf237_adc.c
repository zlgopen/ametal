...
/**
 * \file
 * \brief ZMF237 ADC 用户配置文件
 * \sa am_hwconf_zmf237_adc.c
 *
 * \internal
...
/**
 * \addtogroup am_if_src_hwconf_zmf237_adc
 * \copydoc am_hwconf_zmf237_adc.c
 * @{
 */
...
am_adc_handle_t am_zmf237_adc1_inst_init (void)
{
    return am_zlg_adc_init(&__g_adc1_dev, &__g_adc1_devinfo);
}

/** \brief ADC实例解初始化 */
void am_zmf237_adc1_inst_deinit (am_adc_handle_t handle)
{
...
/** \brief ADC实例初始化，获得ADC标准服务句柄 */
am_adc_handle_t am_zmf237_adc2_inst_init (void)
{
    return am_zlg_adc_init(&__g_adc2_dev, &__g_adc2_devinfo);
}

/** \brief ADC实例解初始化 */
void am_zmf237_adc2_inst_deinit (am_adc_handle_t handle)
{
    am_zlg_adc_deinit(handle);
}

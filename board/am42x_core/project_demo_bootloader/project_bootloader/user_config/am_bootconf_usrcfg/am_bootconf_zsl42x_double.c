...
/**
 * \file
 * \brief ZSL42x bootloader 双区用户配置文件
 *
 * \internal
...
/**
 * \addtogroup am_if_src_bootconf_zsl42x_double
 * \copydoc am_bootconf_zsl42x_double.c
 * @{
 */

#include <am_bootconf_usrcfg/am_bootconf_zsl42x.h>

#include "am_zsl42x.h"
#include "am_zsl42x_inst_init.h"
#include "am_zsl42x_boot_flash.h"
 ...
 */
static am_zsl42x_boot_flash_devinfo_t __g_flash_devinfo = {
 ...
 */
static am_zsl42x_boot_flash_dev_t __g_flash_dev;
 ...
 */
static am_boot_flash_handle_t am_zsl42x_boot_double_flash_inst_init(void)
{
    return am_zsl42x_boot_flash_init(&__g_flash_dev, &__g_flash_devinfo);
}
...
am_boot_firmware_handle_t am_zsl42x_boot_double_firmware_flash_inst_init()
{
    flash_handle = am_zsl42x_boot_double_flash_inst_init();
...
am_boot_msg_handle_t am_zsl42x_boot_msg_flag_inst_init()
{
...
static void __zsl42x_boot_plfm_deinit()
{
    ...
    am_zsl42x_clk_inst_deinit();

    am_int_disable(INUM_UART0_2);
    am_clk_disable(CLK_UART0);

    am_zsl42x_clk_reset(CLK_UART0);
    am_zsl42x_clk_reset(CLK_GPIO);

    am_int_disable(INUM_TIM2);
    am_clk_disable(CLK_TIM012);
    am_zsl42x_clk_reset(CLK_TIM012);

    ZSL42x_RCCCTRL->peri_clken[0] = 0x80800000;
    ZSL42x_RCCCTRL->peri_clken[1] = 0;
 ...
 */
static am_arm_boot_devinfo_t __g_zsl42x_boot_double_devinfo = {
    /**< \brief 应用代码区起始地址*/
    0x00006800,
    /** \brief ram起始地址 */
    0x20000000,
    ...
    NULL,
    /** \brief 平台解初始化函数 */
    __zsl42x_boot_plfm_deinit,
};

int am_zsl42x_boot_double_inst_init()
{
    return am_arm_boot_init(&__g_zsl42x_boot_double_devinfo);
}

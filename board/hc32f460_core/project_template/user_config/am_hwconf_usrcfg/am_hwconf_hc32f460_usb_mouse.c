 ...
 ******************************************************************************/
/** \brief USB设备描述配置宏*/
#define __USBD_MOUSE_VENDOR_ID                 (0x4195) /**< \brief 厂商编号,作为产品发布必须写自己公司的厂商编号，以免侵权，此处填了一个没有在USB协会注册的编号*/
#define __USBD_MOUSE_PRODUCT_ID                (0x6515) /**< \brief 产品编号*/
#define __USBD_MOUSE_DEVICE_ID                 (0x0101) /**< \brief 设备编号*/
#define __USBD_MOUSE_VENDOR_STRING_INDEX       (0x01)   /**< \brief 描述厂商的字符串索引*/
#define __USBD_MOUSE_PRODUCT_STRING_INDEX      (0x02)   /**< \brief 描述产品的字符串索引 */
#define __USBD_MOUSE_DEVICE_STRING_INDEX       (0x03)   /**< \brief 描述设备序列号的字符串索引*/
#define __USBD_CONFIGURATION_COUNT           (1U)     /**< \brief 配置数，一般设置只有一个配置*/

/** \brief USB配置描述符配置宏*/
#define __USBD_MOUSE_INTERFACE_COUNT           (1U)               /**< \brief 接口数*/
#define __USBD_MOUSE_CONFIGURE_INDEX           (1U)               /**< \brief 配置描述符索引*/
#define __USBD_MOUSE_DEVICE_POWER               AM_USBD_MAX_POWER /**< \brief 设备向主机申请的电流，单位2mA,最大100mA*/
/** \brief 设备属性，自供电，不支持远程唤醒*/
#define __USBD_MOUSE_DEVICE_ATTRIBUTE          \
              (AM_USBD_CONFIG_SELF_POWER | AM_USBD_CONFIG_REMOTE_WAKEUP)

/**\brief USB端点描述符配置宏*/
#define __USBD_MOUSE_ENDPOINT_IN               (1U)                     /**< \brief 输入端点号*/
#define __USBD_MOUSE_ENDPOINT_IN_PACKSIZE      AM_USBD_MAX_EP_DATA_CNT  /**< \brief 端点包大小,设置为64*/
#define __USBD_MOUSE_ENDPOINT_IN_ATTRIBUTE     AM_USB_ENDPOINT_BULK     /**< \brief 设置端点属性为批量传输*/
#define __USBD_MOUSE_ENDPOINT_IN_QUERY_TIME    (0x01)                   /**< \brief 设置端点查询时间为10ms,单位为1ms*/
...
/**\brief USB接口描述符配置宏*/
#define __USBD_MOUSE_ENDPOINT_COUNT            (1U)     /**< \brief 端点个数,输入端点与输出端点(不包括控制端点)*/
...
static const uint8_t __g_usb_mouse_desc_dev[AM_USB_DESC_LENGTH_DEVICE]  = {
    ...
    __USBD_MOUSE_VENDOR_STRING_INDEX,    /**< \brief 描述厂商的字符串索引 */
    __USBD_MOUSE_PRODUCT_STRING_INDEX,   /**< \brief 描述产品的字符串索引 */
    __USBD_MOUSE_DEVICE_STRING_INDEX,    /**< \brief 描述设备序列号的字符串索引 */
...
static uint8_t __g_usb_mouse_desc_conf[AM_USB_DESC_LENGTH_ALL(__USBD_MOUSE_ENDPOINT_COUNT) + AM_USB_DESC_LENGTH_HID] = {
    ...
    34,
    0x00,
    __USBD_MOUSE_INTERFACE_COUNT,             /* 接口描述符个数 */
    __USBD_MOUSE_CONFIGURE_INDEX,             /* 配置值 */
    0x00,                                   /* 描述该配置的字符串索引,0x00表示没有 */

    /* 设备属性：总线供电，不支持远程唤醒 */
    0x80|(1<<6),
    __USBD_MOUSE_DEVICE_POWER,                /* 从总线获取的最大电流：100mA， 2mA一个单位 */
...
/**< \brief 描述产品的字符串描述符 */
 static const uint8_t __g_usb_mouse_desc_str_iproduct[16] = {
    sizeof(__g_usb_mouse_desc_str_iproduct), /* 字符串描述符字节数 */
...
/**< \brief 美式英语的语言ID为0x0409，简体中文的语言ID为0x0804，注意大小端。 */
 static const uint8_t __g_usb_mouse_desc_str_language_id[4] = {
    sizeof(__g_usb_mouse_desc_str_language_id),       /* 字符串描述符字节数 */
    AM_USB_DESC_TYPE_STRING,       /* 字符串描述符类型编号，固定为0x03 */
    0x09,
    0x04,       /* 简体中文 */
};
...
/**< \brief 描述厂商的字符串描述符 */
 static const uint8_t __g_usb_mouse_desc_str_imanufacturer[22] = {
    sizeof(__g_usb_mouse_desc_str_imanufacturer),       /* 字符串描述符字节数 */
    AM_USB_DESC_TYPE_STRING,       /* 字符串描述符类型编号，固定为0x03 */
    0x7f, 0x5e, /* 广 */
    0xde, 0x5d, /* 州 */
    0xf4, 0x81, /* 致 */
...
/**< \brief 描述设备序列号的字符串描述符 */
 static const uint8_t __g_usb_mouse_desc_str_iserialnumber[22] = {
    ...
    sizeof(__g_usb_mouse_desc_str_iserialnumber),    /* 字符串描述符字节数 */
    AM_USB_DESC_TYPE_STRING,                 /* 字符串描述符类型编号，固定为0x03 */
    0x32, 0x00, /* 2 */
    0x30, 0x00, /* 0 */
    0x31, 0x00, /* 1 */
...
static const am_usbd_devinfo_t __g_usbd_mouse_info = {
        __g_usb_mouse_descriptor,                                                 /**< \brief 描述符地址 */
        sizeof(__g_usb_mouse_descriptor) / sizeof(__g_usb_mouse_descriptor[0]),     /**< \brief 描述符个数 */
};
...
static const am_hc32f460_usbd_devinfo_t  __g_hc32f460_usbd_mouse_info = {
    ...
    __usb_mouse_init,                            /**< \brief 平台初始化 */
    __usb_mouse_deinit,                          /**< \brief 平台去初始化 */
    &__g_usbd_mouse_info,
};

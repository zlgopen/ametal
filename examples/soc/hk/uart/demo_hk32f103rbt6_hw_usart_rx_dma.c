 ...
 *
 * \par 源代码
 * \snippet demo_hk32f103rbt6_hw_usart_rx_dma.c src_hk32f103rbt6_hw_usart_rx_dma
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_hk32f103rbt6_hw_usart_rx_dma
 * \copydoc demo_hk32f103rbt6_hw_usart_rx_dma.c
 */

/** [src_hk32f103rbt6_hw_usart_rx_dma] */
...
/*******************************************************************************
  全局变量
*******************************************************************************/
static amhw_hk32f103rbt6_dma_xfer_desc_t g_desc;                  /**< \brief DMA 重载描述符 */
...
static amhw_hk32f103rbt6_usart_t     *gp_hw_usart = NULL;      /**< \brief USART 外设 */
 ...
 * \brief DMA 中断服务程序
 *
 * \param[in] p_arg : 用户自定义参数，通过 am_hk32f103rbt6_dma_isr_connect() 函数传递
 * \param[in] flag  : DMA中断标志，由底层驱动传入，该参数的可能取值：
 *                    (#AM_HK32F103RBT6_DMA_INT_ERROR) 或 (#AM_HK32F103RBT6_DMA_INT_NORMAL)
 *
 * \return 无
 */
static void usart_dma_isr (void *p_arg , uint32_t flag)
{
    ...
    if (flag == AM_HK32F103RBT6_DMA_INT_NORMAL) {
        if (flag_chan == g_dma_chan) {
            ...
            /* 禁能USART接收 */
        	amhw_hk32f103rbt6_usart_receiver_disable(gp_hw_usart);

            /* 禁能USART发送时使用DMA传输 */
        	amhw_hk32f103rbt6_usart_dmat_enable(gp_hw_usart);
 ...
 */
static int usart_rx_dma_tran_cfg (amhw_hk32f103rbt6_usart_t *p_hw_usart,
                                 int32_t               dma_chan,
                                 uint32_t              dma_tran_len)
{
    ...
    /* DMA传输配置 */
    flags = AMHW_HK32F103RBT6_DMA_CHAN_PRIORITY_HIGH         |  /* 外设请求源禁能 */
            AMHW_HK32F103RBT6_DMA_CHAN_MEM_SIZE_8BIT         |  /* 源地址1字节 */
            AMHW_HK32F103RBT6_DMA_CHAN_PER_SIZE_8BIT         |  /* 目的地址1字节写入 */
            AMHW_HK32F103RBT6_DMA_CHAN_MEM_ADD_INC_ENABLE    |  /* 请求有影响 */
            AMHW_HK32F103RBT6_DMA_CHAN_PER_ADD_INC_DISABLE   |  /* 无通道连接 */
            AMHW_HK32F103RBT6_DMA_CHAN_CIRCULAR_MODE_DISABLE ,  /* DMA中断使能 */

    /* 连接DMA中断服务函数 */
    am_hk32f103rbt6_dma_isr_connect(dma_chan, usart_dma_isr, (void *)dma_chan);


    /* 建立通道描述符 */
    am_hk32f103rbt6_dma_xfer_desc_build(&g_desc,                      /* 通道描述符 */
    ...
    /* 启动DMA传输，马上开始传输 */
    if (am_hk32f103rbt6_dma_xfer_desc_chan_cfg(&g_desc,
                                       AMHW_HK32F103RBT6_DMA_PER_TO_MER, /* 外设到 内存 */
 ...
 * \brief USART 初始化
 */
static void usart_hw_init (amhw_hk32f103rbt6_usart_t *p_hw_usart, uint32_t clk_rate)
{
 ...
 */
static void usart_hw_dma_init (amhw_hk32f103rbt6_usart_t *p_hw_usart)
{
    /* 串口接收DMA传输使能 */
	amhw_hk32f103rbt6_usart_dmar_enable(p_hw_usart);
 ...
 * \brief 例程入口
 */
void demo_hk32f103rbt6_hw_usart_rx_dma_entry (amhw_hk32f103rbt6_usart_t *p_hw_usart,
                                        uint32_t             clk_rate,
                                        int32_t              dma_chan)
{
    ...
    amhw_hk32f103rbt6_usart_poll_send(p_hw_usart,
                               (uint8_t *)"USART DMA RX start:\r\n",
                               (uint32_t)sizeof("USART DMA RX start:\r\n") - 1);
    /* 开始DMA传输 */
    am_hk32f103rbt6_dma_init(&__g_dma_dev, &__g_dma_devinfo);
    ...
    amhw_hk32f103rbt6_usart_poll_send(p_hw_usart, g_buf_dst, sizeof(g_buf_dst));
    amhw_hk32f103rbt6_usart_poll_send(p_hw_usart,
                               (uint8_t *)"\r\nDMA transfer done!\r\n",
                                sizeof("\r\nDMA transfer done!\r\n") - 1);

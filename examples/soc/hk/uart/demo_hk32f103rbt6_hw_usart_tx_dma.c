...
/** \brief DMA 重载描述符 */
static amhw_hk32f103rbt6_dma_xfer_desc_t g_desc;
 ...
 *                    (# AM_HK32F103RBT6_DMA_INT_NORMAL) 或 (# AM_HK32F103RBT6_DMA_INT_ERROR)
 *
 * \return 无。
 */
static void usart_dma_isr (void *p_arg , uint32_t flag)
{
    ...
    if (flag == AM_HK32F103RBT6_DMA_INT_NORMAL) {
        if (flag_chan == g_dma_chan) {
            ...
            /* 禁能USART发送时使用DMA传输 */
        	amhw_hk32f103rbt6_usart_dmat_disable(gp_hw_usart);
 ...
 * \brief DMA传输配置。
 */
static int usart_tx_dma_tran_cfg (amhw_hk32f103rbt6_usart_t *p_hw_usart,
                                 int32_t               dma_chan,
...
static void usart_hw_init (amhw_hk32f103rbt6_usart_t *p_hw_usart, uint32_t clk_rate)
{
    ...
    /* 串口发送DMA传输使能 */
	amhw_hk32f103rbt6_usart_dmat_enable(p_hw_usart);
 ...
 * \brief 例程入口
 */
void demo_hk32f103rbt6_hw_usart_tx_dma_entry (amhw_hk32f103rbt6_usart_t *p_hw_usart,
                                        uint32_t             clk_rate,
                                        int32_t              dma_chan)
{
    uint8_t i = 0;
    ...
    while (1) {
        ...
        if (g_trans_done == AM_TRUE) {
            ...
            if (j < 3) {
                am_hk32f103rbt6_dma_chan_stop(dma_chan);
                if (0 == i) {
...
/** [src_hk32f103rbt6_hw_usart_tx_dma] */

 ...
 *
 *      报文结构体can_rcv_msg在demo_hk32f103rbt6_can_int_entry()中使用的是全局变量，正常工作(已唤醒)，
 *      开启了接收或溢出中断后，上位机发送的数据，本测试工程会每500ms返回给上位机。
 *
 *   2. 串口打印相关调试信息。
 *
 * \par 源代码
 * \snippet src_hk32f103rbt6_can_int.c
 *
 * \internal
...
/**
 * \addtogroup demo_hk32f103rbt6_can_int
 * \copydoc demo_hk32f103rbt6_can_int.c
 */
...
void can_int_handle(void *p_arg)
{
    ...
    am_hk32f103rbt6_can_dev_t *p_dev    = (am_hk32f103rbt6_can_dev_t *)p_arg;
...
//        amhw_hk32f103rbt6_can_clear_itstatus(p_hw_can, AMHW_HK32F103RBT6_CAN_INT_FMPIE1);
    ...
    if(p_int_type & AM_CAN_INT_ERROR) {
        ...
        /* 清除错误标志*/
        amhw_hk32f103rbt6_can_clear_itstatus (p_hw_can, AMHW_HK32F103RBT6_CAN_INT_ERRIE);
    }
}
                                ...
                                size_t              lenth)
{
    ...
    /* 连接中断服务函数  */
    /* 第二个参数传入为NULL时，连接内部默认中断服务函数， __can_hk32f103rbt6_int_handle

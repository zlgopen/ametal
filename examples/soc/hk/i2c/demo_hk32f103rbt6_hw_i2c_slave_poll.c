 ...
 *
 * \par 源代码
 * \snippet demo_hk32f103rbt6_hw_i2c_slave_poll.c src_hk32f103rbt6_hw_i2c_slave_poll
 *
 * \internal
...
/**
 * \addtogroup demo_if_hk32f103rbt6_hw_i2c_slave_poll
 * \copydoc demo_hk32f103rbt6_hw_i2c_slave_poll.c
 */

/** [src_hk32f103rbt6_hw_i2c_slave_poll] */
#include "ametal.h"
#include "am_vdebug.h"
 ...
 */
static int __i2c_slave_init (amhw_hk32f103rbt6_i2c_t *p_hw_i2c, uint8_t dev_addr)
{
     uint32_t pclk1 = 0;
     uint16_t tmpreg = 0, i2c_freq_value = 0;
     uint16_t result = 0x04;
     uint32_t speed = 100000;
     uint16_t i2c_dutycycle = AMHW_HK32F103RBT6_I2C_DUTY_CYCLE_2;
     uint16_t i2c_mode      = AMHW_HK32F103RBT6_I2C_MODE_I2C;
     uint16_t i2c_ack       = AMHW_HK32F103RBT6_I2C_ACK_ENABLE;

     uint16_t i2c_acknowledgedaddress = AMHW_HK32F103RBT6_I2C_ACK_ADDRESS_7_BIT;
     ...
     if (speed <= 100000) {
         ...
         /** <brief 最高为 400khz */
         if (i2c_dutycycle == AMHW_HK32F103RBT6_I2C_DUTY_CYCLE_2 ) {
...
static void __i2c_slave_event (amhw_hk32f103rbt6_i2c_t *p_hw_i2c)
{
    ...
    case __I2C_SLV_ST_SUBADDR_GET:
        if (is_sub_addr == AM_FALSE) {
            am_udelay(100);
            sub_addr =  amhw_hk32f103rbt6_i2c_recv_data (p_hw_i2c);
            is_sub_addr = AM_TRUE;
            status = __I2C_SLV_ST_ADDR_MATCH;
        } else {
            if (amhw_hk32f103rbt6_i2c_checkflagstaus(p_hw_i2c,
                    AMHW_HK32F103RBT6_I2C_FLAG_TRA)) {
    ...
    case __I2C_SLV_ST_TXBYTE_GET:
        if (amhw_hk32f103rbt6_i2c_checkflagstaus(p_hw_i2c,
                AMHW_HK32F103RBT6_I2C_FLAG_TXE)) {

            amhw_hk32f103rbt6_i2c_send_data (p_hw_i2c, eeprom[sub_addr++]);
            sub_addr %= EEPROM_BUFFER_LEN;
        }
        if (amhw_hk32f103rbt6_i2c_checkflagstaus(p_hw_i2c,
                   AMHW_HK32F103RBT6_I2C_FLAG_AF)) {

            amhw_hk32f103rbt6_i2c_clearflag(p_hw_i2c, AMHW_HK32F103RBT6_I2C_FLAG_AF);
    ...
    case __I2C_SLV_ST_RXBYTE_PUT:
        if (amhw_hk32f103rbt6_i2c_checkflagstaus(p_hw_i2c,
                AMHW_HK32F103RBT6_I2C_FLAG_RXNE)) {

            eeprom[sub_addr++] = amhw_hk32f103rbt6_i2c_recv_data (p_hw_i2c);
            sub_addr %= EEPROM_BUFFER_LEN;
        }

        if (amhw_hk32f103rbt6_i2c_checkflagstaus(p_hw_i2c,
                AMHW_HK32F103RBT6_I2C_FLAG_STOPF)) {

...
void demo_hk32f103rbt6_drv_flash_entry (amhw_hk32f103rbt6_flash_t *p_hw_flash, uint8_t page_num)
{
    int             i;
    ...
    /* FLASH 初始化 */
    am_hk32f103rbt6_flash_init(p_hw_flash);

    /* 擦除扇区 */
    status = am_hk32f103rbt6_flash_page_erase(p_hw_flash, page_num * 1024);
    ...
    /* 向扇区中写入数据 */
    status = am_hk32f103rbt6_flash_sector_program(p_hw_flash,
                                            page_num * 1024,
                                            data,
                                            1024 / 4);

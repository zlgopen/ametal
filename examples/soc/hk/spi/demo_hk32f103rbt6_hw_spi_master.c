 ...
 *
 * \par 源代码
 * \snippet demo_hk32f103rbt6_hw_spi_master.c src_hk32f103rbt6_hw_spi_master
 *
 * \internal
 * \par History
...
/**
 * \addtogroup demo_if_hk32f103rbt6_hw_spi_master
 * \copydoc demo_hk32f103rbt6_hw_spi_master.c
 */
 
/** [src_hk32f103rbt6_hw_spi_master] */
#include "ametal.h"
#include "am_gpio.h"
#include "am_delay.h"
#include "am_vdebug.h"
...
static void spi_master_cfg (amhw_hk32f103rbt6_spi_t *p_hw_spi)
{
    /* 关闭spi外设*/
    ...
    /* 配置时钟相位和极性 */
    amhw_hk32f103rbt6_spi_clk_mode_set(p_hw_spi, SPI_CFG_MODE_1 );
 ...
 * \brief SPI回环传输测试
 */
void spi_loop_trans (amhw_hk32f103rbt6_spi_t *p_hw_spi,
					 ...
					 uint32_t           cs_mdelay)
{
    ...
    while(pos < p_trans->nbytes) {
        ...
        amhw_hk32f103rbt6_spi_tx_put(p_hw_spi,*((uint8_t*)(p_trans->p_txbuf) + pos));

        /* 等待可以接收 */
        while (amhw_hk32f103rbt6_spi_status_flag_check (
            p_hw_spi,AMHW_HK32F103RBT6_SPI_RX_NOT_EMPTY_FLAG) == AM_FALSE) {
        	;
        }

        *(uint8_t*)((uint8_t*)p_trans->p_rxbuf + pos) =
            amhw_hk32f103rbt6_spi_rx_get(p_hw_spi);
        pos += 1;
    }

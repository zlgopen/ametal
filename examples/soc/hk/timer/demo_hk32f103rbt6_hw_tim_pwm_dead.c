 ...
 *
 * \par 源代码
 * \snippet demo_hk32f103rbt6_hw_tim_pwm_dead.c src_hk32f103rbt6_hw_tim_pwm_dead
 *
 * \internal
...
/**
 * \addtogroup demo_if_hk32f103rbt6_hw_tim_pwm_dead
 * \copydoc demo_hk32f103rbt6_hw_tim_pwm_dead.c
 */

/** [src_hk32f103rbt6_hw_tim_pwm_dead] */
#include "ametal.h"
#include "am_delay.h"
#include "am_vdebug.h"
#include "am_bitops.h"
 ...
 */
am_local void __tim_pwm_chan_config (amhw_hk32f103rbt6_tim_t *p_hw_tim,
                                     uint32_t        chan,
                                     uint32_t        freq,
                                     uint32_t        duty,
                                     uint32_t        clk_rate)
{
    ...
    /* 设置比较输出通道的匹配值 */
    amhw_hk32f103rbt6_tim_ccr_ouput_reload_val_set(p_hw_tim,  duty_c, chan);
    ...
    /* 选择该通道的模式为 PWM 模式 2 */
    amhw_hk32f103rbt6_tim_ocm_set(p_hw_tim, AMHW_HK32F103RBT6_TIM_PWM_MODE2, chan);

    /* PWM 输出通道预装载使能 */
    amhw_hk32f103rbt6_tim_ccs_ocpe_enable(p_hw_tim, chan);

    /* 设置比较输出通道 CCP 高电平极性有效 */
    amhw_hk32f103rbt6_tim_ccp_output_set(p_hw_tim, 0, chan);
}
 ...
 * \return 无
 */
am_local void __tim_pwm_enable (amhw_hk32f103rbt6_tim_t *p_hw_tim, uint32_t chan)
{
 ...
 * \return 无
 */
am_local void __tim_pwm_init (amhw_hk32f103rbt6_tim_t *p_hw_tim)
{
    /* 边沿对齐模式 */
 ...
 * \return 死区时间，单位为皮秒
 */
am_local int32_t __dead_time_get (amhw_hk32f103rbt6_tim_t *p_hw_tim,
                                  uint8_t         dtg,
                                  uint32_t        clk_rate)
{
    int32_t  dt;
 ...
 * \return DTG 配置值
 */
am_local int32_t __dtg_calculate (amhw_hk32f103rbt6_tim_t *p_hw_tim,
                                  uint32_t        dead_time,
                                  uint32_t        clk_rate)
{
    int32_t i;
...
void demo_hk32f103rbt6_hw_tim_pwm_dead_entry (amhw_hk32f103rbt6_tim_t     *p_hw_tim,
                                     ...
                                     uint32_t            clk_rate)
{
    if ((AMHW_HK32F103RBT6_TIM_TYPE0 != type) &&
        (AMHW_HK32F103RBT6_TIM_TYPE2 != type) &&
        (AMHW_HK32F103RBT6_TIM_TYPE3 != type)) {
    ...
    /* 使能互补输出 */
    amhw_hk32f103rbt6_tim_ccne_enable(p_hw_tim, AMHW_HK32F103RBT6_TIM_CC1S);

    /* 使能互补输出高电平有效 */
    amhw_hk32f103rbt6_tim_ccne_set(p_hw_tim, 0, AMHW_HK32F103RBT6_TIM_CC1S);

    /* 设置死区时间 */
    amhw_hk32f103rbt6_tim_utg_set(p_hw_tim, __dtg_calculate(p_hw_tim, __DEAD_TIME, clk_rate));

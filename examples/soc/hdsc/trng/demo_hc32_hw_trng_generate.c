 ...
 *
 * \par 源代码
 * \snippet demo_hc32_hw_trng_dac.c src_hc32_hw_trng_generate
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_trng_generate
 * \copydoc demo_hc32_hw_trng_generate.c
 */

/** [src_hc32_hw_trng_generate] */
#include "ametal.h"
#include "am_hc32.h"
#include "am_int.h"
#include "am_delay.h"
...
/*******************************************************************************
  全局变量
*******************************************************************************/
static amhw_hc32_trng_t       *gp_hw_trng   = NULL;  /**< \brief trng 外设 */
...
static void rng_generate (uint8_t env_flag)
{
    ...
    /* 打开随机源电路 */
    amhw_hc32_trng_cir_enable (gp_hw_trng);

    if (env_flag == AM_HC32_TRNG_POWER_ON){
        /* 选择重新装载初始值 */
        amhw_hc32_trng_load_set (gp_hw_trng, AM_TRUE);
    }else{
        /* 选择不重新装载初始值 */
        amhw_hc32_trng_load_set (gp_hw_trng, AM_FALSE);
    }
    ...
    /* 选择移位次数 */
    amhw_hc32_trng_load_set (gp_hw_trng, AMHW_HC32_TRNG_CNT_256);

    /* 随机数生成启动 */
    amhw_hc32_trng_run (gp_hw_trng);

    /* 等待生成完成 */
    while (amhw_hc32_trng_stu_check (gp_hw_trng)){
    ...
    /* 选择移位次数 */
    amhw_hc32_trng_load_set (gp_hw_trng, AMHW_HC32_TRNG_CNT_64);
    ...
    /* 关闭随机源电路 */
    amhw_hc32_trng_cir_disable (gp_hw_trng);
}
...
{
    gp_hw_trng  = (amhw_hc32_trng_t *)p_hw_trng;

    /* 第一次上电产生随机数 */
    rng_generate (AM_HC32_TRNG_POWER_ON);
    ...
    /* 非上电产生随机数 */
    rng_generate (AM_HC32_TRNG_POWER_NON);

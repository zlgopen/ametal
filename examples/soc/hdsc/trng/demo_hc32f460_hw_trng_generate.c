...
/*******************************************************************************
  全局变量
*******************************************************************************/
static amhw_hc32f460_trng_t       *gp_hw_trng   = NULL;  /**< \brief trng 外设 */
...
static void rng_generate (uint8_t env_flag)
{
    ...
    /* 打开随机源电路 */
    amhw_hc32f460_trng_cir_enable (gp_hw_trng);

    if (env_flag == AM_HC32F460_TRNG_LOAD_NEW_INIT_VALUE){
        /* 选择重新装载初始值 */
        amhw_hc32f460_trng_load_set (gp_hw_trng, AM_TRUE);
    }else{
        /* 选择不重新装载初始值 */
        amhw_hc32f460_trng_load_set (gp_hw_trng, AM_FALSE);
    }

    /* 随机数生成启动 */
    amhw_hc32f460_trng_run (gp_hw_trng);

    /* 等待生成完成 */
    while (amhw_hc32f460_trng_stu_check (gp_hw_trng));

    /* 读取64位随机数 */
    rng_data[0] = amhw_hc32f460_trng_low32_data_read (gp_hw_trng);
    rng_data[1] = amhw_hc32f460_trng_high32_data_read (gp_hw_trng);


    /* 关闭随机源电路 */
    amhw_hc32f460_trng_cir_disable (gp_hw_trng);
}
...
{
    gp_hw_trng  = (amhw_hc32f460_trng_t *)p_hw_trng;

    while(1) {
        /* 随机数生成前，数据寄存器从随机数发生器装载新的初始值 */
        rng_generate (AM_HC32F460_TRNG_LOAD_NEW_INIT_VALUE);

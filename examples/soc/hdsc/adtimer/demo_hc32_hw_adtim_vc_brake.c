 ...
 *
 * \par 源代码
 * \snippet demo_hc32_hw_adtim_vc_brake.c src_hc32_hw_adtim_vc_brake
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_adtim_vc_brake
 * \copydoc demo_hc32_hw_adtim_vc_brake.c
 */

/** [src_hc32_hw_adtim_vc_brake] */
#include "ametal.h"
#include "am_clk.h"
#include "am_delay.h"
#include "am_vdebug.h"
...
static void __vc0_irq_handler (void *drv)
{
    ...
    amhw_hc32_vc_t *p_hw_vc = (amhw_hc32_vc_t *)drv;
...
static am_bool_t __hc32_hw_adtim_init(void          *p_hw_adtim,
                                        uint8_t        chan,
    ...
    portcfg.dissel    = AMHW_HC32_ADTIM_DISSEL0;    // 选择强制输出无效条件0(VC刹车)
    portcfg.disval    = AMHW_HC32_ADTIM_DISVAL_LOW; // 强制输出无效条件0~3中条件成立时，CHx端口输出低电平
                                        ...
                                        uint8_t       vc0_p)
{
    ...
    /* 定时器初始化配置 */
    if(__hc32_hw_adtim_init(p_adtim, chan, duty_ns, period_ns) ==
       AM_FALSE) {
        return;
    }

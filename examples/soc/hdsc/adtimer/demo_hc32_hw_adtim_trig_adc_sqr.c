 ...
 *     则需要更改demo_hc32_hw_adtim_trig_adc_sqr.c中定时器分频系数的设定。
 *   3.不同情况下，ADC的转换速度有限，需要考虑定时器的周期时间是否大于ADC转换时间（多个通道则需要累加转换时间）
 *
 * \par 源代码
 * \snippet demo_hc32_hw_adtim_trig_adc_sqr.c src_hc32_hw_adtim_trig_adc_sqr
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_adtim_trig_adc_sqr
 * \copydoc demo_hc32_hw_adtim_trig_adc_sqr.c
 */

/** [src_hc32_hw_adtim_trig_adc_sqr] */
#include "ametal.h"
#include "am_delay.h"
#include "am_int.h"
#include "am_board.h"
...
static am_bool_t __hc32_hw_adtim_init (amhw_hc32_adtim_t *p_hw_adtim,
                                         uint32_t             period_us)
{
    amhw_hc32_adtim_basecnt_cfg_t   basecnt;
    amhw_hc32_adtim_irq_trig_cfg_t irq_trig;
    ...
    amhw_hc32_adtim_irqtrigcfg(p_hw_adtim, &irq_trig);
...
static void __hc32_hw_adc_init (amhw_hc32_adtim_t *p_hw_adtim,
                                  ...
                                  uint8_t              sqr_num)
{
    ...
    /* 外部中断触发源设置 */
    if((uint32_t)p_hw_adtim == HC32_TIM4_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM4);
    } else if((uint32_t)p_hw_adtim == HC32_TIM5_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM5);
    } else if((uint32_t)p_hw_adtim == HC32_TIM6_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM6);
    }
                                            ...
                                            uint8_t  sqr_num)
{
    ...
    /* 定时器配置 */
    if((__hc32_hw_adtim_init(p_adtim, period_us)) == AM_FALSE) {
        return;
    }

    /* ADC配置 */
    __hc32_hw_adc_init(p_adtim, p_adc, adc_int_num, p_adc_chan, sqr_num);

    /* 启动定时器 */
    amhw_hc32_adtim_startcount(p_adtim);

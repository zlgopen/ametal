 ...
 *
 * \par 源代码
 * \snippet demo_hc32_hw_uart_tx_dma.c src_hc32_hw_uart_tx_dma
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_uart_tx_dma
 * \copydoc demo_hc32_hw_uart_tx_dma.c
 */

/** [src_hc32_hw_uart_tx_dma] */
#include "ametal.h"
#include "am_int.h"
 ...
 *                    (# AM_hc32_DMA_INT_NORMAL) 或 (# AM_hc32_DMA_INT_ERROR)
 *
 * \return 无。
 */
static void uart_dma_isr (void *p_arg , uint32_t flag)
{
    ...
    if (flag == AM_HC32_DMA_INT_COMPLETE) {
        if (flag_chan == g_dma_chan) {
            ...
            /* 禁能UART发送时使用DMA传输 */
            amhw_hc32_uart_disable(gp_hw_uart, AMHW_HC32_UART_DMA_TX);
 ...
 * \brief DMA传输配置。
 */
static int uart_tx_dma_tran_cfg (amhw_hc32_uart_t *p_hw_uart,
                                 int32_t             dma_chan,
...
{
    /* 串口发送DMA传输使能 */
    amhw_hc32_uart_enable(p_hw_uart, AMHW_HC32_UART_DMA_TX);

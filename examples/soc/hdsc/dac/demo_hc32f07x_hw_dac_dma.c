 ...
 *
 * \par 源代码
 * \snippet demo_hc32_hw_dac_dma.c src_hc32_hw_dac_dma
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_dac_dma
 * \copydoc demo_hc32_hw_dac_dma.c
 */
...
/*******************************************************************************
  宏定义
*******************************************************************************/
static amhw_hc32f07x_dac_t      *gp_hw_dac = NULL;     /**< \brief dac 外设 */
 ...
 * \brief DAC DMA传输配置
 */
static int dac_dma_tran_cfg (amhw_hc32f07x_dac_t *p_hw_dac,
                             int32_t           dma_chan)
{
    ...
    /* 连接DMA中断服务函数 */
    am_hc32_dma_isr_connect(dma_chan, dac_dma_isr, (void *)dma_chan);

    /* DMA 传输配置 */
    flags = AMHW_HC32_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
            AMHW_HC32_DMA_CHAN_SIZE_16BIT             | /* 数据位宽 8bit */
    ...
    am_hc32_dma_xfer_desc_build(&g_desc,                      /* 通道描述符 */
                                   ...
                                   (uint32_t)(&(p_hw_dac->dacdhr12r1)), /* 目标数据缓冲 */
                                   1,                           /* 传输次数 */
                                   flags);                      /* 传输配置 */
                        ...
                        uint32_t           verf)
{
    ...
    /* 设置参考电压源 */
    amhw_hc32f07x_dac_chan1_sref_sel(p_hw_dac, verf);

    /* 失能输出缓冲器 */
    amhw_hc32f07x_dac_chan1_output_buf_enable(p_hw_dac);

    /* 触发使能 */
    amhw_hc32f07x_dac_chan1_trg_enable(p_hw_dac);

    /* 触发方式选择  软件触发*/
    amhw_hc32f07x_dac_chan1_trg_sel(p_hw_dac, AMHW_HC32F07X_DAC_CHAN_SOFTWARE_TRG);
 ...
 * \brief DAC DMA初始化
 */
static void dac_hw_dma_init(amhw_hc32f07x_dac_t *p_hw_dac)
{
    if (p_hw_dac == NULL){
        return ;
    }
    ...
    /* 使能DAC DMA功能 */
    amhw_hc32f07x_dac_chan1_dma_enbale (p_hw_dac);
}
...
//        /* 软件触发 */
        amhw_hc32f07x_dac_chan1_software_trg_enable(gp_hw_dac);

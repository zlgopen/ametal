 ...
 *   2. 拉低PB7，产生下降沿触发序列A转换。
 *
 * - 实验现象：
 *   1. 拉低引脚PB6 后触发ADC1 序列B转换，序列A转换完成后触发序列B转换。
 *   2. 调试串口输出 PA0,PA1电压采样值。
 *
 * \par 源代码
...
static void __adc_isr (void *p_arg)
{
    ...
    __g_adc_dat[1] = amhw_hc32f460_adc_ch_data_get (p_hw_adc,
                                                    ADC_CHAN_B,
                                                    AMHW_HC32F460_ADC_RESOLUTION_12BIT,
                                                    AMHW_HC32F460_ADC_DATA_ALIGN_FORMAT_RIGHT);
                                                       ...
                                                       uint32_t vref_mv)
{
    ...
    /* 映射中断源到具体中断向量 */
    amhw_hc32f460_int_sel0_31(int_num, EVT_ADC1_EOCB);
    /* 连接转换完成中断 */
    ...
    /* 转换模式为序列A单次转换、序列B单次转换 */
    amhw_hc32f460_adc_mode_set(p_hw_adc, AMHW_HC32F460_ADC_SEQ_A_ONCE_B_ONCE);

    /* ADC通道选择 */
    amhw_hc32f460_adc_seq_a_channel_sel0(p_hw_adc, chan_a);
    amhw_hc32f460_adc_seq_b_channel_sel0(p_hw_adc, chan_b);

    /* 设置ADC平均转换次数 */
    amhw_hc32f460_adc_average_count_set(p_hw_adc, AMHW_HC32F460_ADC_AVERAGE_COUNT_32);

    /* ADC平均通道选择 */
    amhw_hc32f460_adc_average_channel_sel0(p_hw_adc, chan_a);
    amhw_hc32f460_adc_average_channel_sel0(p_hw_adc, chan_b);

    /* 设置ADC序列A触发条件为外部引脚触发 */
    amhw_hc32f460_adc_seq_a_trigger_source_sel(p_hw_adc, AMHW_HC32F460_ADC_SEQ_TRIGGER_ADTRx);
    amhw_hc32f460_adc_seq_b_trigger_source_sel(p_hw_adc, AMHW_HC32F460_ADC_SEQ_TRIGGER_IN_TRGx0);
    amhw_hc32f460_adc_intrgx0_sel(EVT_ADC1_EOCA);
    amhw_hc32f460_aos_adc_in_event_trigger_sel(HC32F460_AOS_ADC1_ITRGSEL0, EVT_ADC1_EOCA);
    /* 使能ADC序列A片内或外部引脚触发许可 */
    amhw_hc32f460_adc_seq_a_trigger_enable(p_hw_adc);
    amhw_hc32f460_adc_seq_b_trigger_enable(p_hw_adc);
    ...
    /* 使能转换完成中断 */
    amhw_hc32f460_adc_int_enable(p_hw_adc, AMHW_HC32F460_ADC_INT_EOCBF);

    amhw_hc32f460_adc_sampling_period_set(p_hw_adc, chan_a, 50);
    amhw_hc32f460_adc_sampling_period_set(p_hw_adc, chan_b, 50);

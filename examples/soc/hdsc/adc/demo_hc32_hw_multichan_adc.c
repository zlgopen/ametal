 ...
 *
 * \par 源代码
 * \snippet demo_hc32_hw_multichan_adc.c src_hc32_hw_multichan_adc
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_multichan_adc
 * \copydoc demo_hc32_hw_multichan_adc.c
 */

/** [src_hc32_hw_multichan_adc] */
#include "ametal.h"
#include "am_vdebug.h"
#include "am_delay.h"
#include "am_int.h"
...
static void __adc_isr (void *p_arg)
{
    ...
    if(amhw_hc32_adc_flag_check(p_hw_adc,
                                  AMHW_HC32_ADC_FLAG_CONVERT_JQR) ==
                                  AM_TRUE) {

        amhw_hc32_adc_flag_clr(p_hw_adc, AMHW_HC32_ADC_FLAG_CONVERT_JQR);

        for(i = __adc_sqr_num; i < __adc_sqr_num + __adc_jqr_num; i++) {

           __g_adc_dat[i] = amhw_hc32_adc_jqr_chan_data_get(p_hw_adc,
                                                              i - __adc_sqr_num);
        }
    }
    ...
    if(i == __adc_sqr_num + __adc_jqr_num) {
        ...
        amhw_hc32_adc_sqr_convert_stop(p_hw_adc);
        amhw_hc32_adc_jqr_convert_stop(p_hw_adc);
    }
}
                                       ...
                                       uint8_t  jqr_num)
{
    ...
    /* 设置参考电压源 */
    amhw_hc32_adc_refvol_sel(p_adc, AMHW_HC32_ADC_REFVOL_INSIDE_2500MV);
    ...
    /* 使能扫描转换模式 */
    amhw_hc32_adc_mode_sel(p_adc, AMHW_HC32_ADC_MODE_SCAN);
    ...
    /* 链接ADC顺序扫描转换通道和引脚 */
    for(i = 0; i < __adc_sqr_num; i++) {
        amhw_hc32_adc_sqr_chan_sel(p_adc,
                                     i,
                                     (amhw_hc32_adc_chan_sel_t)p_adc_chan[i]);
    }

    amhw_hc32_adc_sqr_num_set(p_adc, __adc_sqr_num - 1);

    /* 链接ADC插入扫描转换通道和引脚 */
    for(i = __adc_sqr_num; i < __adc_sqr_num + __adc_jqr_num; i++) {

        amhw_hc32_adc_jqr_chan_sel(p_adc,
                                     i - __adc_sqr_num,
                                     (amhw_hc32_adc_chan_sel_t)p_adc_chan[i]);
    }

    amhw_hc32_adc_jqr_num_set(p_adc, __adc_jqr_num - 1);
    ...
    while(1) {
        ...
        amhw_hc32_adc_jqr_convert_start(p_adc);

 ...
 *       要更改demo_hc32_hw_tim_pwm_dead.c中的分频系数，外设时钟两个参数。
 *
 * \par 源代码
 * \snippet demo_hc32116_hw_tim_pwm_dead.c src_hc32116_hw_tim_pwm_dead
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32116_hw_tim_pwm_dead
 * \copydoc demo_hc32116_hw_tim_pwm_dead.c
 */

/** [src_hc32116_hw_tim_pwm_dead] */
#include "ametal.h"
#include "am_clk.h"
#include "am_clk.h"
#include "am_int.h"
                                       ...
                                       uint16_t           dead_us)
{
    ...
    /* 设置通道A为比较模式 */
    amhw_hc32_tim_mode23_csa_compare(p_hw_tim, chan_a);

    /* 设置通道 B为比较模式 */
    amhw_hc32_tim_mode23_csb_compare(p_hw_tim, chan_b);
    ...
    amhw_hc32_tim_mode23_compare_set(
       p_hw_tim,
       (amhw_hc32_tim_compare_type_t)(chan_a * 4),
       AMHW_HC32_TIM_COMPARE_PWM2);

    /* 设置通道B的PWM输出模式为PWM2 */
    amhw_hc32_tim_mode23_compare_set(
       p_hw_tim,
       (amhw_hc32_tim_compare_type_t)(chan_b * 4),
       AMHW_HC32_TIM_COMPARE_PWM2);

    /* 通道A正常输出 */
    amhw_hc32_tim_mode23_phase_same(
       p_hw_tim,
       (amhw_hc32_tim_phase_type_t)(chan_a * 4 + 3));

    /* 通道A正常输出 */
    amhw_hc32_tim_mode23_phase_same(
       p_hw_tim,
       (amhw_hc32_tim_phase_type_t)(chan_b * 4 + 3));

    /* 清除标志位 */
    amhw_hc32_tim_mode23_int_flag_clr(p_hw_tim, AMHW_HC32_TIM_INT_FLAG_ALL);

    /* 死区控制使能  */
    amhw_hc32_tim_mode23_dtr_enable(p_hw_tim, AMHW_HC32_TIM_DTR_DTEN);

    deadtime = (float)dead_us * (float)clkfreq / (float)(fre_div * 1000000) + 0.5;

    /* 死区时间设置  */
    amhw_hc32_tim_mode23_dtr_set(p_hw_tim, (uint8_t)deadtime);

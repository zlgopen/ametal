 ...
 *     则需要更改demo_hc32_hw_tim_trig_adc_sqr.c中定时器分频系数的设定。
 *   3.不同情况下，ADC的转换速度有限，需要考虑定时器的周期时间是否大于ADC转换时间（多个通道则需要累加转换时间）
 *
 * \par 源代码
 * \snippet demo_hc32_hw_tim_trig_adc_sqr.c src_hc32_hw_tim_trig_adc_sqr
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_tim_trig_adc_sqr
 * \copydoc demo_hc32_hw_tim_trig_adc_sqr.c
 */

/** [src_hc32_hw_tim_trig_adc_sqr] */
#include "ametal.h"
#include "am_delay.h"
#include "am_int.h"
#include "am_board.h"
...
static void __adc_isr (void *p_arg)
{
    ...
    if(amhw_hc32_adc_flag_check(p_hw_adc,
                                  AMHW_HC32_ADC_FLAG_CONVERT_SQR) ==
                                  AM_TRUE) {

        amhw_hc32_adc_flag_clr(p_hw_adc, AMHW_HC32_ADC_FLAG_CONVERT_SQR);

        for(i = 0; i < __adc_sqr_num; i++) {

            __g_adc_dat[i] = amhw_hc32_adc_sqr_chan_data_get(p_hw_adc, i);
        }
                                       ...
                                       uint32_t           period_us)
{
    ...
    /* 使能ADC触发全局控制（总开关） */
    amhw_hc32_tim_mode23_adc_trigger_enable(
        p_hw_tim,
        AMHW_HC32_TIM_ADC_TRIGGER_ADTE);

    /* 使能事件更新触发ADC（实际事件选择） */
    amhw_hc32_tim_mode23_adc_trigger_enable(
        p_hw_tim,
        AMHW_HC32_TIM_ADC_TRIGGER_UEVE);

    return AM_TRUE;
}

static void __hc32_hw_adc_init (amhw_hc32_tim_t *p_hw_tim,
                                  ...
                                  uint8_t            sqr_num)
{
    ...
    amhw_hc32_adc_sample_period_sel(p_hw_adc,
                                      AMHW_HC32_ADC_SAMPLE_PERIOD_4);

    /* 设置ADC时钟分频 */
    amhw_hc32_adc_pclk_div_sel(p_hw_adc, AMHW_HC32_ADC_PCLK_DIV2);
    ...
    amhw_hc32_adc_sqr_num_set(p_hw_adc, __adc_sqr_num - 1);

    /* 清标志位 */
    amhw_hc32_adc_flag_clr(p_hw_adc, AMHW_HC32_ADC_FLAG_ALL);

    /* 外部中断触发源设置 */
    if((uint32_t)p_hw_tim == HC32_TIM0_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM0);
    } else if((uint32_t)p_hw_tim == HC32_TIM1_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM1);
    } else if((uint32_t)p_hw_tim == HC32_TIM2_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM2);
    } else if((uint32_t)p_hw_tim == HC32_TIM3_BASE) {
        amhw_hc32_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_HC32_ADC_INT_TRIGGER_TIM3);
    }
                                          ...
                                          uint8_t  sqr_num)
{
    ...
    /* 定时器配置 */
    if((__hc32_hw_tim_init(p_tim, type, chan, duty_us , period_us)) ==
       AM_FALSE) {
        return;
    }

    /* ADC配置 */
    __hc32_hw_adc_init(p_tim, p_adc, adc_int_num, p_adc_chan, sqr_num);

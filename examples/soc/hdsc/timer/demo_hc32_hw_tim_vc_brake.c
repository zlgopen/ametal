 ...
 *
 * \par 源代码
 * \snippet demo_hc32_hw_tim_vc_brake.c src_hc32_hw_tim_vc_brake
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32_hw_tim_vc_brake
 * \copydoc demo_hc32_hw_tim_vc_brake.c
 */

/** [src_hc32_hw_tim_vc_brake] */
#include "ametal.h"
#include "am_clk.h"
#include "am_delay.h"
#include "am_vdebug.h"
...
static void __vc0_irq_handler (void *p_arg)
{
    ...
    amhw_hc32_vc_t *p_hw_vc = (amhw_hc32_vc_t *)p_arg;
 ...
 * \brief VC初始化
 */
static am_bool_t __vc_init (amhw_hc32_vc_t       *p_hw_vc,
                            amhw_hc32_vc0_n_sel_t vc0_n,
                            amhw_hc32_vc0_p_sel_t vc0_p)
{
    /* 使能BGR */
    amhw_hc32_bgr_enable(AM_TRUE);

    /* 配置参考电压为VCC */
    amhw_hc32_vc_ref2p5_sel(p_hw_vc, AMHW_HC32_VC_REF2P5_VCC);

    /* 设置参考电压不分压 */
    amhw_hc32_vc_div_enable(p_hw_vc);
    amhw_hc32_vc_div_sel(p_hw_vc, AMHW_HC32_VC_DIV_1);

    /* 选择 迟滞电压、功耗*/
    amhw_hc32_vc0_hys_sel(p_hw_vc, AMHW_HC32_VC0_HYS_10_MV);
    amhw_hc32_vc0_bias_sel(p_hw_vc, AMHW_HC32_VC0_BIAS_10_UA);

    /*  通道N端输入、通道P端输入配置 */
    amhw_hc32_vc0_n_sel(p_hw_vc, vc0_n);
    amhw_hc32_vc0_p_sel(p_hw_vc, vc0_p);

    /* 高电平触发中断 */
    amhw_hc32_vc0_out_tri_int_sel(p_hw_vc,
                                    AMHW_HC32_VC_OUT_TRI_INT_HIGH_LEVEL);

    /* VC0结果使能输出到Advanced Timer刹车 */
    amhw_hc32_vc_outcfg_enable(p_hw_vc,
                                 AMHW_HC32_VC_OUT_CFG_FIT_DATA_TIM0_1_2_3_EN,
                                 AMHW_HC32_VC0);
    ...
    /* 滤波使能 */
    amhw_hc32_vc_flt_disable (p_hw_vc, AMHW_HC32_VC0);

    /* 输出触发中断使能 */
    amhw_hc32_vc_int_enable (p_hw_vc, AMHW_HC32_VC0);
...
static am_bool_t __hc32_hw_tim_init (amhw_hc32_tim_t *p_hw_tim,
                                       uint8_t            chan,
                                       uint32_t           duty_ns,
                                       uint32_t           period_ns)
{
    ...
    amhw_hc32_tim_mode23_dtr_enable(p_hw_tim, AMHW_HC32_TIM_DTR_AOE);

    /* TIM1/2使用本身的的刹车控制 */
    amhw_hc32_tim_mode23_bksel_mine(p_hw_tim);

    /* 刹车动作产生后输出电平选择 */
    if((chan % 2) == 0) {
        amhw_hc32_tim_mode23_bksa_set(p_hw_tim,
                                        chan,
                                        AMHW_HC32_TIM_CMP_BRAKEOUT_LOW);
    } else {
        amhw_hc32_tim_mode23_bksb_set(p_hw_tim,
                                        chan,
                                        AMHW_HC32_TIM_CMP_BRAKEOUT_LOW);
    }

    /* 刹车BK输入相位同相（高电平刹车） */
    amhw_hc32_tim_mode23_phase_same(p_hw_tim, AMHW_HC32_TIM_PHASE_BKP);

    /* 刹车输入滤波（pclk 3个连续有效） */
    amhw_hc32_tim_mode23_fliter_set(p_hw_tim,
                                      AMHW_HC32_TIM_FLITER_TYPE_FLTBK,
                                      AMHW_HC32_TIM_FILTER_PCLKDIV1_3);

    /* VC刹车使能 */
    amhw_hc32_tim_mode23_dtr_enable(p_hw_tim, AMHW_HC32_TIM_DTR_VCE);
                                      ...
                                      uint8_t       vc0_p)
{
    ...
    /* 定时器初始化配置 */
    if(__hc32_hw_tim_init(p_tim, chan, duty_ns, period_ns) ==
       AM_FALSE) {
        return;
    }
    ...
    if(__vc_init(p_vc, 
			           (amhw_hc32_vc0_n_sel_t)vc0_n, 
		             (amhw_hc32_vc0_p_sel_t)vc0_p) == AM_FALSE) {
        return;
    }

    /* 使能定时器TIM允许计数 */
    amhw_hc32_tim_enable(p_tim);

    /* 使能电压比较器 */
    amhw_hc32_vc_volt_cmp_enable(p_vc, AMHW_HC32_VC0);

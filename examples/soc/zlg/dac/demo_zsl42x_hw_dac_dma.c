 ...
 *
 * \par 源代码
 * \snippet demo_zsl42x_hw_dac_dma.c src_zsl42x_hw_dac_dma
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsl42x_hw_dac_dma
 * \copydoc demo_zsl42x_hw_dac_dma.c
 */

/** [src_zsl42x_hw_dac_dma] */
...
/*******************************************************************************
  宏定义
*******************************************************************************/
static amhw_zsl42x_dac_t      *gp_hw_dac = NULL;     /**< \brief dac 外设 */
 ...
 * \brief DAC DMA传输配置
 */
static int dac_dma_tran_cfg (amhw_zsl42x_dac_t *p_hw_dac,
                             int32_t           dma_chan)
{
    ...
    /* 连接DMA中断服务函数 */
    am_zsl42x_dma_isr_connect(dma_chan, dac_dma_isr, (void *)dma_chan);

    /* DMA 传输配置 */
    flags = AMHW_ZSL42x_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
            AMHW_ZSL42x_DMA_CHAN_SIZE_16BIT             | /* 数据位宽 8bit */
                        ...
                        uint32_t           verf)
{
    ...
    /* 触发方式选择  软件触发*/
    amhw_zsl42x_dac_chan_trg_sel(p_hw_dac, AMHW_ZSL42x_DAC_CHAN_SOFTWARE_TRG);
 ...
 * \brief DAC DMA初始化
 */
static void dac_hw_dma_init(amhw_zsl42x_dac_t *p_hw_dac)
{
    if (p_hw_dac == NULL){
        return ;
    }
    ...
    /* 使能DAC DMA功能 */
    amhw_zsl42x_dac_chan_dma_enbale (p_hw_dac);
}
                                ...
                                uint16_t *p_vol_val)
{
    ...
    gp_hw_dac = (amhw_zsl42x_dac_t *)p_hw_dac;

    /* DAC 初始化 默认参考电压类型 外部参考电压源  PB01*/
    dac_hw_init(gp_hw_dac, AMHW_ZSL42x_DAC_CHAN_MASK_AVCC_VOLT);
    ...
    AM_FOREVER {
        ...
        /* 软件触发 */
        amhw_zsl42x_dac_chan_software_trg_enable(gp_hw_dac);

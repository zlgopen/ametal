 ...
 *
 * \par 源代码
 * \snippet demo_zsn700_drv_dma_m2m.c src_zsn700_drv_dma_m2m
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsn700_drv_dma_m2m
 * \copydoc demo_zsn700_drv_dma_m2m.c
 */

/** [src_zsn700_drv_dma_m2m] */
#include "ametal.h"
#include "am_delay.h"
#include "am_vdebug.h"
...
static amhw_zsn700_dma_xfer_desc_t g_desc; /**< \brief 描述符 */
...
static int dma_m2m_test (uint8_t dma_chan)
{
    ...
    /* 连接 DMA 中断服务函数 */
    am_zsn700_dma_isr_connect(dma_chan, dma_isr, (void *)0);

    /* DMA 传输配置 */
    flags = AMHW_ZSN700_DMA_CHAN_TRANSFER_MODE_BURST   |    /* 突发传输模式 */
            ...
            AMHW_ZSN700_DMA_CHAN_RELOAD_COUNTER_ENABLE |    /* 重载传输数据量 */
            AMHW_ZSN700_DMA_CHAN_RELOAD_SRC_ADD_ENABLE |    /* 重载源地址 */
            AMHW_ZSN700_DMA_CHAN_RELOAD_DST_ADD_ENABLE |    /* 重载目标地址 */
            AMHW_ZSN700_DMA_CHAN_INT_ERR_DISABLE       |    /* 错误中断禁能 */
            AMHW_ZSN700_DMA_CHAN_INT_TX_CMP_ENABLE     |    /* 传输完成中断使能 */
            AMHW_ZSN700_DMA_CHAN_CIRCULAR_MODE_ENABLE ;     /* 循环工作模式 */
    ...
    if (am_zsn700_dma_xfer_desc_chan_cfg(&g_desc,
                                       AMHW_ZSN700_DMA_MER_TO_MER, /* 内存到内存 */
                                       dma_chan) == AM_ERROR) {
        return AM_ERROR;
    } else {
        ...
        /* 设置传输请求信号来源为软件请求 */
        am_zsn700_dma_chan_src_set (dma_chan, ZSN700_DMA_SRC_TYPE_SOFT);
    ...
    /* 软件请求触发DMA传输 */
    am_zsn700_dma_chan_soft_ask_start(dma_chan);

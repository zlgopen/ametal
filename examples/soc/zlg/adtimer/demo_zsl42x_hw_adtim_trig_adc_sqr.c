 ...
 *     则需要更改demo_zsl42x_hw_adtim_trig_adc_sqr.c中定时器分频系数的设定。
 *   3.不同情况下，ADC的转换速度有限，需要考虑定时器的周期时间是否大于ADC转换时间（多个通道则需要累加转换时间）
 *
 * \par 源代码
 * \snippet demo_zsl42x_hw_adtim_trig_adc_sqr.c src_zsl42x_hw_adtim_trig_adc_sqr
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsl42x_hw_adtim_trig_adc_sqr
 * \copydoc demo_zsl42x_hw_adtim_trig_adc_sqr.c
 */

/** [src_zsl42x_hw_adtim_trig_adc_sqr] */
#include "ametal.h"
#include "am_delay.h"
#include "am_int.h"
#include "am_board.h"
...
static am_bool_t __zsl42x_hw_adtim_init (amhw_zsl42x_adtim_t *p_hw_adtim,
                                         uint32_t             period_us)
{
    amhw_zsl42x_adtim_basecnt_cfg_t   basecnt;
    amhw_zsl42x_adtim_irq_trig_cfg_t irq_trig;
    ...
    amhw_zsl42x_adtim_irqtrigcfg(p_hw_adtim, &irq_trig);
...
static void __zsl42x_hw_adc_init (amhw_zsl42x_adtim_t *p_hw_adtim,
                                  ...
                                  uint8_t              sqr_num)
{
    ...
    /* 外部中断触发源设置 */
    if((uint32_t)p_hw_adtim == ZSL42x_TIM4_BASE) {
        amhw_zsl42x_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZSL42x_ADC_INT_TRIGGER_TIM4);
    } else if((uint32_t)p_hw_adtim == ZSL42x_TIM5_BASE) {
        amhw_zsl42x_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZSL42x_ADC_INT_TRIGGER_TIM5);
    } else if((uint32_t)p_hw_adtim == ZSL42x_TIM6_BASE) {
        amhw_zsl42x_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZSL42x_ADC_INT_TRIGGER_TIM6);
    }
                                            ...
                                            uint8_t  sqr_num)
{
    ...
    /* 定时器配置 */
    if((__zsl42x_hw_adtim_init(p_adtim, period_us)) == AM_FALSE) {
        return;
    }

    /* ADC配置 */
    __zsl42x_hw_adc_init(p_adtim, p_adc, adc_int_num, p_adc_chan, sqr_num);

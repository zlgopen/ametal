 ...
 *
 * \par 源代码
 * \snippet demo_zsn700_hw_tim_cap.c src_zsn700_hw_tim_cap
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsn700_hw_tim_cap
 * \copydoc demo_zsn700_hw_tim_cap.c
 */

/** [src_zsn700_hw_tim_cap] */
#include "ametal.h"
#include "am_cap.h"
#include "am_int.h"
#include "am_delay.h"
...
static void __zsn700_tim_hw_cap_irq_handler (void *p_arg)
{
    ...
    for(i = 0; i < chans_num;i = i + 2) {
           if ((amhw_zsn700_tim_mode23_int_flag_check(p_hw_tim,
               ...
               /* 得到对应通道的值 */
               value = amhw_zsn700_tim_mode23_ccrxy_get(p_hw_tim, i) +
                       (__update_num << 16ul);
 ...
 * \brief 配置定时器为输入捕获
 */
static void tim_cap_chan_config (amhw_zsn700_tim_t *p_hw_tim,
                                 uint32_t           chan,
                                 uint32_t           cap_edge)
{

     amhw_zsn700_tim_filter_type_t filter_type =
                                       AMHW_ZSN700_TIM_FLITER_TYPE_FLTA0;
    ...
    case 1:
        filter_type = AMHW_ZSN700_TIM_FLITER_TYPE_FLTB0;
        break;

    case 2:
        filter_type = AMHW_ZSN700_TIM_FLITER_TYPE_FLTA1;
        break;

    case 3:
        filter_type = AMHW_ZSN700_TIM_FLITER_TYPE_FLTB1;
        break;

    case 4:
        filter_type = AMHW_ZSN700_TIM_FLITER_TYPE_FLTA2;
        break;

    case 5:
        filter_type = AMHW_ZSN700_TIM_FLITER_TYPE_FLTB2;
        break;
...
static void tim_cap_enable (amhw_zsn700_tim_t *p_hw_tim,
                            uint32_t           chan,
                            uint8_t            int_num)
{

    /* 中断连接并使能 */
    am_int_connect(int_num, __zsn700_tim_hw_cap_irq_handler, (void *)p_hw_tim);
 ...
 * \brief 定时器输入捕获初始化函数
 */
static void tim_cap_init (amhw_zsn700_tim_t *p_hw_tim, uint32_t chan)
{
    /* 设置定时器模式2 */
    ...
    /* 设置通道为捕获模式 */
    if((chan % 2) == 0) {
        amhw_zsn700_tim_mode23_csa_cap(p_hw_tim, chan);
    } else {
        amhw_zsn700_tim_mode23_csb_cap(p_hw_tim, chan);
    }

 ...
 *       要更改demo_zsl42x_hw_tim_pwm_dead.c中的分频系数，外设时钟两个参数。
 *
 * \par 源代码
 * \snippet demo_zsl42x116_hw_tim_pwm_dead.c src_zsl42x116_hw_tim_pwm_dead
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsl42x116_hw_tim_pwm_dead
 * \copydoc demo_zsl42x116_hw_tim_pwm_dead.c
 */

/** [src_zsl42x116_hw_tim_pwm_dead] */
#include "ametal.h"
#include "am_clk.h"
#include "am_clk.h"
#include "am_int.h"
                                       ...
                                       uint16_t           dead_us)
{
    ...
    /* 设置通道A为比较模式 */
    amhw_zsl42x_tim_mode23_csa_compare(p_hw_tim, chan_a);

    /* 设置通道 B为比较模式 */
    amhw_zsl42x_tim_mode23_csb_compare(p_hw_tim, chan_b);
    ...
    /* 设置某一通道A的比较值 */
    amhw_zsl42x_tim_mode23_ccr_set(p_hw_tim, chan_a, duty_c - 1);

    /* 设置某一通道B的比较值 */
    amhw_zsl42x_tim_mode23_ccr_set(p_hw_tim, chan_b, duty_c - 1);

    /* 设置通道A的PWM输出模式为PWM2 */
    amhw_zsl42x_tim_mode23_compare_set(
       p_hw_tim,
       (amhw_zsl42x_tim_compare_type_t)(chan_a * 4),
       AMHW_ZSL42x_TIM_COMPARE_PWM2);

    /* 设置通道B的PWM输出模式为PWM2 */
    amhw_zsl42x_tim_mode23_compare_set(
       p_hw_tim,
       (amhw_zsl42x_tim_compare_type_t)(chan_b * 4),
       AMHW_ZSL42x_TIM_COMPARE_PWM2);

    /* 通道A正常输出 */
    amhw_zsl42x_tim_mode23_phase_same(
       p_hw_tim,
       (amhw_zsl42x_tim_phase_type_t)(chan_a * 4 + 3));

    /* 通道A正常输出 */
    amhw_zsl42x_tim_mode23_phase_same(
       p_hw_tim,
       (amhw_zsl42x_tim_phase_type_t)(chan_b * 4 + 3));

    /* 清除标志位 */
    amhw_zsl42x_tim_mode23_int_flag_clr(p_hw_tim, AMHW_ZSL42x_TIM_INT_FLAG_ALL);

    /* 死区控制使能  */
    amhw_zsl42x_tim_mode23_dtr_enable(p_hw_tim, AMHW_ZSL42x_TIM_DTR_DTEN);

    deadtime = (float)dead_us * (float)clkfreq / (float)(fre_div * 1000000) + 0.5;

    /* 死区时间设置  */
    amhw_zsl42x_tim_mode23_dtr_set(p_hw_tim, (uint8_t)deadtime);

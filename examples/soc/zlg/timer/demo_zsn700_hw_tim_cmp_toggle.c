 ...
 *
 * \par 源代码
 * \snippet demo_zsn700_hw_tim_cmp_toggle.c src_zsn700_hw_tim_cmp_toggle
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsn700_hw_tim_cmp_toggle
 * \copydoc demo_zsn700_hw_tim_cmp_toggle.c
 */

/** [src_zsn700_hw_tim_cmp_toggle] */
#include "ametal.h"
#include "am_int.h"
#include "am_delay.h"
#include "am_vdebug.h"
 ...
 * \brief CMP 中断服务函数
 */
static void __zsn700_tim_cmp_irq_handler (void *p_arg)
{
 ...
 */
static void tim_cmp_toggle_chan_config (amhw_zsn700_tim_t *p_hw_tim,
                                        uint32_t        chan,
                                        uint32_t        cnt,
                                        uint8_t         clk_div)
{
    ...
    /* 分频系数设置 */
    amhw_zsn700_tim_mode_clkdiv_set(p_hw_tim,
                                    (amhw_zsn700_tim_clkdiv_t)clk_div);
    ...
    /* 设置自动重装寄存器的值 */
    amhw_zsn700_tim_arr_count_set(p_hw_tim, match - 1);

    /* 清零计数器 */
    amhw_zsn700_tim_cnt16_count_set(p_hw_tim, 0);

    /* 设置某一通道的比较值 */
    amhw_zsn700_tim_mode23_ccrxy_set(p_hw_tim, chan, match / 2 - 1);

    /* 比较匹配时翻转 */
    amhw_zsn700_tim_mode23_compare_set(
        p_hw_tim,
        (amhw_zsn700_tim_compare_type_t)(chan * 4),
        AMHW_ZSN700_TIM_COMPARE_FIT_TOGGLE);
...
void tim_cmp_toggle_enable (amhw_zsn700_tim_t     *p_hw_tim,
                            ...
                            uint8_t                int_num)
{
    ...
    /* 连接中断回调函数 */
    am_int_connect(int_num, __zsn700_tim_cmp_irq_handler, (void *)p_hw_tim);
 ...
 */
void tim_cmp_toggle_init (amhw_zsn700_tim_t     *p_hw_tim,
                          amhw_zsn700_tim_type_t type,
                          uint8_t                chan)
{
                                          ...
                                          uint8_t  inum)
{
    ...
    /* 初始化定时器为输出比较通道翻转功能 */
    tim_cmp_toggle_init(p_tim, (amhw_zsn700_tim_type_t)type, chan);
    tim_cmp_toggle_chan_config(p_tim, chan, clk_rate / 20 , clk_div);
    tim_cmp_toggle_enable(p_tim, (amhw_zsn700_tim_type_t)type, chan,inum);

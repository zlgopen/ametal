 ...
 * \brief PWM 中断服务函数
 */
static void __zsn700_tim_hw_timing_irq_handler (void *p_arg)
{
 ...
 * \return 无
 */
void tim_timing_chan_config (amhw_zsn700_tim_t *p_hw_tim, uint32_t cnt)
{
    uint32_t match;
    uint16_t pre_real = 1;
    ...
    /* 设置自动重装寄存器的值 */
    amhw_zsn700_tim_arr_count_set(p_hw_tim, 65535 - (match - 1));

    /* 重置计数器 */
    amhw_zsn700_tim_cnt16_count_set(p_hw_tim, 65535 - (match - 1));
}

/**
 * \brief 使能定时器为定时功能
 */
void tim_timing_enable (amhw_zsn700_tim_t *p_hw_tim, uint8_t int_num)
{

    /* 连接中断回调函数 */
    am_int_connect(int_num, __zsn700_tim_hw_timing_irq_handler, (void *)p_hw_tim);
 ...
 */
void tim_timing_init (amhw_zsn700_tim_t *p_hw_tim, amhw_zsn700_tim_type_t type)
{
    /* 16位重载计数器 */
                                      ...
                                      int32_t  int_num)
{
    ...
    /* 初始化定时器为定时功能 */
    tim_timing_init(p_tim, (amhw_zsn700_tim_type_t)type);

 ...
 *     则需要更改demo_zlg118_hw_tim_trig_adc_sqr.c中定时器分频系数的设定。
 *   3.不同情况下，ADC的转换速度有限，需要考虑定时器的周期时间是否大于ADC转换时间（多个通道则需要累加转换时间）
 *
 * \par 源代码
 * \snippet demo_zlg118_hw_tim_trig_adc_sqr.c src_zlg118_hw_tim_trig_adc_sqr
 *
 * \internal
...
/**
 * \addtogroup demo_if_zlg118_hw_tim_trig_adc_sqr
 * \copydoc demo_zlg118_hw_tim_trig_adc_sqr.c
 */

/** [src_zlg118_hw_tim_trig_adc_sqr] */
#include "ametal.h"
#include "am_delay.h"
#include "am_int.h"
#include "am_board.h"
                                       ...
                                       uint32_t           period_us)
{
    ...
    /* 使能ADC触发全局控制（总开关） */
    amhw_zlg118_tim_mode23_adc_trigger_enable(
        p_hw_tim,
        AMHW_ZLG118_TIM_ADC_TRIGGER_ADTE);

    /* 使能事件更新触发ADC（实际事件选择） */
    amhw_zlg118_tim_mode23_adc_trigger_enable(
        p_hw_tim,
        AMHW_ZLG118_TIM_ADC_TRIGGER_UEVE);

    return AM_TRUE;
}

static void __zlg118_hw_adc_init (amhw_zlg118_tim_t *p_hw_tim,
                                  ...
                                  uint8_t            sqr_num)
{
    ...
    /* 外部中断触发源设置 */
    if((uint32_t)p_hw_tim == ZLG118_TIM0_BASE) {
        amhw_zlg118_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZLG118_ADC_INT_TRIGGER_TIM0);
    } else if((uint32_t)p_hw_tim == ZLG118_TIM1_BASE) {
        amhw_zlg118_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZLG118_ADC_INT_TRIGGER_TIM1);
    } else if((uint32_t)p_hw_tim == ZLG118_TIM2_BASE) {
        amhw_zlg118_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZLG118_ADC_INT_TRIGGER_TIM2);
    } else if((uint32_t)p_hw_tim == ZLG118_TIM3_BASE) {
        amhw_zlg118_adc_sgl_sqr_trigger_src_set(
            p_hw_adc,
            AMHW_ZLG118_ADC_INT_TRIGGER_TIM3);
    }
                                            ...
                                            uint8_t  sqr_num)
{
    ...
    /* 定时器配置 */
    if((__zlg118_hw_tim_init(p_tim, type, chan, duty_us , period_us)) ==
       AM_FALSE) {
        return;
    }

    /* ADC配置 */
    __zlg118_hw_adc_init(p_tim, p_adc, adc_int_num, p_adc_chan, sqr_num);

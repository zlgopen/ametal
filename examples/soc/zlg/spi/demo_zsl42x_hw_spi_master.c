 ...
 *
 * \par 源代码
 * \snippet demo_zsl42x_hw_spi_master.c src_zsl42x_hw_spi_master
 *
 * \internal
 * \par History
...
/**
 * \addtogroup demo_if_zsl42x_hw_spi_master
 * \copydoc demo_zsl42x_hw_spi_master.c
 */
 
/** [src_zsl42x_hw_spi_master] */
#include "ametal.h"
#include "am_gpio.h"
#include "am_vdebug.h"
...
static void spi_master_cs_ctr (amhw_zsl42x_spi_t *p_hw_spi,
                               int                pin,
                               am_bool_t          state)
{
 ...
 * \brief SPI回环传输测试
 */
void spi_loop_trans (amhw_zsl42x_spi_t *p_hw_spi,
                     ...
                     int32_t         cs_pin)
{
    ...
    while(pos < p_trans->nbytes) {
        ...
        *(uint8_t*)((uint8_t*)p_trans->p_rxbuf + pos) =
             amhw_zsl42x_spi_rx_data8_get(p_hw_spi, AM_FALSE);
        pos += 1;
    }
                                    ...
                                    int32_t  cs_pin)
{
    ...
    amhw_zsl42x_spi_t *p_spi   = (amhw_zsl42x_spi_t *)p_hw_spi;
    ...
    /* 配置为主机模式 */
    amhw_zsl42x_spi_mode_sel(p_spi, AMHW_ZSL42x_SPI_MODE_TYPE_MASTER);

    /* 配置SPI模式 */
    amhw_zsl42x_spi_clk_mode_set(p_spi, SPI_CFG_MODE_1);

    /* 设置SPI速率 */
    amhw_zsl42x_spi_baud_rate_set(p_spi, AMHW_ZSL42x_SPI_BAUD_RATE_PCLK_DIV16);

    /* SPI使能 */
    amhw_zsl42x_spi_enable(p_spi, AM_TRUE);

...
static void spi_master_cs_ctr (amhw_zlg118_spi_t *p_hw_spi,
                               int                pin,
                               am_bool_t          state)
{
 ...
 * \brief SPI回环传输测试
 */
void spi_loop_trans (amhw_zlg118_spi_t *p_hw_spi,
                     ...
                     int32_t         cs_pin)
{
    ...
    while(pos < p_trans->nbytes) {
        ...
        while(amhw_zlg118_spi_flag_check (p_hw_spi,
            ...
            amhw_zlg118_spi_tx_data8_put(p_hw_spi,
                                         *((uint8_t*)(p_trans->p_txbuf) + pos),
                                         AM_FALSE);

        /* 等待可以接收 */
        while (amhw_zlg118_spi_flag_check (p_hw_spi,
                                           AMHW_ZLG118_SPI_FLAG_RX_NOT_EMPTY) ==
                                           AM_FALSE);

        *(uint8_t*)((uint8_t*)p_trans->p_rxbuf + pos) =
             amhw_zlg118_spi_rx_data8_get(p_hw_spi, AM_FALSE);
        pos += 1;
    }
                                      ...
                                      int32_t  cs_pin)
{
    ...
    amhw_zlg118_spi_t *p_spi   = (amhw_zlg118_spi_t *)p_hw_spi;
    ...
    /* 配置为主机模式 */
    amhw_zlg118_spi_mode_sel(p_spi, AMHW_ZLG118_SPI_MODE_TYPE_MASTER);

    /* 配置SPI模式 */
    amhw_zlg118_spi_clk_mode_set(p_spi, SPI_CFG_MODE_1);

    /* 设置SPI速率 */
    amhw_zlg118_spi_baud_rate_set(p_spi, AMHW_ZLG118_SPI_BAUD_RATE_PCLK_DIV16);

    /* SPI使能 */
    amhw_zlg118_spi_enable(p_spi, AM_TRUE);

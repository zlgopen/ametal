 ...
 */
static uint8_t lpuart_int_send (amhw_zsl42x_lpuart_t *p_hw_lpuart,
                                const uint8_t   *p_buf,
                                uint32_t         len)
{
    if (g_tx_index >= g_tx_nbytes)  {   /* 上次发送已经发送完成 */
        ...
        amhw_zsl42x_lpuart_data_write(p_hw_lpuart, p_buf[g_tx_index++]);
...
{
    amhw_zsl42x_lpuart_t *p_hw_lpuart = (amhw_zsl42x_lpuart_t *)p_arg;
    uint8_t             data;

    if (amhw_zsl42x_lpuart_flag_check(p_hw_lpuart,
        ...
        if(g_tx_index < g_tx_nbytes) {
            ...
            /* 缓冲区还有待发送数据 */
            amhw_zsl42x_lpuart_data_write(p_hw_lpuart, gp_tx_buf[g_tx_index++]);
 ...
 */
void lpuart_int_init (amhw_zsl42x_lpuart_t   *p_hw_lpuart,
                      void                (*pfn_init) (void),
                      uint8_t               clk_src,
                      uint32_t              clk_rate,
                      unsigned char         inum_lpuart)
{
    ...
    amhw_zsl42x_lpuart_clk_div_sel(p_hw_lpuart,
                                   AMHW_ZSL42x_LPUART_SCLK_DIV_MODE13_4);
...
void demo_zsl42x_hw_lpuart_int_entry (void             *p_hw_lpuart,
                                    ...
                                    unsigned char     inum_lpuart)
{
    amhw_zsl42x_lpuart_t *p_lpuart = (amhw_zsl42x_lpuart_t *)p_hw_lpuart;

 ...
 *
 * \par 源代码
 * \snippet demo_zsn700_hw_lpuart_tx_dma.c src_zsn700_hw_lpuart_tx_dma
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsn700_hw_lpuart_tx_dma
 * \copydoc demo_zsn700_hw_lpuart_tx_dma.c
 */

/** [src_zsn700_hw_lpuart_tx_dma] */
#include "ametal.h"
#include "am_int.h"
...
/** \brief DMA 重载描述符 */
static amhw_zsn700_dma_xfer_desc_t g_desc;
...
static amhw_zsn700_lpuart_t *gp_hw_lpuart = NULL; /**< \brief LPUART 外设 */
...
static void lpuart_dma_isr (void *p_arg , uint32_t flag)
{
    ...
    if (flag == AM_ZSN700_DMA_INT_COMPLETE) {
        if (flag_chan == g_dma_chan) {
            ...
            /* 禁能LPUART发送时使用DMA传输 */
            amhw_zsn700_lpuart_disable(gp_hw_lpuart, AMHW_ZSN700_LPUART_DMA_TX);
 ...
 * \brief DMA传输配置。
 */
static int lpuart_tx_dma_tran_cfg (amhw_zsn700_lpuart_t *p_hw_lpuart,
                                   int32_t             dma_chan,
                                   ...
                                   uint32_t            dma_tran_len)
{
    ...
    /* DMA 传输配置 */
       flags = AMHW_ZSN700_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
               AMHW_ZSN700_DMA_CHAN_SIZE_8BIT              | /* 数据位宽 8bit */

               AMHW_ZSN700_DMA_CHAN_SRC_ADD_INC_ENABLE     | /* 源地址自增 */
               AMHW_ZSN700_DMA_CHAN_RELOAD_SRC_ADD_DISABLE | /* 源地址不重载*/
               ...
               AMHW_ZSN700_DMA_CHAN_CIRCULAR_MODE_DISABLE ;  /* 关闭循环模式 */
    ...
    if (am_zsn700_dma_xfer_desc_chan_cfg(&g_desc,
                                         AMHW_ZSN700_DMA_PER_TO_MER,/* 内存到外设 */
                                         dma_chan) == AM_ERROR) {
        return AM_ERROR;
    }
...
{
    /* 串口发送DMA传输使能 */
    amhw_zsn700_lpuart_enable(p_hw_lpuart, AMHW_ZSN700_LPUART_DMA_TX);
 ...
 * \brief 例程入口
 */
void demo_zsn700_hw_lpuart_tx_dma_entry (void    *p_hw_lpuart,
                                         uint8_t  clk_src,
                                         uint32_t clk_rate,
                                         uint32_t baudrate,
                                         int32_t  dma_chan,
                                         int32_t  dma_src)
{
    ...
    while (1) {
        ...
        if (g_trans_done == AM_TRUE) {
            ...
            if (j < 3) {
                am_zsn700_dma_chan_stop(dma_chan);
                if (0 == i) {
                    lpuart_tx_dma_tran_cfg(gp_hw_lpuart,
                                         dma_chan,
                                         g_buf_dst,
...
/** [src_zsn700116_hw_lpuart_tx_dma] */

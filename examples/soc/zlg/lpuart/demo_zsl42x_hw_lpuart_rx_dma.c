 ...
 *
 * \par 源代码
 * \snippet demo_zsl42x_hw_lpuart_rx_dma.c src_zsl42x_hw_lpuart_rx_dma
 *
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsl42x_hw_lpuart_rx_dma
 * \copydoc demo_zsl42x_hw_lpuart_rx_dma.c
 */

/** [src_zsl42x_hw_lpuart_rx_dma] */
#include "ametal.h"
#include "am_int.h"
...
/*******************************************************************************
  全局变量
*******************************************************************************/
static amhw_zsl42x_dma_xfer_desc_t g_desc;           /**< \brief DMA 重载描述符 */
...
static amhw_zsl42x_lpuart_t    *gp_hw_lpuart = NULL; /**< \brief LPUART 外设 */
...
static void lpuart_dma_isr (void *p_arg , uint32_t flag)
{
    ...
    if (flag == AM_ZSL42x_DMA_INT_COMPLETE) {
        if (flag_chan == g_dma_chan) {
            ...
            /* 禁能LPUART发送时使用DMA传输 */
            amhw_zsl42x_lpuart_disable(gp_hw_lpuart, AMHW_ZSL42x_LPUART_DMA_RX);
 ...
 */
static int lpuart_rx_dma_tran_cfg (amhw_zsl42x_lpuart_t *p_hw_lpuart,
                                 int32_t          dma_chan,
                                 uint32_t         dma_tran_len)
{
    ...
    /* 连接DMA中断服务函数 */
    am_zsl42x_dma_isr_connect(dma_chan, lpuart_dma_isr, (void *)dma_chan);
    ...
    flags = AMHW_ZSL42x_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
            ...
            AMHW_ZSL42x_DMA_CHAN_DST_ADD_INC_ENABLE     | /* 目标地址自增 */
    ...
    if (am_zsl42x_dma_xfer_desc_chan_cfg(&g_desc,
                                          AMHW_ZSL42x_DMA_PER_TO_MER, /* 外设到 内存 */
 ...
 * \brief LPUART 初始化
 */
static void lpuart_hw_init (amhw_zsl42x_lpuart_t *p_hw_lpuart,
                            uint8_t               clk_src,
                            uint32_t              clk_rate)
{
    /* 工作模式设置  */
    ...
    /* 设置通信时钟源 */
    amhw_zsl42x_lpuart_clk_src_sel(p_hw_lpuart,
                                   (amhw_zsl42x_lpuart_sclk_src_t)clk_src);

    if(clk_src == AMHW_ZSL42x_LPUART_SCLK_SRC_XTL) {
    ...
    } else if(clk_src == AMHW_ZSL42x_LPUART_SCLK_SRC_RCL) {
    ...
    amhw_zsl42x_lpuart_clk_div_sel(p_hw_lpuart,
                                   AMHW_ZSL42x_LPUART_SCLK_DIV_MODE13_8);
    ...
    /* 停止位设置 */
    amhw_zsl42x_lpuart_stop_bit_sel(p_hw_lpuart,
                                    AMHW_ZSL42x_LPUART_STOP_1_0_BIT);

    /* 校验设置 */
    amhw_zsl42x_lpuart_parity_bit_sel(p_hw_lpuart,
                                      AMHW_ZSL42x_LPUART_PARITY_NO);

    /* 波特率设置 */
    amhw_zsl42x_lpuart_baudrate_set( p_hw_lpuart, clk_rate, lpuart_baudrate);
 ...
 */
static void lpuart_hw_dma_init (amhw_zsl42x_lpuart_t *p_hw_lpuart)
{
    /* 串口发送DMA传输使能 */
    amhw_zsl42x_lpuart_enable(p_hw_lpuart, AMHW_ZSL42x_LPUART_DMA_RX);
...
void demo_zsl42x_hw_lpuart_rx_dma_entry (void    *p_hw_lpuart,
                                       ...
                                       int32_t  dma_src)
{
    gp_hw_lpuart = (amhw_zsl42x_lpuart_t *)p_hw_lpuart;
    g_dma_chan = dma_chan;
    ...
    amhw_zsl42x_lpuart_poll_send(gp_hw_lpuart,
    ...
    amhw_zsl42x_lpuart_poll_send(gp_hw_lpuart, g_buf_dst, sizeof(g_buf_dst));
    amhw_zsl42x_lpuart_poll_send(gp_hw_lpuart,
                                 (uint8_t *)"\r\nDMA transfer done!\r\n",
                                  sizeof("\r\nDMA transfer done!\r\n") - 1);

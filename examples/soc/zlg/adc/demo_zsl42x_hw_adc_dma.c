 ...
 *
 * \par 源代码
 * \snippet demo_zsl42x_adc_dma.c src_zsl42x_adc_dma
 *
 * \internal
...
/**
 * \addtogroup demo_if_zsl42x_adc_dma
 * \copydoc demo_zsl42x_adc_dma.c
 */
...
static void dma_isr (void *p_arg , uint32_t flag)
{
    ...
    if (flag == AM_ZSL42x_DMA_INT_COMPLETE) {
        ...
        /* 顺序扫描转换停止 */
        amhw_zsl42x_adc_sqr_convert_stop((amhw_zsl42x_adc_t *)p_arg);

        g_trans_done = AM_TRUE;
    }
}

static void __zsl42x_adc_init (amhw_zsl42x_adc_t   *p_hw_adc,
                             int               *p_adc_chan,
                             int                chan_num)
{
    int i;
    ...
    /* 设置ADC时钟分频 */
    amhw_zsl42x_adc_pclk_div_sel(p_hw_adc, AMHW_ZSL42x_ADC_PCLK_DIV8);
    ...
    amhw_zsl42x_adc_sqr_num_set(p_hw_adc, chan_num - 1);

    /* 顺序扫描转换DMA使能*/
    amhw_zsl42x_adc_dma_sqr_enable(p_hw_adc);
                                 ...
                                 uint8_t  dma_src)
{
    ...
    uint32_t flags;
    int      i;
    static   amhw_zsl42x_dma_xfer_desc_t g_desc;  /**< \brief DMA描述符 */
    ...
    __zsl42x_adc_init(p_adc, p_adc_chan, chan_num);

    /* 连接DMA中断服务函数 */
    am_zsl42x_dma_isr_connect(dma_chan, dma_isr, (void *)p_adc);

    /* DMA 传输配置 */
    flags = AMHW_ZSL42x_DMA_CHAN_TRANSFER_MODE_BLOCK    | /* SPI只有块传输模式 */
            AMHW_ZSL42x_DMA_CHAN_SIZE_32BIT             | /* 数据位宽 32bit */
            ...
            AMHW_ZSL42x_DMA_CHAN_CIRCULAR_MODE_ENABLE ;   /* 开启循环模式 */
    ...
    am_zsl42x_dma_xfer_desc_chan_cfg(&g_desc,
    ...
    am_zsl42x_dma_block_data_size(dma_chan, chan_num);

...
void demo_zsl42x_hw_gpio_trigger_entry (void *p_hw_gpio, int32_t pin)
{
    ...
    /* 取消引脚的复用功能 */
    amhw_zsl42x_gpio_afio_set(p_gpio, AMHW_ZSL42x_AFIO_NO, pin);

    /* 数字端口设置*/
    amhw_zsl42x_gpio_pin_digital_set(p_gpio, pin);

    /* 引脚输入 */
    amhw_zsl42x_gpio_pin_dir_input(p_gpio, pin);

    /* 使能引脚的上拉电阻 */
    amhw_zsl42x_gpio_pin_pu_enable(p_gpio, pin);

    /* 下降沿触发 */
    amhw_zsl42x_gpio_pin_falling_int_enable(p_gpio, pin);

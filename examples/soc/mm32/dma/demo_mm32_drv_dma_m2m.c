 ...
 *
 * \par 源代码
 * \snippet demo_mm32_drv_dma_m2m.c src_mm32_drv_dma_m2m
 *
 * \internal
...
/**
 * \addtogroup demo_if_mm32_drv_dma_m2m
 * \copydoc demo_mm32_drv_dma_m2m.c
 */

/** [src_mm32_drv_dma_m2m] */
#include "ametal.h"
#include "am_delay.h"
#include "am_vdebug.h"
...
static amhw_mm32_dma_xfer_desc_t g_desc; /**< \brief 描述符 */
...
static int dma_m2m_test (uint8_t dma_chan)
{
    ...
    /* 连接 DMA 中断服务函数 */
    am_mm32_dma_isr_connect(dma_chan, dma_isr, (void *)0);

    /* DMA 传输配置 */
    flags = AMHW_MM32_DMA_CHAN_PRIORITY_HIGH         |  /* 中断优先级 高 */
            ...
            AMHW_MM32_DMA_CHAN_PER_ADD_INC_ENABLE    |  /* 外设地址自增 */
    ...
    if (am_mm32_dma_xfer_desc_chan_cfg(&g_desc,
                                       AMHW_MM32_DMA_MER_TO_MER, /* 内存到内存 */
                                       dma_chan) == AM_ERROR) {
        return AM_ERROR;
    } else {

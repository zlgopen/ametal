 ...
 *
 * \par 源代码
 * \snippet demo_mm32_drv_flash.c src_mm32_drv_flash
 *
 * \internal
...
/**
 * \addtogroup demo_if_mm32_drv_flash
 * \copydoc demo_mm32_drv_flash.c
 */
...
void demo_mm32_drv_flash_entry (amhw_mm32_flash_t *p_hw_flash, uint8_t sector)
{
    ...
    /* FLASH 初始化 */
    am_mm32_flash_init(p_hw_flash);

    /* 擦除扇区 */
    status = am_mm32_flash_sector_erase(p_hw_flash, sector * 1024);
    ...
    /* 向扇区中写入数据 */
    status = am_mm32_flash_flash_program(p_hw_flash,
                                        sector * 1024,
                                        data,
                                        1024 / 4);

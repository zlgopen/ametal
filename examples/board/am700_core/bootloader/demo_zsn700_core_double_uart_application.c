 ...
 * - 操作步骤:
 *
 *   1. eclipse工程打开demo_am700_core_application.ld文件，可设置应用程序代码起始地址，如下：
 *           FLASH (rx)  : ORIGIN = 0x00006800, LENGTH = 115K //应用程序代码起始地址不应超过0x6A00
 *
 *      keil工程打开demo_am700_core_application.sct文件，根据提示修改boot程序大小，如下：
 *          #define     ALL_START    0X00006800    //不建议超过0x6A00
 *          #define     ALL_SIZE     0X0001CC00    //根据实际情况设置，建议本分区大小与应用区大小一致
 *
 *      除上面工程链接文件之外，需要更改/user_config/../am_appconf_double_zsn700.c文件内的信息结构体设置
 *
 *      默认flash分配情况如下：
 *      区域划分:    |------------|-------|-----------------|-------------------|
...
#include "am_appconf_double_zsn700.h"
#include "ametal.h"
#include "am_vdebug.h"
#include "am_led.h"
#include "am_delay.h"
...
{
    AM_DBG_INFO("application : am700_core double application start up successful!\r\n");
    am_mdelay(1);
    ...
    am_zsn700_boot_inst_init();
    firmware_handle = am_zsn700_boot_firmware_flash_inst_init();
    ...
    msg_handle = am_zsn700_boot_msg_flag_inst_init();
    ...
    while(1) {
        ...
        am_led_toggle(0);
        am_mdelay(1000);
        AM_DBG_INFO("application : am700_core double application running!\r\n");
    }
}

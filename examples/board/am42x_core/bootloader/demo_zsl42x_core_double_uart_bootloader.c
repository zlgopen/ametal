 ...
 *
 * - 操作步骤（本地升级）：
 *   1. eclipse工程打开demo_am42x_core_bootloader.ld文件，可根据具体的boot程序大小设置参数，如下：
 *          FLASH  (rx) : ORIGIN = 0x00000000, LENGTH = 0X6600  //长度最大为26.5k
 *
 *      keil工程打开demo_am42x_core_bootloader.sct文件，根据提示修改boot程序大小，如下：
 *          #define     ALL_SIZE      0X00006600     //只可更改此参数
 *
 *      除上面工程链接文件之外，需要更改/user_config/../am_bootconf_zsl42x_double.c文件内的信息结构体设置
 *
 *      默认flash分配情况如下：
 *      区域划分:    |------------|-------|-----------------|-------------------|
...
void demo_zsl42x_core_double_bootloader_uart_entry (void)
{
    ...
    /* bootloader 标准接口初始化  */
    am_zsl42x_boot_double_inst_init();

    firmware_handle = am_zsl42x_boot_double_firmware_flash_inst_init();
    ...
    demo_std_double_bootloader_entry(
        ...
        am_zsl42x_boot_msg_flag_inst_init());
}

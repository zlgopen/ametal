...
void demo_zsl42x_core_mb_master_demo_entry (void)
{
    ...
    /* 串口设备信息初始化 */
    mb_master_info.serial.uart      = am_zsl42x_uart1_inst_init();
    mb_master_info.serial.baudrate  = 115200;         /* 波特率115200 */
    mb_master_info.serial.parity    = AM_MB_PAR_EVEN; /* 偶校验 */
    mb_master_info.serial.rs485_en  = AM_FALSE;       /* 不使能485功能 */
    /* 定时器设备信息初始化 */
    mb_master_info.timer.timer      = am_zsl42x_tim0_timing_inst_init ();

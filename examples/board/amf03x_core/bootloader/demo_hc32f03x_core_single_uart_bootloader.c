 ...
 *
 * - 操作步骤（本地升级）：
 *   1. eclipse工程打开demo_amf03x_core_bootloader.ld文件，可根据具体的boot程序大小设置参数，如下：
 *          FLASH  (rx) : ORIGIN = 0x00000000, LENGTH = 0X6600  //长度最大为26.5K
 *
 *      keil工程打开demo_amf03x_core_bootloader.sct文件，根据提示修改boot程序大小，如下：
 *          #define     ALL_SIZE      0X00006600     //只可更改此参数
 *
 *      除上面工程链接文件之外，需要更改/user_config/../am_bootconf_hc32f03x_single.c文件内的信息结构体设置
 *
 *      默认flash分配情况如下：
 *      区域划分:    |----------|----------------------------|
...
#include "am_bootconf_hc32f03x.h"

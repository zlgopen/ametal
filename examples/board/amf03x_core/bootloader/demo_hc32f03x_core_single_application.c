 ...
 *           FLASH (rx)  : ORIGIN = 0x00006C00, LENGTH = 37K
 *
 *      keil工程打开demo_amf03x_core_application.sct文件，根据提示修改boot程序大小，如下：
 *          #define     ALL_START    0X00006C00
 *          #define     ALL_SIZE     0X00009400    //根据实际情况设置
 *
 *      默认flash分配情况如下：
 *      区域划分:    |----------|-------------------|
...
{
    AM_DBG_INFO("application : amf03x_core single application start up successful!\r\n");

    while(1) {
        AM_DBG_INFO("application : amf03x_core single application running\r\n");
        am_led_toggle(0);
        am_mdelay(1000);
    }
}

 ...
 *   3. 可在am_hwconf_hc32f03x_rtc.c文件中配置1Hz输出控制，若使能，PIOB_14将输出1Hz的方波。
 *   4. 关闭时钟补偿的情况下，测得1Hz方波的实际频率，带入下列计算式计算：
 *           ppm = （1Hz方波的实际频率 - 1Hz）* 10^6
 *      将计算出的结果填入am_hwconf_hc32f03x_rtc.c的设备信息对应位置，可自行决定是否使能高速补偿时钟。
 ...
 *
 * \par 源代码
 * \snippet demo_hc32f03x_std_rtc.c src_hc32f03x_std_rtc
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32f03x_std_rtc
 * \copydoc demo_hc32f03x_std_rtc.c
 */

/** [src_hc32f03x_std_rtc] */
...
void demo_hc32f03x_core_drv_rtc_entry (void)
{
    ...
    AM_DBG_INFO("demo amf03x_core rtc entry!\r\n");

 ...
 *    2. 需要保证am_hwconf_zmf237_adc.c文件中的对应通道引脚的初始化注释打开；
 *    3. 如需观察串口打印的调试信息，需要将 PIOA_10 引脚连接 PC 串口的 TXD，
 *       PIOA_9 引脚连接 PC 串口的 RXD。
 *    4. 若用户用户需要添加ADC轮询读取通道的数目，或者变更读取通道的顺序，只需修改
 *       adc_chan[]数组中的通道值即可，注意需要在am_hwconf_zmf237_adc.c
 *       文件中将通道引脚的初始化注释打开。
 *
 * \par 源代码
 * \snippet demo_zmf237_core_std_multichan_adc.c src_zmf237_core_std_multichan_adc
 *
 * \internal
...
/**
 * \addtogroup demo_if_zmf237_core_std_multichan_adc
 * \copydoc demo_zmf237_core_std_multichan_adc.c
 */
 ...
 * \brief 例程入口
 */
void demo_zmf237_core_std_multichan_adc_entry (void)
{
    /* ADC通道0,1,2,3 */
    ...
    demo_std_multichan_adc_entry(am_zmf237_adc1_inst_init(),
                                 adc_chan,

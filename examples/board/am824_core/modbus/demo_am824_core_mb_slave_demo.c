...
/******************************************************************************/
/** \brief Modbus 从站测试示例 */
void am_mb_slave_test_entry (void)
{
    uint8_t     i       = 0;
    uint32_t    cur_ms;
    uint32_t    pre_ms;
    ...
    /* 串口设备信息初始化 */
    mb_slave_info.serial_info.uart      = am_lpc82x_usart1_inst_init();
    mb_slave_info.serial_info.baudrate  = 9600;         /* 从站波特率-9600 */
    mb_slave_info.serial_info.parity    = AM_MB_PAR_NONE; /* 检验-偶校验            */
    /* 硬件定时器设备信息初始化 */
    mb_slave_info.timer_info.timer      = am_lpc82x_sct0_timing_inst_init();

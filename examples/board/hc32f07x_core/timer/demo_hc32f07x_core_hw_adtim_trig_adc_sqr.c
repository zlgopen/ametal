 ...
 *     定其他时间值，则需要更改demo_hc32f07x_hw_adtim_trig_adc_sqr.c中定时器分频系数的设定。
 *   4.不同情况下，ADC的转换速度有限，需要考虑定时器的周期时间是否大于ADC转换时间（多个通道则需要累加转换时间）
 *
 * \par 源代码
 * \snippet demo_hc32f07x_hw_adtim_trig_adc_sqr.c src_hc32f07x_hw_adtim_trig_adc_sqr
 *
 * \internal
...
/**
 * \addtogroup demo_if_hc32f07x_hw_adtim_trig_adc_sqr
 * \copydoc demo_hc32f07x_hw_adtim_trig_adc_sqr.c
 */

/** [src_hc32f07x_hw_adtim_trig_adc_sqr] */
#include "ametal.h"
#include "am_gpio.h"
#include "am_vdebug.h"
#include "am_hc32.h"
...
{
    /* 16个顺序通道ADC采集 */
    int adc_chan[] = {AMHW_HC32_CHAN_AIN0_PA0,
    ...
    AM_DBG_INFO("demo amf07x_core hw adtim trig adc sqr!\r\n");

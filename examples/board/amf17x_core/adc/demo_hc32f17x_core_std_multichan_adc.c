 ...
 *
 * - 操作步骤：
 *   1. am_hwconf_hc32f17x_adc.c文件__g_adc_ioinfo_list[]数组的第一个有效元素引脚。默认是PA0
 *   2. am_hwconf_hc32f17x_adc.c文件__g_adc_ioinfo_list[]数组的第一个有效元素引脚。默认是PA1
 *   3. am_hwconf_hc32f17x_adc.c文件__g_adc_ioinfo_list[]数组的第一个有效元素引脚。默认是PA2
 *   4. am_hwconf_hc32f17x_adc.c文件__g_adc_ioinfo_list[]数组的第一个有效元素引脚。默认是PA3
 *
 * - 实验现象：
 *   1. 串口输出电压采样值。
 *
 * \note
 *    1. 需要保证am_hwconf_hc32f17x_adc.c文件中的对应通道引脚的初始化注释打开；

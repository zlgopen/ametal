 ...
 *
 * - 操作步骤:
 *    1. eclipse工程打开demo_aml166_core_application.ld文件，可设置应用程序代码起始地址，如下：
 *           FLASH (rx)  : ORIGIN = 0x00006800, LENGTH = 115K //应用程序代码起始地址不应超过0x6A00
 *
 *      keil工程打开demo_aml166_core_application.sct文件，根据提示修改boot程序大小，如下：
...
{
    AM_DBG_INFO("application : aml166_core single application start up successful!\r\n");

    while(1) {
        AM_DBG_INFO("application : aml166_core single application running\r\n");
        am_led_toggle(0);
        am_mdelay(1000);
    }
}

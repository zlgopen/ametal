 ...
 *
 * - 操作步骤：
 *   1. 使用杜邦线，将 PIOE_2 与 PIOE_13 连接。
 *
 * - 实验现象：
 *   1. TIMEA1 通过 PIOE_2 引脚输出 2KHz 的 PWM；
 *   2. ADTIM61 捕获输入通道 1 使用 PIOE_13 引脚捕获；
 ...
 *
 * \par 源代码
 * \snippet demo_hc32f460_std_tim_cap.c src_hc32f460_std_adtim_cap
 *
 * \internal
...
void demo_hc32f460_core_std_adtim63_cap_entry (void)
{
    ...
    am_cap_handle_t adtim63_cap_handle = am_hc32f460_adtim63_cap_inst_init();
    
    AM_DBG_INFO("demo hc32f460_core std adtim63 cap!\r\n");

    /* TIM0 鏉堟挸鍤０鎴犲芳娑擄拷 2KHz 閻拷 PWM */
    am_pwm_config(timea3_pwm_handle, TIMERA_CH5, 500000 / 2, 500000);
    am_pwm_enable(timea3_pwm_handle, TIMERA_CH5);

    demo_std_timer_cap_entry(adtim63_cap_handle, AMHW_HC32F460_ADTIM_CHX_A);
}
/** [src_hc32f460_std_tim0_cap] */

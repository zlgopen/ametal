 ...
 *
 * - 操作步骤：
 *   1. 使用杜邦线，将 PIOE_2 与 PIOD_12 连接。
 *
 * - 实验现象：
 *   1. TIMEA1 通过 PIOE_2 引脚输出 2KHz 的 PWM；
 *   2. TIMEA1 捕获输入通道 2 使用 PIOD_12 引脚捕获；
...
void demo_hc32f460_core_std_timea4_cap_entry (void)
{
    ...
    am_cap_handle_t timea4_cap_handle = am_hc32f460_timea4_cap_inst_init();
    
    AM_DBG_INFO("demo hc32f460_core std timea4 cap!\r\n");
    ...
    demo_std_timer_cap_entry(timea4_cap_handle, TIMERA_CH1);
}
/** [src_hc32f460_std_tim0_cap] */

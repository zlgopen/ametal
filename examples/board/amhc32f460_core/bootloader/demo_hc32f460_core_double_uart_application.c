 ...
 * - 操作步骤:
 *
 *   1. eclipse工程打开demo_hc32f460_core_application.ld文件，可设置应用程序代码起始地址，如下：
 *           FLASH (rx)  : ORIGIN = 0x00008000, LENGTH = 480K //应用程序代码起始地址不应超过0x6A00
 *
 *      keil工程打开demo_hc32f460_core_application.sct文件，根据提示修改boot程序大小，如下：
 *          #define     ALL_START    0X00008000    //不建议超过0x6A00
 *          #define     ALL_SIZE     0X00078000    //根据实际情况设置，建议本分区大小与应用区大小一致
 *
 *      除上面工程链接文件之外，需要更改/user_config/../am_appconf_double_hc32f460.c文件内的信息结构体设置
 *
 *      默认flash分配情况如下：
 *      区域划分:    |------------|-------|-----------------|-------------------|
 *      名称:        bootloader    flag      application        save_flie
 *      大小:         25.5k        6.5k          240k             240k
 *
 *   2.编译工程
 *   3.固件处理，参考下面note提示
 *6

 ...
 *            >=0                     : 角度值
 */
int am_mk100_get_angle(am_mk100_dev_t *p_this, uint8_t gear);
/**
 * \brief 获取磁旋钮位置
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >=0                     : 磁旋钮位置
 */             
int am_mk100_get_knob(am_mk100_dev_t *p_this);
/**
 ...
 *            AM_OK                   : 成功
 */                                 
int am_mk100_get_strength_threshold(am_mk100_dev_t *p_this, uint8_t* upper_limit, uint8_t* lower_limit);
/**
 * \brief 获取当前旋转方向
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >=0                     : 当前旋转方向
 */
int am_mk100_get_rotation_direction(am_mk100_dev_t *p_this);
/**
 ...
 *            AM_OK                   : 成功
 */
int am_mk100_get_strength_threshold_status(am_mk100_dev_t *p_this, am_bool_t* upper_limit_status, am_bool_t* lower_limit_status);
/**
 * \brief 获取方向传感器数据
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 该方向的数据
 */
int am_mk100_get_direction_data(am_mk100_dev_t *p_this, uint8_t direction);
/**
 * \brief 获取红外传感器数据
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的角度值
 */
int am_mk100_get_infrared_data(am_mk100_dev_t *p_this);
/**
 * \brief 设置当前角度值
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的角度值
 */
int am_mk100_set_angle(am_mk100_dev_t *p_this, uint16_t angle);
/**
 * \brief 设置当前磁场强度阈值上限
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的阈值上限
 */
int am_mk100_set_strength_upper_limit(am_mk100_dev_t *p_this, uint8_t limit);
/**
 * \brief 设置当前磁场强度阈值下限
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的阈值下限
 */
int am_mk100_set_strength_lower_limit(am_mk100_dev_t *p_this, uint8_t limit);
/**
 * \brief 设置当前旋转方向
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的方向
 */
int am_mk100_set_rotation_direction(am_mk100_dev_t *p_this, uint8_t direction);
/**
 * \brief 方向传感器初始值校准
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            AM_OK                   : 成功
 */
int am_mk100_direction_cal(am_mk100_dev_t *p_this);
/**
 * \brief 设置方向传感器参数
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的方向传感器参数
 */
int am_mk100_set_direction_parameter(am_mk100_dev_t *p_this, uint8_t direction, uint16_t threshold);
/**
 * \brief 设置红外传感器参数
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的红外传感器参数
 */
int am_mk100_set_infrared_parameter(am_mk100_dev_t *p_this, uint16_t data);
/**
 * \brief 设置系统波特率
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置的波特率
 */
int am_mk100_set_baudrate(am_mk100_dev_t *p_this, uint8_t baudrate_setting);
/**
 * \brief 设置系统功能
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设置
 */
int am_mk100_set_system_func(am_mk100_dev_t *p_this, uint8_t mode);
/**
 * \brief 设置系统为查询模式
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前模式
 */
int am_mk100_set_query_mode(am_mk100_dev_t *p_this);
/**
 ...
 *            >= 0                    : 当前模式
 */
int am_mk100_set_report_change_mode(am_mk100_dev_t *p_this);
/**
 ...
 *            >= 0                    : 当前模式
 */
int am_mk100_set_report_periodically_mode(am_mk100_dev_t *p_this, uint16_t time);
/**
 * \brief 设置旋钮灵敏度
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前灵敏度等级
 */
int am_mk100_set_knob_sensitivity(am_mk100_dev_t *p_this, uint8_t sensitivity);
/**
 * \brief 设置延时窗口宽度
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前延时窗口宽度
 */
int am_mk100_set_delay_window(am_mk100_dev_t *p_this, uint8_t delay_window);
/**
 * \brief 设置进入低功耗时间
 *
 * \param[in] handle                  : mk100句柄
 ...
 *            >= 0                    : 当前设定的时间
 */
int am_mk100_set_enter_low_power_time(am_mk100_dev_t *p_this, uint8_t enter_low_power_time);
/**
 ...
 *            AM_OK                   : 成功
 */
int am_mk100_get_report(am_mk100_dev_t *p_this, uint8_t* cmd_buff, uint32_t cmd_len);
/**
 * \brief 打印错误接收
 *
 * \param[in] handle              : mk100句柄
 * \return    无 
 */
void am_mk100_display_error_recevice(am_mk100_dev_t *p_this);
